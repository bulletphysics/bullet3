...
/// parts/materials it will split into separate obj files for each part/material
/// and create an sdf file with visuals/collisions pointing to the new obj files
...
int main(int argc, char* argv[]) {
  ...
  std::string err =
      tinyobj::LoadObj(shapes, fileNameWithPath, materialPrefixPath);
  ...
  if (mergeMaterials) {
    for (int m = 0; m < gMaterialNames.size(); m++) {
      ...
      for (int face = 0; face < faceCount; face += 3) {
        ...
        fprintf(
            f, "f %d/%d/%d %d/%d/%d %d/%d/%d\n", shapeCon->indices[face] + 1,
            shapeCon->indices[face] + 1, shapeCon->indices[face] + 1,
            shapeCon->indices[face + 1] + 1, shapeCon->indices[face + 1] + 1,
            shapeCon->indices[face + 1] + 1, shapeCon->indices[face + 2] + 1,
            shapeCon->indices[face + 2] + 1, shapeCon->indices[face + 2] + 1);
      }
      fclose(f);
    ...
    for (int s = 0; s < (int)shapes.size(); s++) {
      ...
      for (int v = 0; v < vertexCount / 3; v++) {
        fprintf(f, "v %f %f %f\n", shape.mesh.positions[v * 3 + 0],
                shape.mesh.positions[v * 3 + 1],
      ...
      for (int vn = 0; vn < numNormals / 3; vn++) {
        fprintf(f, "vn %f %f %f\n", shape.mesh.normals[vn * 3 + 0],
                shape.mesh.normals[vn * 3 + 1], shape.mesh.normals[vn * 3 + 2]);
      }

      fprintf(f, "\n");
      int numTexCoords = int(shape.mesh.texcoords.size());
      for (int vt = 0; vt < numTexCoords / 2; vt++) {
        fprintf(f, "vt %f %f\n", shape.mesh.texcoords[vt * 2 + 0],
                shape.mesh.texcoords[vt * 2 + 1]);
      }
      ...
      for (int face = 0; face < faceCount; face += 3) {
        ...
        fprintf(
            f, "f %d/%d/%d %d/%d/%d %d/%d/%d\n", shape.mesh.indices[face] + 1,
            shape.mesh.indices[face] + 1, shape.mesh.indices[face] + 1,
            shape.mesh.indices[face + 1] + 1, shape.mesh.indices[face + 1] + 1,
            shape.mesh.indices[face + 1] + 1, shape.mesh.indices[face + 2] + 1,
            shape.mesh.indices[face + 2] + 1, shape.mesh.indices[face + 2] + 1);
      }
      fclose(f);
