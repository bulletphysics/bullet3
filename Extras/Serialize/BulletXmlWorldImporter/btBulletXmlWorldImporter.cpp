...
static int get_int_attribute_by_name(const TiXmlElement* pElement,
  ...
  while (pAttrib) {
    if (!strcmp(pAttrib->Name(), attribName))
      if (pAttrib->QueryIntValue(value) == TIXML_SUCCESS) return 1;
...
void btBulletXmlWorldImporter::deSerializeVector3FloatData(
    TiXmlNode* pParent, btAlignedObjectArray<btVector3FloatData>& vectors) {
...
#define SET_INT_VALUE(xmlnode, targetdata, argname)       \
  btAssert((xmlnode)->FirstChild(#argname) &&             \
           (xmlnode)->FirstChild(#argname)->ToElement()); \
  if ((xmlnode)->FirstChild(#argname) &&                  \
      (xmlnode)->FirstChild(#argname)->ToElement())       \
    (targetdata)->argname =                               \
        (int)atof(xmlnode->FirstChild(#argname)->ToElement()->GetText());
...
#define SET_FLOAT_VALUE(xmlnode, targetdata, argname)     \
  ...
  if ((xmlnode)->FirstChild(#argname) &&                  \
      (xmlnode)->FirstChild(#argname)->ToElement())       \
    (targetdata)->argname =                               \
        (float)atof(xmlnode->FirstChild(#argname)->ToElement()->GetText());
...
void btBulletXmlWorldImporter::deSerializeCollisionShapeData(
    TiXmlNode* pParent, btCollisionShapeData* colShapeData) {
...
void btBulletXmlWorldImporter::deSerializeCompoundShapeData(
    TiXmlNode* pParent) {
  MyLocalCaster cast;
  get_int_attribute_by_name(pParent->ToElement(), "pointer", &cast.m_int);
...
void btBulletXmlWorldImporter::deSerializeDynamicsWorldData(
    TiXmlNode* pParent) {
...
void btBulletXmlWorldImporter::deSerializeRigidBodyFloatData(
    TiXmlNode* pParent) {
  MyLocalCaster cast;
...
in broadphase proxy! TRIANGLE_MESH_SHAPE_PROXYTYPE,
