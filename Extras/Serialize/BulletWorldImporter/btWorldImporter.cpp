...
btCollisionShape* btWorldImporter::convertCollisionShape(
  ...
  switch (shapeData->m_shapeType) {
    ...
    case GIMPACT_SHAPE_PROXYTYPE: {
      ...
      // The btCapsuleShape* API has issue passing the
      // margin/scaling/halfextents unmodified through the API so deal with this
  ...
  switch (constraintData->m_objectType) {
    ...
    case GEAR_CONSTRAINT_TYPE: {
      ...
      if (rbA && rbB) {
        ...
        // serialized? btGearConstraint(btRigidBody& rbA, btRigidBody& rbB, const
        // btVector3& axisInA,const btVector3& axisInB, btScalar ratio=1.f);
      }
      constraint = gear;
      break;
    }
...
btStridingMeshInterfaceData* btWorldImporter::createStridingMeshInterfaceData(
  ...
  for (int i = 0; i < newData->m_numMeshParts; i++) {
    ...
    /// cause crashes at loading time we catch it by only dealing with
    /// m_3indices8 if none of the other indices are initialized
