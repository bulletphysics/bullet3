      ...
      if (pair.m_algorithm) {
        ...
        for (int m = 0; m < manifoldArray.size(); m++) {
          ...
          for (int q = 0; q < numContactManifolds; q++) {
            if (uid0 == getBody0FromContactManifold(contactManifolds[q])
                            ->m_uniqueId &&
                uid1 == getBody1FromContactManifold(contactManifolds[q])
                            ->m_uniqueId) {
  ...
  for (int i = 0; i < mbd->m_numLinks; i++) {
    ...
    switch (mbd->m_links[i].m_jointType) {
      ...
      case btMultibodyLink::eSpherical: {
        btScalar jointPos[3] = {(btScalar)mbd->m_links[i].m_jointPos[0],
    ...
    if (bulletFile2->getFlags() & bParse::FD_DOUBLE_PRECISION) {
      ...
      for (int i = bulletFile2->m_rigidBodies.size() - 1; i >= 0; i--) {
        ...
        for (int i = 0; i < m_data->m_mbDynamicsWorld->getNumCollisionObjects();
             i++) {
          if (uid == m_data->m_mbDynamicsWorld->getCollisionObjectArray()[i]
                         ->getBroadphaseHandle()
                         ->m_uniqueId) {
            foundRb = i;
            break;
