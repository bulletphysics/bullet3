...
/// rotational joints, converts in to an inverse dynamics model and compares
/// forward to inverse dynamics for random input
TEST(InvDynCompare, bulletUrdfR2D2) {
  ...
  ASSERT_TRUE(
      b3ResourcePath::findResourcePath(kUrdfFile, relativeFileName, 1024));
  ...
  for (int loop = 0; loop < kNLoops; loop++) {
    ...
    // body (fixed, so q, u, dot_u arbitrary)
