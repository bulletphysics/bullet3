...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < NUM_SIM; i++) {
    ...
    // example: sim->connect(eCONNECT_UDP, "localhost", 1234);
    sim->configureDebugVisualizer(COV_ENABLE_GUI, 0);
    //	sim->configureDebugVisualizer( COV_ENABLE_SHADOWS,
    //0);//COV_ENABLE_WIREFRAME sim->setTimeOut(3); syncBodies is only needed
    // when connecting to an existing physics server that has already some bodies
    sim->syncBodies();
