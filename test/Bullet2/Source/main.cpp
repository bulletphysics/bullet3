...
int main(int argc, const char *argv[]) {
  ...
  if (NULL == gNodeList) {  // test everything
    ...
    for (i = 0; NULL != gTestList[i].test_func; i++) {
      ...
      if (local_error) {
        ...
        printf("*** %s test failed with error: %d\n", gTestList[i].name,
               local_error);
        if (gExitOnError) return local_error;
        if (0 == err) err = local_error;
      } else {
        numPassedTests++;
    ...
    while (NULL != gNodeList) {
      ...
      size_t i;
      for (i = 0; NULL != gTestList[i].test_func; i++)
        if (0 == strcasecmp(currentNode->name, gTestList[i].name)) break;
...
static int ParseArgs(int argc, const char *argv[]) {
  ...
  if (NULL == gAppName) gAppName = "<unknown app name>";
  ...
  for (i = 1; i < argc; i++) {
      ...
      while (arg[0] != '\0') {
        ...
        switch (arg[0]) {
          ...
          case ' ':
            stop = 1;
            break;
          case 'N':  // ignore the -NSDocumentRevisionsDebugMode argument from
                     // XCode 4.3.2
            skipremaining = 1;
            stop = 1;
            break;
    ...
    } else {  // add function name to the list
      TestNode *node = (TestNode *)malloc(sizeof(TestNode));
      node->name = arg;
      node->next = list;
      list = node;
...
static void PrintUsage(void) {
  ...
  printf("Options:\n");
  printf(
      "\t-a\tToggle report average times vs. best times. (Default: best "
      "times)\n");
  printf("\t-e\tToggle exit immediately on error behavior. (Default: off)\n");
  printf("\t-h\tPrint this message.\n");
  printf("\t-l\tToggle list available test names.  (Default: off)\n");
  printf(
      "\t-s\tToggle report times in cycles or nanoseconds. (Default: "
      "cycles)\n\n");
  printf(
      "\tOptions may be followed by one or more test names. If no test names "
      "\n");
...
static void ListTests(void) {
  ...
  for (i = 0; NULL != gTestList[i].test_func; i++) {
    ...
    if (NULL != gTestList[i].test_func) printf(",");
    if (3 == (i & 3)) printf("\n");
  }
}
#else
...
int main(int argc, char* argv[]) {
  printf(
      "error: no SIMD enabled through BT_USE_NEON or BT_USE_SSE_IN_API "
