...
static inline btQuaternion qtmulQV3_ref(const btQuaternion& q,
                                        const btVector3& w);
...

  {
    ...
    if (fabsf(correct_res.x() - test_res.x()) +
      ...
      vlog(
          ...
          correct_res.x(), correct_res.y(), correct_res.z(), correct_res.w(),
          test_res.x(), test_res.y(), test_res.z(), test_res.w());
