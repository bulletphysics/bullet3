...
static btMatrix3x3 M3x3mulM_ref(btMatrix3x3 &in, const btMatrix3x3 &m) {
  ...
  in.setValue(m.tdotx(m_el[0]), m.tdoty(m_el[0]), m.tdotz(m_el[0]),
...
static SIMD_FORCE_INLINE bool fuzzyEqualSlow(const btVector3 &ref,
...
static int operator!=(const btMatrix3x3 &a, const btMatrix3x3 &b) {
  ...
  if (a.getRow(1) != b.getRow(1)) {
    if (!fuzzyEqualSlow(a.getRow(1), b.getRow(1))) return 1;
  }
...
int Test_3x3mulM(void) {
  ...
  for (j = 0; j < LOOPCOUNT; j++) {
    startTime = ReadTicks();
    for (i = 0; i < ARRAY_SIZE; i++) out[i] = M3x3mulM_ref(in1[i], in2[i]);
  ...
  for (j = 0; j < LOOPCOUNT; j++) {
    startTime = ReadTicks();
    for (i = 0; i < ARRAY_SIZE; i++) out2[i] = (in3[i] *= in2[i]);
    currentTime = ReadTicks() - startTime;
