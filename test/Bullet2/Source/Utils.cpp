...
#ifdef _WIN32
#include <intrin.h>
uint64_t ReadTicks(void) { return __rdtsc(); }
double TicksToCycles(uint64_t delta) { return double(delta); }

double TicksToSeconds(uint64_t delta) { return double(delta); }

void *GuardCalloc(size_t count, size_t size, size_t *objectStride) {
  if (objectStride) *objectStride = size;
  return (void *)btAlignedAlloc(count * size, 16);
}
void GuardFree(void *buf) { btAlignedFree(buf); }

#endif

#ifdef __APPLE__

uint64_t ReadTicks(void) { return mach_absolute_time(); }
...
double TicksToCycles(uint64_t delta) {
  ...
  if (0.0L == conversion) {
    ...
    if (0 == gReportNanoseconds) {
      ...
      err =
          sysctlbyname("hw.cpufrequency_max", &frequency, &freq_size, NULL, 0);
      if (err || 0 == frequency)
        vlog(
            "Failed to get max cpu frequency. Reporting times as "
            "nanoseconds.\n");
      else {
...
void *GuardCalloc(size_t count, size_t size, size_t *objectStride) {
  if (objectStride) *objectStride = 0;
  ...
  // Allocate
  char *buf = (char *)mmap(NULL, totalSize, PROT_READ | PROT_WRITE,
  ...
  if (objectStride) *objectStride = stride + kPageSize;
...
int GuardMarkReadOnly(void *buf) { return GuardMarkBuffer(buf, PROT_READ); }

int GuardMarkReadWrite(void *buf) {
  return GuardMarkBuffer(buf, PROT_READ | PROT_WRITE);
}

int GuardMarkWriteOnly(void *buf) { return GuardMarkBuffer(buf, PROT_WRITE); }
...
static int GuardMarkBuffer(void *buf, int flag) {
  ...
  for (i = 0; i < count; i++) {
    if (mprotect(buf, stride, flag)) {
      vlog(
          "Unable to protect segment %ld. GuardMarkBuffer %d failed! %p  "
          "(%d)\n",
          i, flag, buf, errno);
      return errno;
    }
  ...
  if (mprotect((char *)buf - kPageSize, kPageSize, PROT_NONE)) {
    vlog(
        "Unable to protect leading guard page. GuardMarkBuffer %d failed! %p  "
        "(%d)\n",
        flag, buf, errno);
    return errno;
  }
...
uint32_t random_number32(void) { return ((uint32_t)rand() << 16) ^ rand(); }
