...
// speeds set CCD speed threshold and testing sphere radius per rigidbody
...
// accuracy static btScalar gPhysicsStepsPerSecond = 240.0f; // For high accuracy
...
struct NN3DWalkersTimeWarpBase : public CommonRigidBodyBase {
  NN3DWalkersTimeWarpBase(struct GUIHelperInterface* helper)
    ...
    mApplicationStart =
        mLoopTimer.getTimeMilliseconds(); /**!< Initialize when the application
                                             started running */
  ...
  void setupAdvancedParamInterface() {
    ...
    {  // create a slider to adjust the number of internal application ticks
      ...
      // cycle of model update (physics and input) and view update (graphics)
      // with average application load. The graphics and input update determine
      // the remaining time for the physics update
    ...
    {  // create a slider to adjust the number of physics steps per second
      ...
      // general simulations For simulations with higher complexity or if you
      // experience undesired behavior, try increasing the number of steps per
      // second Alternatively, try increasing the number of solver iterations if
      // you experience jittering constraints due to non-converging solutions
    ...
    {  // create a slider to adjust the number of solver iterations to converge
      ...
      // converge, it also depends on the type of solver.
  ...
  void createEmptyDynamicsWorld() {  // create an empty dynamics worlds
    ...
    switch (SOLVER_TYPE) {
      ...
      case FSSOLVER: {
        ...
        // Use the btMultiBodyConstraintSolver for Featherstone btMultiBody
        // support
    ...
    if (useSplitImpulse) {  // If you experience strong repulsion forces in your
      ...
      //= 			-0.02; 		m_dynamicsWorld->getSolverInfo().m_erp2 = BulletUtils::getERP(
      ...
      //ERP/TurnERP: (%f,%f)", 				m_dynamicsWorld->getSolverInfo().m_erp2,
      //				m_dynamicsWorld->getSolverInfo().m_splitImpulseTurnErp);
    }
  ...
  void stepSimulation(float deltaTime) {  // customly step the simulation
    do {
      ...
      if (mInputDt >= gApplicationTick) {
        ...
        //handlers */ 	         mInputHandler.update(mInputClock); /**!< update elements
        //that work on the current input state */
      }
      ...
      // model describes In the example browser, there is a separate method
      // called renderScene() for this
      ...
      //milliseconds", 		mLastModelTick, mLastGraphicsTick, mLastInputTick,
      //mApplicationRuntime);
  ...
  void performInterpolatedSteps(
    ...
    int subSteps =
        1 + floor((timeStep / fixedPhysicsStepSizeSec) +
                  0.5); /**!< Calculate the number of full normal time steps we
                           can take, plus 1 for safety of not losing time */
    if (timeStep && m_dynamicsWorld) {
      m_dynamicsWorld->stepSimulation(
          btScalar(timeStep), btScalar(subSteps),
          btScalar(
              fixedPhysicsStepSizeSec)); /**!< Perform the number of substeps to
                                            reach the timestep*/
    }
  }
  ...
  void performMaxStep() {  // perform as many steps as possible
    if (gApplicationTick >=
        mLastGraphicsTick +
            mLastInputTick) {  // if the remaining time for graphics is going to
                               // be positive
    ...
    if (mPhysicsTick > 0) {  // with positive physics tick we perform as many
      ...
      mPhysicsStepStart =
          mLoopTimer.getTimeMilliseconds(); /**!< The physics updates start (in
                                               Milliseconds)*/
      mPhysicsStepEnd = mPhysicsStepStart;

      while (mPhysicsTick >
             mPhysicsStepEnd -
                 mPhysicsStepStart) { /**!< Update the physics until we run out
        ...
        mPhysicsStepEnd =
            mLoopTimer.getTimeMilliseconds(); /**!< Update the last physics step
                                                 end to stop updating in time
                                                 (in Milliseconds) */
      }
    }
  }
  ...
  void
  performSpeedStep() {  // force-perform the number of steps needed to achieve a
                        // certain speed (safe to too high speeds, meaning the
                        // application will lose time, not the physics)
    if (mFrameTime >
        gApplicationTick) { /** cap frametime to make the application lose time,
                               not the physics (in Milliseconds) */
    ...
    int steps = floor(mModelAccumulator /
                      fixedPhysicsStepSizeMilli); /**!< Calculate the number of
                                                     time steps we can take */
  ...
  unsigned long int mModelStart; /**!< The timestamp the model started updating
                                    last (absolute, in Milliseconds)*/
  ...
  unsigned long int mPhysicsStepStart; /**!< The physics start timestamp
                                          (absolute, in Milliseconds) */
  unsigned long int mPhysicsStepEnd; /**!< The last physics step end (absolute,
                                        in Milliseconds) */
