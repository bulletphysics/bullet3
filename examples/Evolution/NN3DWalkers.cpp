  ...
  NNWalker(int index, btDynamicsWorld* ownerWorld,
    ...
    for (i = 0; i < NUM_LEGS; i++) {
      ...
      float footYUnitPosition =
          sin(footAngle);  // y position of the leg on the unit circle
      float footXUnitPosition =
          cos(footAngle);  // x position of the leg on the unit circle
  ...
  void addToWorld() {
    ...
    for (i = 0; i < JOINT_COUNT; ++i) {
      m_ownerWorld->addConstraint(
          m_joints[i],
          true);  // important! If you add constraints back, you must set bullet
                  // physics to disable collision between constrained bodies
    }
...
void NN3DWalkersExample::initPhysics() {
  ...
  // the motor should be (numberOfsolverIterations * oldLimits)
...
void NN3DWalkersExample::update(const btScalar timeSinceLastTick) {
  updateEvaluations(timeSinceLastTick); /**!< We update all evaluations that are
                                           in the loop */
