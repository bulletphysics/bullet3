...
struct Shader : public IShader {
  ...
  virtual bool fragment(Vec3f bar, TGAColor& color) {
    ...
    float specular =
        pow(b3Max(reflection_direction.z, 0.f), m_model->specular(uv));
    ...
    for (int i = 0; i < 3; ++i) {
      color[i] = b3Min(int(m_ambient_coefficient * color[i] +
                           shadow *
...
void TinyRenderObjectData::registerMesh2(
  ...
  if (0 == m_model) {
    ...
    m_model = new Model();
    char relativeFileName[1024];
    if (b3ResourcePath::findResourcePath("floor_diffuse.tga", relativeFileName,
                                         1024)) {
      m_model->loadDiffuseTexture(relativeFileName);
    }
