...
void btFractureDynamicsWorld::glueCallback() {
  ...
  for (startIslandIndex = 0; startIslandIndex < numElem;
    ...
    if (fractureObjectIndex >= 0 && numObjects > 1) {
      ...
      /// objects delay the removal of old objects to avoid array indexing
      /// problems
...
void btFractureDynamicsWorld::fractureCallback() {
  ...
  for (int i = 0; i < numManifolds; i++) {
    ...
    // child shapes as rigid bodies, without constraints, running it in a
    // separate simulation world (or by running the constraint solver without
    //actually modifying the dynamics world) Then measure some "impact value"
    // using the offset and applied impulse for each child shape weaken the
    // connections based on this "impact value" and only break if this impact
    // value exceeds the breaking threshold. you can propagate the weakening and
    // breaking of connections using the connectivity information
    ...
    if (f0 < m_fractureBodies.size()) {
      ...
      //(btCollisionObject*)manifold->getBody1(); 		btRigidBody* otherOb =
      //btRigidBody::upcast(colOb); 	if (!otherOb->getInvMass()) 		continue;
