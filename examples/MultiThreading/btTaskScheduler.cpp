...
b3ThreadSupportInterface* createThreadSupport(
    int numThreads, btThreadFunc threadFunc,
    btThreadLocalStorageFunc localStoreFunc, const char* uniqueName) {
  b3Win32ThreadSupport::Win32ThreadConstructionInfo constructionInfo(
      uniqueName, threadFunc, localStoreFunc, numThreads);
  // constructionInfo.m_priority = 0;  // highest priority (the default) -- can
  // cause erratic performance when numThreads > numCores
  //                                     we don't want worker threads to be
  //                                     higher priority than the main thread or
  //                                     the main thread could get totally shut
  //                                     out and unable to tell the workers to
  //                                     stop
  constructionInfo.m_priority = -1;  // normal priority
  b3Win32ThreadSupport* threadSupport =
      new b3Win32ThreadSupport(constructionInfo);
...
int getNumHardwareThreads() { return std::thread::hardware_concurrency(); }
...
static void* WorkerThreadAllocFunc() { return new WorkerThreadLocalStorage; }
...
class btTaskSchedulerDefault : public btITaskScheduler {
 ...
 public:
  ...
  virtual ~btTaskSchedulerDefault() { shutdown(); }

  void init() {
    int maxNumWorkerThreads = BT_MAX_THREAD_COUNT - 1;
    m_threadSupport =
        createThreadSupport(maxNumWorkerThreads, WorkerThreadFunc,
                            WorkerThreadAllocFunc, "TaskScheduler");
    m_jobContext.m_queueLock = m_threadSupport->createCriticalSection();
    for (int i = 0; i < maxNumWorkerThreads; i++) {
      WorkerThreadLocalStorage* storage =
          (WorkerThreadLocalStorage*)m_threadSupport->getThreadLocalMemory(i);
      btAssert(storage);
      storage->threadId = i;
  ...
  void waitForWorkersToSleep() {
    ...
    while (m_numWorkersRunning > 0) {
      int iThread;
      int threadStatus;
      m_threadSupport->waitForResponse(
          &iThread,
          &threadStatus);  // wait for worker threads to finish working
      m_numWorkersRunning--;
    }
...
btITaskScheduler* createDefaultTaskScheduler() { return NULL; }
