    ...
    case BT_ID_LOAD_URDF: {
      ...
      if (loadOk) {
        ...
        if (m_multiBody) {
          ...
          // soon due to timestep/integrator temporarily set some extreme damping
          // factors until we have some joint control or constraints
  ...
  if (m_multiBody) {
    ...
    if (useInverseModel) {
      ...
      if (m_multiBody->hasFixedBase()) {
        if (-1 != m_inverseModel->calculateInverseDynamics(q, qdot, nu,
                                                           &joint_force)) {
        ...
        // btMultiBody. append some dummy values to represent the 6 DOFs of the
        // base
        ...
        if (-1 != m_inverseModel->calculateInverseDynamics(q6, qdot6, nu6,
                                                           &joint_force6)) {
          // joint_force(dof) += damping*dot_q(dof);
          // use inverse model: apply joint force corresponding to
          // desired acceleration nu
