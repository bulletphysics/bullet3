    ...
    if (rayCallback.hasHit()) {
      ...
      if (body) {
        ...
        if (multiCol && multiCol->m_multiBody) {
          ...
          // the mouse pick can apply it is not satisfying, hopefully we find a
          // better solution (higher order integrator, using joint friction using
          // a zero-velocity target motor with limited force etc?)
          btScalar scaling = 1;
