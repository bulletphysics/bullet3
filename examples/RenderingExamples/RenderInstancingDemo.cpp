...

    {
      ...
      for (int i = -numCubesX / 2; i < numCubesX / 2; i++) {
        for (int j = -numCubesY / 2; j < numCubesY / 2; j++) {
          ...
          int instanceId = m_app->m_renderer->registerGraphicsInstance(
              boxId, pos, orn, color, scaling);
    ...
    for (int i = -numCubesX / 2; i < numCubesX / 2; i++) {
      for (int j = -numCubesY / 2; j < numCubesY / 2; j++) {
        ...
        m_app->m_renderer->writeSingleInstanceTransformToCPU(
            pos, orn, m_movingInstances[index++]);
      }
    }
    m_app->m_renderer->writeTransforms();
  }
...
class CommonExampleInterface* RenderInstancingCreateFunc(
