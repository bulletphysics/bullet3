...
class DynamicTexturedCubeDemo : public CommonExampleInterface {
  ...
  enum { numCubesX = 1, numCubesY = 1 };

 public:
  DynamicTexturedCubeDemo(CommonGraphicsApp* app) : m_app(app), m_tinyVrGUI(0) {
    m_app->setUpAxis(2);
  ...
  virtual ~DynamicTexturedCubeDemo() { delete m_tinyVrGUI; }
  ...
  virtual void stepSimulation(float deltaTime) {
    ...
    t += deltaTime;
    tr.setOrigin(b3MakeVector3(0., 0., 2.) +
                 b3MakeVector3(0., 0., 0.02 * b3Sin(t)));

    m_tinyVrGUI->tick(deltaTime, tr);

    m_app->m_renderer->writeTransforms();
  }
  virtual void renderScene() { m_app->m_renderer->renderScene(); }
...
class CommonExampleInterface* DynamicTexturedCubeDemoCreateFunc(
