...
b3PhysicsClientHandle getPhysicsClient(int physicsClientId) {
  ...
  if ((physicsClientId < 0) || (physicsClientId >= MAX_PHYSICS_CLIENTS) ||
      (0 == sPhysicsClients1[physicsClientId])) {
    return 0;
  }
...
static int pybullet_internalGetVector3FromSequence(PyObject* seq, int index,
                                                   double vec[3]) {
  int v = 0;
  PyObject* item;
...
static int pybullet_internalGetVector4FromSequence(PyObject* seq, int index,
                                                   double vec[4]) {
  int v = 0;
  PyObject* item;
...
static PyObject* pybullet_connectPhysicsServer(PyObject* self, PyObject* args,
...

  {
    ...
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "i|is", kwlist1, &method,
                                     &key, &options)) {
      int port = -1;
      if (!PyArg_ParseTupleAndKeywords(args, keywds, "i|sis", kwlist2, &method,
    ...
    if (method == eCONNECT_GUI) {
      ...
      for (i = 0; i < MAX_PHYSICS_CLIENTS; i++) {
        if ((sPhysicsClientsGUI[i] == eCONNECT_GUI) ||
            (sPhysicsClientsGUI[i] == eCONNECT_GUI_SERVER)) {
          PyErr_SetString(
              SpamError,
              "Only one local in-process GUI/GUI_SERVER connection allowed. "
              "Use DIRECT connection mode or start a separate GUI physics "
              "server (ExampleBrowser, App_SharedMemoryPhysics_GUI, "
              "App_SharedMemoryPhysics_VR) and connect over SHARED_MEMORY, UDP "
              "or TCP instead.");
          return NULL;
        }
      }
...
#ifdef __APPLE__
        sm = b3CreateInProcessPhysicsServerAndConnectMainThreadSharedMemory(
            argc, argv);
#else
...
static PyObject* pybullet_isConnected(PyObject* self, PyObject* args,
                                      PyObject* keywds) {
  int physicsClientId = 0;
  int isConnected = 0;
  int method = 0;
...
static PyObject* pybullet_getConnectionInfo(PyObject* self, PyObject* args,
                                            PyObject* keywds) {
  int physicsClientId = 0;
  int isConnected = 0;
  int method = 0;
...
static PyObject* pybullet_saveWorld(PyObject* self, PyObject* args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "s|i", kwlist, &worldFileName,
                                   &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
...
static PyObject* pybullet_loadBullet(PyObject* self, PyObject* args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "s|i", kwlist, &bulletFileName,
                                   &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
...
static PyObject* pybullet_restoreState(PyObject* self, PyObject* args,
                                       PyObject* keywds) {
  const char* fileName = "";
  b3SharedMemoryStatusHandle statusHandle;
  int statusType;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "|isi", kwlist, &stateId,
                                   &fileName, &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
  if (sm == 0) {
...
static PyObject* pybullet_saveState(PyObject* self, PyObject* args,
                                    PyObject* keywds) {
  b3SharedMemoryStatusHandle statusHandle;
  int statusType;
  b3SharedMemoryCommandHandle command;
...
static PyObject* pybullet_loadMJCF(PyObject* self, PyObject* args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "s|ii", kwlist, &mjcfFileName,
  ...
  if (numBodies > MAX_SDF_BODIES) {
    char str[1024];
    sprintf(str, "SDF exceeds body capacity: %d > %d", numBodies,
            MAX_SDF_BODIES);
...
static PyObject* pybullet_changeDynamicsInfo(PyObject* self, PyObject* args,
                                             PyObject* keywds) {
  int bodyUniqueId = -1;
  int linkIndex = -2;
  double mass = -1;
  ...
  static char* kwlist[] = {
      ...
      "localInertiaDiagonal", "physicsClientId",  NULL};
  if (!PyArg_ParseTupleAndKeywords(
          args, keywds, "ii|dddddddddiOi", kwlist, &bodyUniqueId, &linkIndex,
          &mass, &lateralFriction, &spinningFriction, &rollingFriction,
          &restitution, &linearDamping, &angularDamping, &contactStiffness,
          &contactDamping, &frictionAnchor, &localInertiaDiagonalObj,
          &physicsClientId)) {
    return NULL;
  }
...

  {
    ...
    if (localInertiaDiagonalObj) {
      ...
      pybullet_internalSetVectord(localInertiaDiagonalObj,
                                  localInertiaDiagonal);
      b3ChangeDynamicsInfoSetLocalInertiaDiagonal(
          command, bodyUniqueId, linkIndex, localInertiaDiagonal);
    }
    ...
    if (contactStiffness >= 0 && contactDamping >= 0) {
      b3ChangeDynamicsInfoSetContactStiffnessAndDamping(
          command, bodyUniqueId, linkIndex, contactStiffness, contactDamping);
    }
    if (frictionAnchor >= 0) {
...
static PyObject* pybullet_getDynamicsInfo(PyObject* self, PyObject* args,
                                          PyObject* keywds) {
  {
    int bodyUniqueId = -1;
    int linkIndex = -2;
    ...
    int physicsClientId = 0;
    static char* kwlist[] = {"bodyUniqueId", "linkIndex", "physicsClientId",
                             NULL};
    ...
    }
    {
      ...
      if (bodyUniqueId < 0) {
        PyErr_SetString(SpamError,
                        "getDynamicsInfo failed; invalid bodyUniqueId");
        return NULL;
      }
      if (linkIndex < -1) {
      ...
      if (status_type != CMD_GET_DYNAMICS_INFO_COMPLETED) {
        PyErr_SetString(SpamError,
                        "getDynamicsInfo failed; invalid return status");
        return NULL;
      }
      ...
      if (b3GetDynamicsInfo(status_handle, &info)) {
        ...
        PyTuple_SetItem(pyDynamicsInfo, 1,
                        PyFloat_FromDouble(info.m_lateralFrictionCoeff));

        {
          PyObject* pyInertiaDiag = PyTuple_New(3);
          PyTuple_SetItem(pyInertiaDiag, 0,
                          PyFloat_FromDouble(info.m_localInertialDiagonal[0]));
          PyTuple_SetItem(pyInertiaDiag, 1,
                          PyFloat_FromDouble(info.m_localInertialDiagonal[1]));
          PyTuple_SetItem(pyInertiaDiag, 2,
                          PyFloat_FromDouble(info.m_localInertialDiagonal[2]));
          PyTuple_SetItem(pyDynamicsInfo, 2, pyInertiaDiag);
        }
        {
          PyObject* pyInertiaPos = PyTuple_New(3);
          PyTuple_SetItem(pyInertiaPos, 0,
                          PyFloat_FromDouble(info.m_localInertialFrame[0]));
          PyTuple_SetItem(pyInertiaPos, 1,
                          PyFloat_FromDouble(info.m_localInertialFrame[1]));
          PyTuple_SetItem(pyInertiaPos, 2,
                          PyFloat_FromDouble(info.m_localInertialFrame[2]));
          PyTuple_SetItem(pyDynamicsInfo, 3, pyInertiaPos);
        }
        {
          PyObject* pyInertiaOrn = PyTuple_New(4);
          PyTuple_SetItem(pyInertiaOrn, 0,
                          PyFloat_FromDouble(info.m_localInertialFrame[3]));
          PyTuple_SetItem(pyInertiaOrn, 1,
                          PyFloat_FromDouble(info.m_localInertialFrame[4]));
          PyTuple_SetItem(pyInertiaOrn, 2,
                          PyFloat_FromDouble(info.m_localInertialFrame[5]));
          PyTuple_SetItem(pyInertiaOrn, 3,
                          PyFloat_FromDouble(info.m_localInertialFrame[6]));
          PyTuple_SetItem(pyDynamicsInfo, 4, pyInertiaOrn);
        }
        PyTuple_SetItem(pyDynamicsInfo, 5,
                        PyFloat_FromDouble(info.m_restitution));
        PyTuple_SetItem(pyDynamicsInfo, 6,
                        PyFloat_FromDouble(info.m_rollingFrictionCoeff));
        PyTuple_SetItem(pyDynamicsInfo, 7,
                        PyFloat_FromDouble(info.m_spinningFrictionCoeff));
        PyTuple_SetItem(pyDynamicsInfo, 8,
                        PyFloat_FromDouble(info.m_contactDamping));
        PyTuple_SetItem(pyDynamicsInfo, 9,
                        PyFloat_FromDouble(info.m_contactStiffness));
...
static PyObject* pybullet_getPhysicsEngineParameters(PyObject* self,
                                                     PyObject* args,
                                                     PyObject* keywds) {
  b3PhysicsClientHandle sm = 0;
  PyObject* val = 0;
...

  {
    ...
    val = Py_BuildValue("{s:d,s:i,s:i,s:i,s:d,s:d,s:d}", "fixedTimeStep",
                        params.m_deltaTime, "numSubSteps",
                        params.m_numSimulationSubSteps, "numSolverIterations",
                        params.m_numSolverIterations, "useRealTimeSimulation",
                        params.m_useRealTimeSimulation, "gravityAccelerationX",
                        params.m_gravityAcceleration[0], "gravityAccelerationY",
                        params.m_gravityAcceleration[1], "gravityAccelerationZ",
                        params.m_gravityAcceleration[2]);
    return val;
  }
  //"fixedTimeStep", "numSolverIterations", "useSplitImpulse",
  //"splitImpulsePenetrationThreshold", "numSubSteps", "collisionFilterMode",
  //"contactBreakingThreshold", "maxNumCmdPer1ms",
  //"enableFileCaching","restitutionVelocityThreshold", "erp", "contactERP",
  //"frictionERP", val = Py_BuildValue("{s:i,s:i}","isConnected", isConnected,
  // "connectionMethod", method);
}

static PyObject* pybullet_setPhysicsEngineParameter(PyObject* self,
                                                    PyObject* args,
                                                    PyObject* keywds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, keywds, "|diidiidiiddddiii", kwlist, &fixedTimeStep,
          &numSolverIterations, &useSplitImpulse,
          &splitImpulsePenetrationThreshold, &numSubSteps, &collisionFilterMode,
          &contactBreakingThreshold, &maxNumCmdPer1ms, &enableFileCaching,
          &restitutionVelocityThreshold, &erp, &contactERP, &frictionERP,
          &enableConeFriction, &deterministicOverlappingPairs,
          &physicsClientId)) {
    return NULL;
  }
...

  {
    ...
    if (splitImpulsePenetrationThreshold >= 0) {
      b3PhysicsParamSetSplitImpulsePenetrationThreshold(
          command, splitImpulsePenetrationThreshold);
    }
    ...
    if (deterministicOverlappingPairs >= 0) {
      b3PhysicsParameterSetDeterministicOverlappingPairs(
          command, deterministicOverlappingPairs);
    }
...
static PyObject* pybullet_loadURDF(PyObject* self, PyObject* args,
  ...
  static char* kwlistBackwardCompatible4[] = {"fileName", "startPosX",
                                              "startPosY", "startPosZ", NULL};
  static char* kwlistBackwardCompatible8[] = {
      "fileName",  "startPosX", "startPosY", "startPosZ", "startOrnX",
      "startOrnY", "startOrnZ", "startOrnW", NULL};
  ...
  if (PyArg_ParseTupleAndKeywords(args, keywds, "sddd",
                                  kwlistBackwardCompatible4, &urdfFileName,
  ...
  if (PyArg_ParseTupleAndKeywords(
          args, keywds, "sddddddd", kwlistBackwardCompatible8, &urdfFileName,
          &startPosX, &startPosY, &startPosZ, &startOrnX, &startOrnY,
          &startOrnZ, &startOrnW)) {
    backwardsCompatibilityArgs = 1;
  } else {
    PyErr_Clear();
  ...
  if (!backwardsCompatibilityArgs) {
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, keywds, "s|OOiiidi", kwlist, &urdfFileName, &basePosObj,
            &baseOrnObj, &useMaximalCoordinates, &useFixedBase, &flags,
            &globalScaling, &physicsClientId)) {
      return NULL;
    } else {
      if (basePosObj) {
...
static PyObject* pybullet_loadSDF(PyObject* self, PyObject* args,
  ...
  int physicsClientId = 0;
  static char* kwlist[] = {"sdfFileName", "useMaximalCoordinates",
                           "globalScaling", "physicsClientId", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "s|idi", kwlist, &sdfFileName,
                                   &useMaximalCoordinates, &globalScaling,
                                   &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
...
static PyObject* pybullet_loadSoftBody(PyObject* self, PyObject* args,
                                       PyObject* keywds) {
  int physicsClientId = 0;
  int flags = 0;

  static char* kwlist[] = {"fileName",        "scale",           "mass",
                           "collisionMargin", "physicsClientId", NULL};
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "s|dddi", kwlist, &fileName,
                                   &scale, &mass, &collisionMargin,
                                   &physicsClientId)) {
    return NULL;
  }
...
static PyObject* pybullet_setJointMotorControl(PyObject* self, PyObject* args) {
  ...
  if (size == 4) {
    ...
    if (!PyArg_ParseTuple(args, "iiid", &bodyUniqueId, &jointIndex,
                          &controlMode, &targetValue)) {
  ...
  if (size == 5) {
    ...
    if (!PyArg_ParseTuple(args, "iiidd", &bodyUniqueId, &jointIndex,
                          &controlMode, &targetValue, &maxForce)) {
      PyErr_SetString(SpamError, "Error parsing arguments");
      return NULL;
    }
    valid = 1;
  ...
  if (size == 6) {
    double gain = 0.0;
    double targetValue = 0.0;
    if (!PyArg_ParseTuple(args, "iiiddd", &bodyUniqueId, &jointIndex,
                          &controlMode, &targetValue, &maxForce, &gain)) {
      PyErr_SetString(SpamError, "Error parsing arguments");
      return NULL;
    }
    valid = 1;
...
static PyObject* pybullet_setJointMotorControlArray(PyObject* self,
                                                    PyObject* args,
                                                    PyObject* keywds) {
  ...
  int physicsClientId = 0;
  static char* kwlist[] = {"bodyUniqueId",     "jointIndices",
                           "controlMode",      "targetPositions",
                           "targetVelocities", "forces",
                           "positionGains",    "velocityGains",
                           "physicsClientId",  NULL};
  if (!PyArg_ParseTupleAndKeywords(
          args, keywds, "iOi|OOOOOi", kwlist, &bodyUniqueId, &jointIndicesObj,
          &controlMode, &targetPositionsObj, &targetVelocitiesObj, &forcesObj,
          &kpsObj, &kdsObj, &physicsClientId)) {
    static char* kwlist2[] = {"bodyIndex",        "jointIndices",
                              "controlMode",      "targetPositions",
                              "targetVelocities", "forces",
                              "positionGains",    "velocityGains",
                              "physicsClientId",  NULL};
    PyErr_Clear();
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "iOi|OOOOOi", kwlist2,
                                     &bodyUniqueId, &jointIndicesObj,
                                     &controlMode, &targetPositionsObj,
                                     &targetVelocitiesObj, &forcesObj, &kpsObj,
                                     &kdsObj, &physicsClientId)) {
      return NULL;
    }
  }
  sm = getPhysicsClient(physicsClientId);
...

  {
    ...
    jointIndicesSeq = PySequence_Fast(jointIndicesObj,
                                      "expected a sequence of joint indices");
...

    {
      ...
      for (i = 0; i < numControlledDofs; i++) {
        int jointIndex =
            pybullet_internalGetIntFromSequence(jointIndicesSeq, i);
        if ((jointIndex >= numJoints) || (jointIndex < 0)) {
          Py_DECREF(jointIndicesSeq);
          PyErr_SetString(SpamError, "Joint index out-of-range.");
          return NULL;
    ...
    if (targetVelocitiesObj) {
      ...
      if (num != numControlledDofs) {
        Py_DECREF(jointIndicesSeq);
        PyErr_SetString(SpamError,
                        "number of target velocies should match the number of "
                        "joint indices");
        return NULL;
      }
      targetVelocitiesSeq = PySequence_Fast(
          targetVelocitiesObj, "expected a sequence of target velocities");
    }
    ...
    if (targetPositionsObj) {
      ...
      if (num != numControlledDofs) {
        ...
        PyErr_SetString(SpamError,
                        "number of target positions should match the number of "
                        "joint indices");
        return NULL;
      }

      targetPositionsSeq = PySequence_Fast(
          targetPositionsObj, "expected a sequence of target positions");
    }
    ...
    if (forcesObj) {
      ...
      if (num != numControlledDofs) {
        ...
        PyErr_SetString(SpamError,
                        "number of forces should match the joint indices");
        return NULL;
      }
    ...
    if (kpsObj) {
      ...
      if (num != numControlledDofs) {
        ...
        PyErr_SetString(SpamError,
                        "number of kps should match the joint indices");
        return NULL;
      }
    ...
    if (kdsObj) {
      ...
      if (num != numControlledDofs) {
        ...
        PyErr_SetString(
            SpamError,
            "number of kds should match the number of joint indices");
        return NULL;
      }
    ...
    for (i = 0; i < numControlledDofs; i++) {
      ...
      if (targetVelocitiesSeq) {
        targetVelocity =
            pybullet_internalGetFloatFromSequence(targetVelocitiesSeq, i);
      }

      if (targetPositionsSeq) {
        targetPosition =
            pybullet_internalGetFloatFromSequence(targetPositionsSeq, i);
      }
...
static PyObject* pybullet_setJointMotorControl2(PyObject* self, PyObject* args,
                                                PyObject* keywds) {
  int bodyUniqueId, jointIndex, controlMode;
  ...
  static char* kwlist[] = {"bodyUniqueId",
                           ...
                           "physicsClientId",
                           NULL};
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "iii|ddddddi", kwlist,
                                   &bodyUniqueId, &jointIndex, &controlMode,
                                   &targetPosition, &targetVelocity, &force,
                                   &kp, &kd, &maxVelocity, &physicsClientId)) {
    // backward compatibility, bodyIndex -> bodyUniqueId, don't need to update
    // this function: people have to migrate to bodyUniqueId
    static char* kwlist2[] = {"bodyIndex",
                              "jointIndex",
                              "controlMode",
                              "targetPosition",
                              "targetVelocity",
    ...
    PyErr_Clear();
    if (!PyArg_ParseTupleAndKeywords(
            args, keywds, "iii|ddddddi", kwlist2, &bodyUniqueId, &jointIndex,
            &controlMode, &targetPosition, &targetVelocity, &force, &kp, &kd,
            &maxVelocity, &physicsClientId)) {
      return NULL;
    }
  }
  sm = getPhysicsClient(physicsClientId);
...

  {
    ...
    switch (controlMode) {
      ...
      case CONTROL_MODE_POSITION_VELOCITY_PD: {
        if (maxVelocity > 0) {
          b3JointControlSetMaximumVelocity(commandHandle, info.m_uIndex,
                                           maxVelocity);
        }
        b3JointControlSetDesiredPosition(commandHandle, info.m_qIndex,
                                         targetPosition);
...
static PyObject* pybullet_setRealTimeSimulation(PyObject* self, PyObject* args,
                                                PyObject* keywds) {
...
static PyObject* pybullet_setInternalSimFlags(PyObject* self, PyObject* args,
                                              PyObject* keywds) {
  int flags = 0;
  int ret;
  int physicsClientId = 0;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "i|i", kwlist, &flags,
                                   &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
...
static PyObject* pybullet_setGravity(PyObject* self, PyObject* args,
                                     PyObject* keywds) {
  {
    ...
    int physicsClientId = 0;
    static char* kwlist[] = {"gravX", "gravY", "gravZ", "physicsClientId",
                             NULL};
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "ddd|i", kwlist, &gravX,
                                     &gravY, &gravZ, &physicsClientId)) {
      return NULL;
    }
    sm = getPhysicsClient(physicsClientId);
    if (sm == 0) {
...
static PyObject* pybullet_setTimeStep(PyObject* self, PyObject* args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "d|i", kwlist, &timeStep,
                                   &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
...
static PyObject* pybullet_setDefaultContactERP(PyObject* self, PyObject* args,
                                               PyObject* keywds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "d|i", kwlist,
                                   &defaultContactERP, &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
  if (sm == 0) {
...
static int pybullet_internalGetBaseVelocity(int bodyUniqueId,
...
static int pybullet_internalGetBasePositionAndOrientation(
    int bodyUniqueId, double basePosition[3], double baseOrientation[4],
    b3PhysicsClientHandle sm) {
  basePosition[0] = 0.;
  basePosition[1] = 0.;
  basePosition[2] = 0.;
...
static PyObject* pybullet_getAABB(PyObject* self, PyObject* args,
                                  PyObject* keywds) {
  int bodyUniqueId = -1;
  int linkIndex = -1;
  ...
  static char* kwlist[] = {"bodyUniqueId", "linkIndex", "physicsClientId",
                           NULL};
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "i|ii", kwlist, &bodyUniqueId,
                                   &linkIndex, &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
  if (sm == 0) {
...

  {
    ...
    cmd_handle = b3RequestCollisionInfoCommandInit(sm, bodyUniqueId);
    status_handle = b3SubmitClientCommandAndWaitStatus(sm, cmd_handle);
...
static PyObject* pybullet_getBasePositionAndOrientation(PyObject* self,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "i|i", kwlist, &bodyUniqueId,
                                   &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
...
static PyObject* pybullet_getBaseVelocity(PyObject* self, PyObject* args,
                                          PyObject* keywds) {
  int bodyUniqueId = -1;
  double baseLinearVelocity[3];
  double baseAngularVelocity[3];
  ...
  if (0 == pybullet_internalGetBaseVelocity(bodyUniqueId, baseLinearVelocity,
                                            baseAngularVelocity, sm)) {
    PyErr_SetString(SpamError, "getBaseVelocity failed.");
    return NULL;
  }
...
static PyObject* pybullet_getBodyUniqueId(PyObject* self, PyObject* args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "i|i", kwlist, &serialIndex,
                                   &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
...
static PyObject* pybullet_removeBody(PyObject* self, PyObject* args,
                                     PyObject* keywds) {
  {
    int bodyUniqueId = -1;
    b3PhysicsClientHandle sm = 0;
    ...
    if (bodyUniqueId >= 0) {
      ...
      if (b3CanSubmitCommand(sm)) {
        statusHandle = b3SubmitClientCommandAndWaitStatus(
            sm, b3InitRemoveBodyCommand(sm, bodyUniqueId));
        statusType = b3GetStatusType(statusHandle);
      }
    }
  }
...

    {
      ...
      if (b3GetBodyInfo(sm, bodyUniqueId, &info)) {
        ...
        PyTuple_SetItem(pyListJointInfo, 0,
                        PyString_FromString(info.m_baseName));
        PyTuple_SetItem(pyListJointInfo, 1,
                        PyString_FromString(info.m_bodyName));
...
static PyObject* pybullet_getConstraintInfo(PyObject* self, PyObject* args,
                                            PyObject* keywds) {
  {
...

    {
      ...
      if (b3GetUserConstraintInfo(sm, constraintUniqueId, &constraintInfo)) {
        ...
        PyTuple_SetItem(pyListConstraintInfo, 0,
                        PyLong_FromLong(constraintInfo.m_parentBodyIndex));
        PyTuple_SetItem(pyListConstraintInfo, 1,
                        PyLong_FromLong(constraintInfo.m_parentJointIndex));
        PyTuple_SetItem(pyListConstraintInfo, 2,
                        PyLong_FromLong(constraintInfo.m_childBodyIndex));
        PyTuple_SetItem(pyListConstraintInfo, 3,
                        PyLong_FromLong(constraintInfo.m_childJointIndex));
        PyTuple_SetItem(pyListConstraintInfo, 4,
                        PyLong_FromLong(constraintInfo.m_jointType));

        {
          PyObject* axisObj = PyTuple_New(3);
          PyTuple_SetItem(axisObj, 0,
                          PyFloat_FromDouble(constraintInfo.m_jointAxis[0]));
          PyTuple_SetItem(axisObj, 1,
                          PyFloat_FromDouble(constraintInfo.m_jointAxis[1]));
          PyTuple_SetItem(axisObj, 2,
                          PyFloat_FromDouble(constraintInfo.m_jointAxis[2]));
          PyTuple_SetItem(pyListConstraintInfo, 5, axisObj);
        }
        {
          PyObject* parentFramePositionObj = PyTuple_New(3);
          PyTuple_SetItem(parentFramePositionObj, 0,
                          PyFloat_FromDouble(constraintInfo.m_parentFrame[0]));
          PyTuple_SetItem(parentFramePositionObj, 1,
                          PyFloat_FromDouble(constraintInfo.m_parentFrame[1]));
          PyTuple_SetItem(parentFramePositionObj, 2,
                          PyFloat_FromDouble(constraintInfo.m_parentFrame[2]));
          PyTuple_SetItem(pyListConstraintInfo, 6, parentFramePositionObj);
        }
        {
          PyObject* childFramePositionObj = PyTuple_New(3);
          PyTuple_SetItem(childFramePositionObj, 0,
                          PyFloat_FromDouble(constraintInfo.m_childFrame[0]));
          PyTuple_SetItem(childFramePositionObj, 1,
                          PyFloat_FromDouble(constraintInfo.m_childFrame[1]));
          PyTuple_SetItem(childFramePositionObj, 2,
                          PyFloat_FromDouble(constraintInfo.m_childFrame[2]));
          PyTuple_SetItem(pyListConstraintInfo, 7, childFramePositionObj);
        }
        {
          PyObject* parentFrameOrientationObj = PyTuple_New(4);
          PyTuple_SetItem(parentFrameOrientationObj, 0,
                          PyFloat_FromDouble(constraintInfo.m_parentFrame[3]));
          PyTuple_SetItem(parentFrameOrientationObj, 1,
                          PyFloat_FromDouble(constraintInfo.m_parentFrame[4]));
          PyTuple_SetItem(parentFrameOrientationObj, 2,
                          PyFloat_FromDouble(constraintInfo.m_parentFrame[5]));
          PyTuple_SetItem(parentFrameOrientationObj, 3,
                          PyFloat_FromDouble(constraintInfo.m_parentFrame[6]));
          PyTuple_SetItem(pyListConstraintInfo, 8, parentFrameOrientationObj);
        }
        {
          PyObject* childFrameOrientation = PyTuple_New(4);
          PyTuple_SetItem(childFrameOrientation, 0,
                          PyFloat_FromDouble(constraintInfo.m_childFrame[3]));
          PyTuple_SetItem(childFrameOrientation, 1,
                          PyFloat_FromDouble(constraintInfo.m_childFrame[4]));
          PyTuple_SetItem(childFrameOrientation, 2,
                          PyFloat_FromDouble(constraintInfo.m_childFrame[5]));
          PyTuple_SetItem(childFrameOrientation, 3,
                          PyFloat_FromDouble(constraintInfo.m_childFrame[6]));
          PyTuple_SetItem(pyListConstraintInfo, 9, childFrameOrientation);
        }
        PyTuple_SetItem(pyListConstraintInfo, 10,
                        PyFloat_FromDouble(constraintInfo.m_maxAppliedForce));
        PyTuple_SetItem(pyListConstraintInfo, 11,
                        PyFloat_FromDouble(constraintInfo.m_gearRatio));
        PyTuple_SetItem(pyListConstraintInfo, 12,
                        PyLong_FromLong(constraintInfo.m_gearAuxLink));
        PyTuple_SetItem(
            pyListConstraintInfo, 13,
            PyFloat_FromDouble(constraintInfo.m_relativePositionTarget));
        PyTuple_SetItem(pyListConstraintInfo, 14,
                        PyFloat_FromDouble(constraintInfo.m_erp));
...
static PyObject* pybullet_getConstraintState(PyObject* self, PyObject* args,
                                             PyObject* keywds) {
  {
    int constraintUniqueId = -1;
    b3PhysicsClientHandle sm = 0;
...

    {
      ...
      if (b3CanSubmitCommand(sm)) {
        ...
        cmd_handle =
            b3InitGetUserConstraintStateCommand(sm, constraintUniqueId);
        ...
        if (b3GetStatusUserConstraintState(statusHandle, &constraintState)) {
          if (constraintState.m_numDofs) {
            PyObject* appliedConstraintForces =
                PyTuple_New(constraintState.m_numDofs);
            int i = 0;
            for (i = 0; i < constraintState.m_numDofs; i++) {
              PyTuple_SetItem(
                  appliedConstraintForces, i,
                  PyFloat_FromDouble(
...
static PyObject* pybullet_getConstraintUniqueId(PyObject* self, PyObject* args,
                                                PyObject* keywds) {
  int physicsClientId = 0;
  int serialIndex = -1;
  b3PhysicsClientHandle sm = 0;
...
static PyObject* pybullet_getNumConstraints(PyObject* self, PyObject* args,
                                            PyObject* keywds) {
...
static PyObject* pybullet_getAPIVersion(PyObject* self, PyObject* args,
                                        PyObject* keywds) {
  int physicsClientId = 0;
  b3PhysicsClientHandle sm = 0;
  static char* kwlist[] = {"physicsClientId", NULL};
...
static PyObject* pybullet_resetJointState(PyObject* self, PyObject* args,
                                          PyObject* keywds) {
  {
    ...
    int physicsClientId = 0;
    static char* kwlist[] = {"bodyUniqueId",   "jointIndex",      "targetValue",
                             "targetVelocity", "physicsClientId", NULL};
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "iid|di", kwlist,
                                     &bodyUniqueId, &jointIndex, &targetValue,
                                     &targetVelocity, &physicsClientId)) {
      return NULL;
    }
    sm = getPhysicsClient(physicsClientId);
    if (sm == 0) {
...
static PyObject* pybullet_resetBaseVelocity(PyObject* self, PyObject* args,
                                            PyObject* keywds) {
  static char* kwlist[] = {"objectUniqueId", "linearVelocity",
                           "angularVelocity", "physicsClientId", NULL};

  {
    ...
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "i|OOi", kwlist,
                                     &bodyUniqueId, &linVelObj, &angVelObj,
                                     &physicsClientId)) {
      return NULL;
    }
...
static PyObject* pybullet_resetBasePositionAndOrientation(PyObject* self,
                                                          ...
                                                          PyObject* keywds) {
  {
    ...
    int physicsClientId = 0;
    static char* kwlist[] = {"bodyUniqueId", "posObj", "ornObj",
                             "physicsClientId", NULL};
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "iOO|i", kwlist,
                                     &bodyUniqueId, &posObj, &ornObj,
                                     &physicsClientId)) {
      return NULL;
    }
    sm = getPhysicsClient(physicsClientId);
...
static PyObject* pybullet_getJointInfo(PyObject* self, PyObject* args,
  ...
  b3PhysicsClientHandle sm = 0;
  int physicsClientId = 0;
  static char* kwlist[] = {"bodyUniqueId", "jointIndex", "physicsClientId",
                           NULL};
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "ii|i", kwlist, &bodyUniqueId,
                                   &jointIndex, &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
  if (sm == 0) {
  ...
  {
    {
      ...
      if (b3GetJointInfo(sm, bodyUniqueId, jointIndex, &info)) {
        ...
        PyTuple_SetItem(pyListJointInfo, 16,
                        PyInt_FromLong(info.m_parentIndex));
  ...
  {
    {
      ...
      if (bodyUniqueId < 0) {
        PyErr_SetString(SpamError,
                        "getJointState failed; invalid bodyUniqueId");
        return NULL;
      }
      if (jointIndex < 0) {
...
static PyObject* pybullet_getJointStates(PyObject* self, PyObject* args,
                                         PyObject* keywds) {
  PyObject* pyListJointForceTorque;
  PyObject* pyListJointState;
  PyObject* jointIndicesObj = 0;
  ...
  b3PhysicsClientHandle sm = 0;
  int physicsClientId = 0;
  static char* kwlist[] = {"bodyUniqueId", "jointIndices", "physicsClientId",
                           NULL};
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "iO|i", kwlist, &bodyUniqueId,
                                   &jointIndicesObj, &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
  if (sm == 0) {
  ...
  {
    {
      ...
      for (i = 0; i < numRequestedJoints; i++) {
        int jointIndex =
            pybullet_internalGetFloatFromSequence(jointIndicesSeq, i);
        if ((jointIndex >= numJoints) || (jointIndex < 0)) {
          Py_DECREF(jointIndicesSeq);
          PyErr_SetString(SpamError, "Joint index out-of-range.");
          return NULL;
...
static PyObject* pybullet_getLinkState(PyObject* self, PyObject* args,
  ...
  int physicsClientId = 0;
  static char* kwlist[] = {"bodyUniqueId",        "linkIndex",
                           "computeLinkVelocity", "computeForwardKinematics",
                           "physicsClientId",     NULL};
  if (!PyArg_ParseTupleAndKeywords(
          args, keywds, "ii|iii", kwlist, &bodyUniqueId, &linkIndex,
          &computeLinkVelocity, &computeForwardKinematics, &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
  if (sm == 0) {
  ...
  {
    {
      ...
      if (computeLinkVelocity) {
        b3RequestActualStateCommandComputeLinkVelocity(cmd_handle,
                                                       computeLinkVelocity);
      }
      ...
      if (b3GetLinkState(sm, status_handle, linkIndex, &linkState)) {
        ...
        for (i = 0; i < 3; ++i) {
          PyTuple_SetItem(
              pyLinkStateLocalInertialPosition, i,
        ...
        for (i = 0; i < 4; ++i) {
          PyTuple_SetItem(
              pyLinkStateLocalInertialOrientation, i,
        ...
        for (i = 0; i < 3; ++i) {
          PyTuple_SetItem(
              pyLinkStateWorldLinkFramePosition, i,
        ...
        for (i = 0; i < 4; ++i) {
          PyTuple_SetItem(
              pyLinkStateWorldLinkFrameOrientation, i,
        ...
        if (computeLinkVelocity) {
          ...
          for (i = 0; i < 3; ++i) {
            PyTuple_SetItem(
                pyLinkStateWorldLinkLinearVelocity, i,
                PyFloat_FromDouble(linkState.m_worldLinearVelocity[i]));
            PyTuple_SetItem(
                pyLinkStateWorldLinkAngularVelocity, i,
...
static PyObject* pybullet_readUserDebugParameter(PyObject* self, PyObject* args,
                                                 PyObject* keywds) {
  b3SharedMemoryCommandHandle commandHandle;
  b3SharedMemoryStatusHandle statusHandle;
  int statusType;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "i|i", kwlist, &itemUniqueId,
                                   &physicsClientId)) {
    return NULL;
  }
...
static PyObject* pybullet_addUserDebugParameter(PyObject* self, PyObject* args,
                                                PyObject* keywds) {
  b3SharedMemoryCommandHandle commandHandle;
  b3SharedMemoryStatusHandle statusHandle;
  int statusType;
  ...
  int physicsClientId = 0;
  b3PhysicsClientHandle sm = 0;
  static char* kwlist[] = {"paramName",  "rangeMin",        "rangeMax",
                           "startValue", "physicsClientId", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, keywds, "s|dddi", kwlist, &text,
                                   &rangeMin, &rangeMax, &startValue,
                                   &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
  ...
  commandHandle =
      b3InitUserDebugAddParameter(sm, text, rangeMin, rangeMax, startValue);
...
static PyObject* pybullet_addUserDebugText(PyObject* self, PyObject* args,
                                           PyObject* keywds) {
  b3SharedMemoryCommandHandle commandHandle;
  b3SharedMemoryStatusHandle statusHandle;
  int statusType;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "sO|OddOiiii", kwlist, &text,
                                   &textPositionObj, &textColorRGBObj,
                                   &textSize, &lifeTime, &textOrientationObj,
                                   &parentObjectUniqueId, &parentLinkIndex,
                                   &replaceItemUniqueId, &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
  if (sm == 0) {
  ...
  commandHandle = b3InitUserDebugDrawAddText3D(sm, text, posXYZ, colorRGB,
                                               textSize, lifeTime);
...
static PyObject* pybullet_addUserDebugLine(PyObject* self, PyObject* args,
                                           PyObject* keywds) {
  b3SharedMemoryCommandHandle commandHandle;
  b3SharedMemoryStatusHandle statusHandle;
  int statusType;
  ...
  b3PhysicsClientHandle sm = 0;
  static char* kwlist[] = {
      "lineFromXYZ",     "lineToXYZ",       "lineColorRGB",
      "lineWidth",       "lifeTime",        "parentObjectUniqueId",
      "parentLinkIndex", "physicsClientId", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, keywds, "OO|Oddiii", kwlist,
                                   &lineFromObj, &lineToObj, &lineColorRGBObj,
                                   &lineWidth, &lifeTime, &parentObjectUniqueId,
                                   &parentLinkIndex, &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
  if (sm == 0) {
  ...
  commandHandle = b3InitUserDebugDrawAddLine3D(sm, fromXYZ, toXYZ, colorRGB,
                                               lineWidth, lifeTime);
...
static PyObject* pybullet_removeUserDebugItem(PyObject* self, PyObject* args,
                                              PyObject* keywds) {
  b3SharedMemoryCommandHandle commandHandle;
  b3SharedMemoryStatusHandle statusHandle;
  int statusType;
...
static PyObject* pybullet_removeAllUserDebugItems(PyObject* self,
                                                  PyObject* args,
                                                  PyObject* keywds) {
  b3SharedMemoryCommandHandle commandHandle;
  b3SharedMemoryStatusHandle statusHandle;
...
static PyObject* pybullet_startStateLogging(PyObject* self, PyObject* args,
                                            PyObject* keywds) {
  b3SharedMemoryStatusHandle statusHandle;
  int statusType;
  ...
  static char* kwlist[] = {"loggingType", "fileName",        "objectUniqueIds",
                           "maxLogDof",   "bodyUniqueIdA",   "bodyUniqueIdB",
                           "linkIndexA",  "linkIndexB",      "deviceTypeFilter",
                           "logFlags",    "physicsClientId", NULL};
  int physicsClientId = 0;

  if (!PyArg_ParseTupleAndKeywords(args, keywds, "is|Oiiiiiiii", kwlist,
                                   &loggingType, &fileName, &objectUniqueIdsObj,
                                   &maxLogDof, &bodyUniqueIdA, &bodyUniqueIdB,
                                   &linkIndexA, &linkIndexB, &deviceTypeFilter,
                                   &logFlags, &physicsClientId))
    return NULL;
  ...
  }
  {
    ...
    if (objectUniqueIdsObj) {
      PyObject* seq = PySequence_Fast(
          objectUniqueIdsObj, "expected a sequence of object unique ids");
      if (seq) {
...
static PyObject* pybullet_submitProfileTiming(PyObject* self, PyObject* args,
                                              PyObject* keywds) {
  //	b3SharedMemoryStatusHandle statusHandle;
  //	int statusType;
  char* eventName = 0;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "s|ii", kwlist, &eventName,
                                   &duractionInMicroSeconds, &physicsClientId))
    return NULL;
  ...
  if (eventName) {
    ...
    if (duractionInMicroSeconds >= 0) {
      b3SetProfileTimingDuractionInMicroSeconds(commandHandle,
                                                duractionInMicroSeconds);
    }
...
static PyObject* pybullet_stopStateLogging(PyObject* self, PyObject* args,
                                           PyObject* keywds) {
  b3SharedMemoryStatusHandle statusHandle;
  int statusType;
  int loggingId = -1;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "i|i", kwlist, &loggingId,
                                   &physicsClientId))
    return NULL;
...
static PyObject* pybullet_setAdditionalSearchPath(PyObject* self,
                                                  PyObject* args,
                                                  PyObject* keywds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "s|i", kwlist, &path,
                                   &physicsClientId))
    return NULL;
  if (path) {
    b3SharedMemoryCommandHandle commandHandle;
...
static PyObject* pybullet_setTimeOut(PyObject* self, PyObject* args,
                                     PyObject* keywds) {
...
static PyObject* pybullet_rayTestObsolete(PyObject* self, PyObject* args,
                                          PyObject* keywds) {
  b3SharedMemoryCommandHandle commandHandle;
  b3SharedMemoryStatusHandle statusHandle;
  int statusType;
  ...
  double to[3];
  b3PhysicsClientHandle sm = 0;
  static char* kwlist[] = {"rayFromPosition", "rayToPosition",
                           "physicsClientId", NULL};
  int physicsClientId = 0;

  if (!PyArg_ParseTupleAndKeywords(args, keywds, "OO|i", kwlist, &rayFromObj,
                                   &rayToObj, &physicsClientId))
    return NULL;
  ...
  if (statusType == CMD_REQUEST_RAY_CAST_INTERSECTIONS_COMPLETED) {
    ...
    for (i = 0; i < raycastInfo.m_numRayHits; i++) {
      ...
      {
        PyObject* ob =
            PyInt_FromLong(raycastInfo.m_rayHits[i].m_hitObjectUniqueId);
        PyTuple_SetItem(singleHitObj, 0, ob);
      }
      {
        PyObject* ob =
            PyInt_FromLong(raycastInfo.m_rayHits[i].m_hitObjectLinkIndex);
        PyTuple_SetItem(singleHitObj, 1, ob);
      }
      {
        PyObject* ob =
            PyFloat_FromDouble(raycastInfo.m_rayHits[i].m_hitFraction);
      ...
      }
      {
        ...
        int p;
        for (p = 0; p < 3; p++) {
          PyObject* ob = PyFloat_FromDouble(
              raycastInfo.m_rayHits[i].m_hitPositionWorld[p]);
      ...
      }
      {
        ...
        for (p = 0; p < 3; p++) {
          PyObject* ob =
              PyFloat_FromDouble(raycastInfo.m_rayHits[i].m_hitNormalWorld[p]);
...
static PyObject* pybullet_rayTestBatch(PyObject* self, PyObject* args,
                                       PyObject* keywds) {
  b3SharedMemoryCommandHandle commandHandle;
  b3SharedMemoryStatusHandle statusHandle;
  int statusType;
  ...
  static char* kwlist[] = {"rayFromPositions", "rayToPositions",
                           "physicsClientId", NULL};
  int physicsClientId = 0;
  ...
  if (rayFromObjList) {
    PyObject* seqRayFromObj = PySequence_Fast(
        rayFromObjList, "expected a sequence of rayFrom positions");
    PyObject* seqRayToObj = PySequence_Fast(
        rayToObjList, "expected a sequence of 'rayTo' positions");
    ...
    if (seqRayFromObj && seqRayToObj) {
      ...
      if (lenFrom != lenTo) {
        PyErr_SetString(
            SpamError,
            "Size of from_positions need to be equal to size of to_positions.");
        ...
        if (lenFrom >= MAX_RAY_INTERSECTION_BATCH_SIZE) {
          PyErr_SetString(SpamError,
                          "Number of rays exceed the maximum batch size.");
          Py_DECREF(seqRayFromObj);
          Py_DECREF(seqRayToObj);
          return NULL;
        }
        for (i = 0; i < lenFrom; i++) {
          ...
          if ((pybullet_internalSetVectord(rayFromObj, rayFromWorld)) &&
            ...
            PyErr_SetString(SpamError,
                            "Items in the from/to positions need to be an "
                            "[x,y,z] list of 3 floats/doubles");
            Py_DECREF(seqRayFromObj);
            Py_DECREF(seqRayToObj);
...
static PyObject* pybullet_getMatrixFromQuaternion(PyObject* self,
                                                  PyObject* args,
                                                  PyObject* keywds) {
  PyObject* quatObj;
  double quat[4];
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "O|i", kwlist, &quatObj,
                                   &physicsClientId)) {
    return NULL;
  }
  ...
  if (quatObj) {
    if (pybullet_internalSetVector4d(quatObj, quat)) {
      ...
      int i;
      double d = quat[0] * quat[0] + quat[1] * quat[1] + quat[2] * quat[2] +
                 quat[3] * quat[3];
      double s = 2.0 / d;
      ...
      double mat3x3[9] = {1.0 - (yy + zz), xy - wz,         xz + wy,
...
static PyObject* pybullet_setVRCameraState(PyObject* self, PyObject* args,
                                           PyObject* keywds) {
  b3SharedMemoryCommandHandle commandHandle;
  b3SharedMemoryStatusHandle statusHandle;
  int statusType;
  ...
  static char* kwlist[] = {"rootPosition",    "rootOrientation", "trackObject",
                           "trackObjectFlag", "physicsClientId", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "|OOiii", kwlist, &rootPosObj,
                                   &rootOrnObj, &trackObjectUid,
                                   &trackObjectFlag, &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
  if (sm == 0) {
...
static PyObject* pybullet_getKeyboardEvents(PyObject* self, PyObject* args,
                                            PyObject* keywds) {
  b3SharedMemoryCommandHandle commandHandle;
  int physicsClientId = 0;
  b3PhysicsClientHandle sm = 0;
  ...
  for (i = 0; i < keyboardEventsData.m_numKeyboardEvents; i++) {
    PyObject* keyObj =
        PyLong_FromLong(keyboardEventsData.m_keyboardEvents[i].m_keyCode);
    PyObject* valObj =
        PyLong_FromLong(keyboardEventsData.m_keyboardEvents[i].m_keyState);
    PyDict_SetItem(keyEventsObj, keyObj, valObj);
  }
  return keyEventsObj;
}

static PyObject* pybullet_getMouseEvents(PyObject* self, PyObject* args,
                                         PyObject* keywds) {
  b3SharedMemoryCommandHandle commandHandle;
  int physicsClientId = 0;
  b3PhysicsClientHandle sm = 0;
  ...
  for (i = 0; i < mouseEventsData.m_numMouseEvents; i++) {
    ...
    PyTuple_SetItem(
        mouseEventObj, 0,
        PyInt_FromLong(mouseEventsData.m_mouseEvents[i].m_eventType));
    PyTuple_SetItem(
        mouseEventObj, 1,
        PyFloat_FromDouble(mouseEventsData.m_mouseEvents[i].m_mousePosX));
    PyTuple_SetItem(
        mouseEventObj, 2,
        PyFloat_FromDouble(mouseEventsData.m_mouseEvents[i].m_mousePosY));
    PyTuple_SetItem(
        mouseEventObj, 3,
        PyInt_FromLong(mouseEventsData.m_mouseEvents[i].m_buttonIndex));
    PyTuple_SetItem(
        mouseEventObj, 4,
        PyInt_FromLong(mouseEventsData.m_mouseEvents[i].m_buttonState));
    PyTuple_SetItem(mouseEventsObj, i, mouseEventObj);
  }
  return mouseEventsObj;
}

static PyObject* pybullet_getVREvents(PyObject* self, PyObject* args,
                                      PyObject* keywds) {
  b3SharedMemoryCommandHandle commandHandle;
  b3SharedMemoryStatusHandle statusHandle;
  int statusType;
  ...
  int allAnalogAxes = 0;
  b3PhysicsClientHandle sm = 0;
  static char* kwlist[] = {"deviceTypeFilter", "allAnalogAxes",
                           "physicsClientId", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "|iii", kwlist,
                                   &deviceTypeFilter, &allAnalogAxes,
                                   &physicsClientId)) {
    return NULL;
  }
  ...
  if (statusType == CMD_REQUEST_VR_EVENTS_DATA_COMPLETED) {
    ...
    for (i = 0; i < vrEvents.m_numControllerEvents; i++) {
      ...
      PyTuple_SetItem(
          vrEventObj, 0,
          PyInt_FromLong(vrEvents.m_controllerEvents[i].m_controllerId));
      {
        PyObject* posObj = PyTuple_New(3);
        PyTuple_SetItem(
            posObj, 0,
            PyFloat_FromDouble(vrEvents.m_controllerEvents[i].m_pos[0]));
        PyTuple_SetItem(
            posObj, 1,
            PyFloat_FromDouble(vrEvents.m_controllerEvents[i].m_pos[1]));
        PyTuple_SetItem(
            posObj, 2,
            PyFloat_FromDouble(vrEvents.m_controllerEvents[i].m_pos[2]));
      ...
      }
      {
        ...
        PyTuple_SetItem(
            ornObj, 0,
            PyFloat_FromDouble(vrEvents.m_controllerEvents[i].m_orn[0]));
        PyTuple_SetItem(
            ornObj, 1,
            PyFloat_FromDouble(vrEvents.m_controllerEvents[i].m_orn[1]));
        PyTuple_SetItem(
            ornObj, 2,
            PyFloat_FromDouble(vrEvents.m_controllerEvents[i].m_orn[2]));
        PyTuple_SetItem(
            ornObj, 3,
            PyFloat_FromDouble(vrEvents.m_controllerEvents[i].m_orn[3]));
        PyTuple_SetItem(vrEventObj, 2, ornObj);
      }

      PyTuple_SetItem(
          vrEventObj, 3,
          PyFloat_FromDouble(vrEvents.m_controllerEvents[i].m_analogAxis));
      PyTuple_SetItem(
          vrEventObj, 4,
          PyInt_FromLong(vrEvents.m_controllerEvents[i].m_numButtonEvents));
      PyTuple_SetItem(
          vrEventObj, 5,
          PyInt_FromLong(vrEvents.m_controllerEvents[i].m_numMoveEvents));
      {
        PyObject* buttonsObj = PyTuple_New(MAX_VR_BUTTONS);
        int b;
        for (b = 0; b < MAX_VR_BUTTONS; b++) {
          PyObject* button =
              PyInt_FromLong(vrEvents.m_controllerEvents[i].m_buttons[b]);
      ...
      PyTuple_SetItem(
          vrEventObj, 7,
          PyInt_FromLong(vrEvents.m_controllerEvents[i].m_deviceType));

      if (allAnalogAxes) {
        PyObject* buttonsObj = PyTuple_New(MAX_VR_ANALOG_AXIS * 2);
        int b;
        for (b = 0; b < MAX_VR_ANALOG_AXIS * 2; b++) {
          PyObject* axisVal = PyFloat_FromDouble(
              vrEvents.m_controllerEvents[i].m_auxAnalogAxis[b]);
...
static PyObject* pybullet_getDebugVisualizerCamera(PyObject* self,
                                                   PyObject* args,
                                                   PyObject* keywds) {
  int physicsClientId = 0;
  b3PhysicsClientHandle sm = 0;
...
static PyObject* pybullet_configureDebugVisualizer(PyObject* self,
                                                   PyObject* args,
                                                   PyObject* keywds) {
  int flag = 1;
  int enable = -1;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "ii|i", kwlist, &flag, &enable,
                                   &physicsClientId))
    return NULL;
  ...
  {
    b3SharedMemoryCommandHandle commandHandle =
        b3InitConfigureOpenGLVisualizer(sm);
    b3ConfigureOpenGLVisualizerSetVisualizationFlags(commandHandle, flag,
                                                     enable);
    b3SubmitClientCommandAndWaitStatus(sm, commandHandle);
  }
  Py_INCREF(Py_None);
...
static PyObject* pybullet_resetDebugVisualizerCamera(PyObject* self,
                                                     PyObject* args,
                                                     PyObject* keywds) {
  ...
  int physicsClientId = 0;
  b3PhysicsClientHandle sm = 0;
  static char* kwlist[] = {"cameraDistance",  "cameraYaw",
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "fffO|i", kwlist,
                                   ...
                                   &cameraTargetPosObj, &physicsClientId))
    return NULL;
...

  {
    ...
    if ((cameraDistance >= 0)) {
      ...
      if (pybullet_internalSetVector(cameraTargetPosObj,
...
static PyObject* pybullet_setDebugObjectColor(PyObject* self, PyObject* args,
                                              PyObject* keywds) {
  ...
  int physicsClientId = 0;
  b3PhysicsClientHandle sm = 0;
  static char* kwlist[] = {"objectUniqueId", "linkIndex", "objectDebugColorRGB",
                           "physicsClientId", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, keywds, "ii|Oi", kwlist,
                                   &objectUniqueId, &linkIndex,
                                   &objectColorRGBObj, &physicsClientId))
    return NULL;
  ...
  if (objectColorRGBObj) {
    if (pybullet_internalSetVectord(objectColorRGBObj, objectColorRGB)) {
      ...
      b3SetDebugObjectColor(commandHandle, objectUniqueId, linkIndex,
                            objectColorRGB);
      b3SubmitClientCommandAndWaitStatus(sm, commandHandle);
    }
  } else {
...
static PyObject* pybullet_getCollisionShapeData(PyObject* self, PyObject* args,
                                                PyObject* keywds) {
  int objectUniqueId = -1;
  b3SharedMemoryCommandHandle commandHandle;
  b3SharedMemoryStatusHandle statusHandle;
  ...
  int physicsClientId = 0;
  b3PhysicsClientHandle sm = 0;
  static char* kwlist[] = {"objectUniqueId", "linkIndex", "physicsClientId",
                           NULL};
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "ii|i", kwlist,
                                   &objectUniqueId, &linkIndex,
                                   &physicsClientId)) {
  ...
  {
    commandHandle =
        b3InitRequestCollisionShapeInformation(sm, objectUniqueId, linkIndex);
    statusHandle = b3SubmitClientCommandAndWaitStatus(sm, commandHandle);
    statusType = b3GetStatusType(statusHandle);
    if (statusType == CMD_COLLISION_SHAPE_INFO_COMPLETED) {
      ...
      for (i = 0; i < collisionShapeInfo.m_numCollisionShapes; i++) {
        ...
        item = PyInt_FromLong(
            collisionShapeInfo.m_collisionShapeData[i].m_objectUniqueId);
        PyTuple_SetItem(collisionShapeObList, 0, item);

        item = PyInt_FromLong(
            collisionShapeInfo.m_collisionShapeData[i].m_linkIndex);
        PyTuple_SetItem(collisionShapeObList, 1, item);

        item = PyInt_FromLong(
            collisionShapeInfo.m_collisionShapeData[i].m_collisionGeometryType);
        PyTuple_SetItem(collisionShapeObList, 2, item);

        {
          PyObject* vec = PyTuple_New(3);
          item = PyFloat_FromDouble(
              collisionShapeInfo.m_collisionShapeData[i].m_dimensions[0]);
          PyTuple_SetItem(vec, 0, item);
          item = PyFloat_FromDouble(
              collisionShapeInfo.m_collisionShapeData[i].m_dimensions[1]);
          PyTuple_SetItem(vec, 1, item);
          item = PyFloat_FromDouble(
              collisionShapeInfo.m_collisionShapeData[i].m_dimensions[2]);
          PyTuple_SetItem(vec, 2, item);
          PyTuple_SetItem(collisionShapeObList, 3, vec);
        }

        item = PyString_FromString(
            collisionShapeInfo.m_collisionShapeData[i].m_meshAssetFileName);
        PyTuple_SetItem(collisionShapeObList, 4, item);

        {
          PyObject* vec = PyTuple_New(3);
          item = PyFloat_FromDouble(collisionShapeInfo.m_collisionShapeData[i]
                                        .m_localCollisionFrame[0]);
          PyTuple_SetItem(vec, 0, item);
          item = PyFloat_FromDouble(collisionShapeInfo.m_collisionShapeData[i]
                                        .m_localCollisionFrame[1]);
          PyTuple_SetItem(vec, 1, item);
          item = PyFloat_FromDouble(collisionShapeInfo.m_collisionShapeData[i]
                                        .m_localCollisionFrame[2]);
          PyTuple_SetItem(vec, 2, item);
...

        {
          ...
          item = PyFloat_FromDouble(collisionShapeInfo.m_collisionShapeData[i]
                                        .m_localCollisionFrame[3]);
          PyTuple_SetItem(vec, 0, item);
          item = PyFloat_FromDouble(collisionShapeInfo.m_collisionShapeData[i]
                                        .m_localCollisionFrame[4]);
          PyTuple_SetItem(vec, 1, item);
          item = PyFloat_FromDouble(collisionShapeInfo.m_collisionShapeData[i]
                                        .m_localCollisionFrame[5]);
          PyTuple_SetItem(vec, 2, item);
          item = PyFloat_FromDouble(collisionShapeInfo.m_collisionShapeData[i]
                                        .m_localCollisionFrame[6]);
...
static PyObject* pybullet_getVisualShapeData(PyObject* self, PyObject* args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "i|i", kwlist, &objectUniqueId,
                                   &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
...

  {
    ...
    if (statusType == CMD_VISUAL_SHAPE_INFO_COMPLETED) {
      ...
      for (i = 0; i < visualShapeInfo.m_numVisualShapes; i++) {
        ...
        PyObject* item;
        item = PyInt_FromLong(
            visualShapeInfo.m_visualShapeData[i].m_objectUniqueId);
        ...
        item = PyInt_FromLong(
            visualShapeInfo.m_visualShapeData[i].m_visualGeometryType);
        ...
        item = PyString_FromString(
            visualShapeInfo.m_visualShapeData[i].m_meshAssetFileName);
        PyTuple_SetItem(visualShapeObList, 4, item);

        {
          PyObject* vec = PyTuple_New(3);
          item = PyFloat_FromDouble(
              visualShapeInfo.m_visualShapeData[i].m_localVisualFrame[0]);
          PyTuple_SetItem(vec, 0, item);
          item = PyFloat_FromDouble(
              visualShapeInfo.m_visualShapeData[i].m_localVisualFrame[1]);
          PyTuple_SetItem(vec, 1, item);
          item = PyFloat_FromDouble(
              visualShapeInfo.m_visualShapeData[i].m_localVisualFrame[2]);
          PyTuple_SetItem(vec, 2, item);
...

        {
          PyObject* vec = PyTuple_New(4);
          item = PyFloat_FromDouble(
              visualShapeInfo.m_visualShapeData[i].m_localVisualFrame[3]);
          PyTuple_SetItem(vec, 0, item);
          item = PyFloat_FromDouble(
              visualShapeInfo.m_visualShapeData[i].m_localVisualFrame[4]);
          PyTuple_SetItem(vec, 1, item);
          item = PyFloat_FromDouble(
              visualShapeInfo.m_visualShapeData[i].m_localVisualFrame[5]);
          PyTuple_SetItem(vec, 2, item);
          item = PyFloat_FromDouble(
              visualShapeInfo.m_visualShapeData[i].m_localVisualFrame[6]);
          PyTuple_SetItem(vec, 3, item);
...

        {
          ...
          item = PyFloat_FromDouble(
              visualShapeInfo.m_visualShapeData[i].m_rgbaColor[0]);
          PyTuple_SetItem(rgba, 0, item);
          item = PyFloat_FromDouble(
              visualShapeInfo.m_visualShapeData[i].m_rgbaColor[1]);
          PyTuple_SetItem(rgba, 1, item);
          item = PyFloat_FromDouble(
              visualShapeInfo.m_visualShapeData[i].m_rgbaColor[2]);
          PyTuple_SetItem(rgba, 2, item);
          item = PyFloat_FromDouble(
              visualShapeInfo.m_visualShapeData[i].m_rgbaColor[3]);
...
static PyObject* pybullet_changeVisualShape(PyObject* self, PyObject* args,
                                            PyObject* keywds) {
  int objectUniqueId = -1;
  int jointIndex = -1;
  int shapeIndex = -1;
  ...
  b3PhysicsClientHandle sm = 0;
  static char* kwlist[] = {"objectUniqueId",  "linkIndex", "shapeIndex",
                           "textureUniqueId", "rgbaColor", "specularColor",
                           "physicsClientId", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "ii|iiOOi", kwlist,
                                   &objectUniqueId, &jointIndex, &shapeIndex,
                                   &textureUniqueId, &rgbaColorObj,
                                   &specularColorObj, &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
  if (sm == 0) {
  ...
  {
    commandHandle = b3InitUpdateVisualShape(sm, objectUniqueId, jointIndex,
                                            shapeIndex, textureUniqueId);
...
static PyObject* pybullet_changeTexture(PyObject* self, PyObject* args,
                                        PyObject* keywds) {
  ...
  b3PhysicsClientHandle sm = 0;
  static char* kwlist[] = {"textureUniqueId", "pixels",          "width",
                           "height",          "physicsClientId", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "iOii|i", kwlist,
                                   &textureUniqueId, &pixelsObj, &width,
                                   &height, &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
  if (sm == 0) {
  ...
  if (textureUniqueId >= 0 && width >= 0 && height >= 0 && pixelsObj) {
    ...
    commandHandle = b3CreateChangeTextureCommandInit(
        sm, textureUniqueId, width, height, (const char*)pixelBuffer);
    free(pixelBuffer);
    statusHandle = b3SubmitClientCommandAndWaitStatus(sm, commandHandle);
    statusType = b3GetStatusType(statusHandle);
...
static PyObject* pybullet_loadTexture(PyObject* self, PyObject* args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "s|i", kwlist, &filename,
                                   &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
...
static PyObject* MyConvertContactPoint(
  ...
  /*
   ...
   3     int m_linkIndexA;
   4     int m_linkIndexB;
   5		double m_positionOnAInWS[3];//contact point location on object
   A, in world space coordinates 6		double
   m_positionOnBInWS[3];//contact point location on object A, in world space
   coordinates
   7		double m_contactNormalOnBInWS[3];//the separating contact
   normal, pointing from object B towards object A
   8		double m_contactDistance;//negative number is penetration,
   positive is distance. 9		double m_normalForce;
   */
  ...
  for (i = 0; i < contactPointPtr->m_numContactPoints; i++) {
    ...
    item =
        PyInt_FromLong(contactPointPtr->m_contactPointData[i].m_bodyUniqueIdA);
    PyTuple_SetItem(contactObList, 1, item);
    item =
        PyInt_FromLong(contactPointPtr->m_contactPointData[i].m_bodyUniqueIdB);
    PyTuple_SetItem(contactObList, 2, item);
    item = PyInt_FromLong(contactPointPtr->m_contactPointData[i].m_linkIndexA);
    PyTuple_SetItem(contactObList, 3, item);
    item = PyInt_FromLong(contactPointPtr->m_contactPointData[i].m_linkIndexB);
...
static PyObject* pybullet_getOverlappingObjects(PyObject* self, PyObject* args,
                                                PyObject* keywds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "OO|i", kwlist, &aabbMinOb,
                                   &aabbMaxOb, &physicsClientId))
    return NULL;
  ...
  if (overlapData.m_numOverlappingObjects) {
    ...
    // For huge amount of overlap, we could use numpy instead (see camera pixel
    // data) What would Python do with huge amount of data? Pass it onto
    // TensorFlow!
    ...
    for (i = 0; i < overlapData.m_numOverlappingObjects; i++) {
      ...
      item = PyInt_FromLong(overlapData.m_overlappingObjects[i].m_linkIndex);
...
static PyObject* pybullet_getClosestPointData(PyObject* self, PyObject* args,
                                              PyObject* keywds) {
  int bodyUniqueIdA = -1;
  int bodyUniqueIdB = -1;
  int linkIndexA = -2;
  ...
  int physicsClientId = 0;
  b3PhysicsClientHandle sm = 0;
  static char* kwlist[] = {"bodyA",      "bodyB",      "distance",
                           "linkIndexA", "linkIndexB", "physicsClientId",
                           NULL};

  if (!PyArg_ParseTupleAndKeywords(
          args, keywds, "iid|iii", kwlist, &bodyUniqueIdA, &bodyUniqueIdB,
          &distanceThreshold, &linkIndexA, &linkIndexB, &physicsClientId))
    return NULL;
...
static PyObject* pybullet_changeUserConstraint(PyObject* self, PyObject* args,
                                               PyObject* keywds) {
  static char* kwlist[] = {"userConstraintUniqueId",
                           "jointChildPivot",
  ...
  double erp = -1;
  if (!PyArg_ParseTupleAndKeywords(
          args, keywds, "i|OOddiddi", kwlist, &userConstraintUniqueId,
          &jointChildPivotObj, &jointChildFrameOrnObj, &maxForce, &gearRatio,
          &gearAuxLink, &relativePositionTarget, &erp, &physicsClientId)) {
    return NULL;
  }
  ...
  if (relativePositionTarget < 1e10) {
    b3InitChangeUserConstraintSetRelativePositionTarget(commandHandle,
                                                        relativePositionTarget);
  }
  if (erp >= 0) {
...
static PyObject* pybullet_removeUserConstraint(PyObject* self, PyObject* args,
                                               PyObject* keywds) {
  ...
  b3PhysicsClientHandle sm = 0;
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "i|i", kwlist,
                                   &userConstraintUniqueId, &physicsClientId)) {
    return NULL;
  }
...
/*
static PyObject* pybullet_updateUserConstraint(PyObject* self, PyObject* args,
PyObject *keywds)
{
        return NULL;
}
...
static PyObject* pybullet_enableJointForceTorqueSensor(PyObject* self,
                                                       PyObject* args,
                                                       PyObject* keywds) {
  int bodyUniqueId = -1;
  int jointIndex = -1;
  ...
  static char* kwlist[] = {"bodyUniqueId", "jointIndex", "enableSensor",
                           "physicsClientId", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, keywds, "ii|ii", kwlist, &bodyUniqueId,
                                   &jointIndex, &enableSensor,
                                   &physicsClientId)) {
    return NULL;
  }
...

  {
    ...
    b3CreateSensorEnable6DofJointForceTorqueSensor(commandHandle, jointIndex,
                                                   enableSensor);
    statusHandle = b3SubmitClientCommandAndWaitStatus(sm, commandHandle);
    statusType = b3GetStatusType(statusHandle);
    if (statusType == CMD_CLIENT_COMMAND_COMPLETED) {
...
static PyObject* pybullet_createCollisionShape(PyObject* self, PyObject* args,
                                               PyObject* keywds) {
  int physicsClientId = 0;
  b3PhysicsClientHandle sm = 0;
  int shapeType = -1;
  ...
  static char* kwlist[] = {"shapeType",
                           ...
                           NULL};
  if (!PyArg_ParseTupleAndKeywords(
          args, keywds, "i|dOdsOOiOOi", kwlist, &shapeType, &radius,
          &halfExtentsObj, &height, &fileName, &meshScaleObj, &planeNormalObj,
          &flags, &collisionFramePositionObj, &collisionFrameOrientationObj,
          &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
  ...
  if (shapeType >= GEOM_SPHERE) {
    ...
    b3SharedMemoryCommandHandle commandHandle =
        b3CreateCollisionShapeCommandInit(sm);
    ...
    if (shapeType == GEOM_CAPSULE && radius > 0 && height >= 0) {
      shapeIndex =
          b3CreateCollisionShapeAddCapsule(commandHandle, radius, height);
    }
    if (shapeType == GEOM_CYLINDER && radius > 0 && height >= 0) {
      shapeIndex =
          b3CreateCollisionShapeAddCylinder(commandHandle, radius, height);
    }
    if (shapeType == GEOM_MESH && fileName) {
      pybullet_internalSetVectord(meshScaleObj, meshScale);
      shapeIndex =
          b3CreateCollisionShapeAddMesh(commandHandle, fileName, meshScale);
    }
    if (shapeType == GEOM_PLANE) {
      double planeConstant = 0;
      pybullet_internalSetVectord(planeNormalObj, planeNormal);
      shapeIndex = b3CreateCollisionShapeAddPlane(commandHandle, planeNormal,
                                                  planeConstant);
    }
    ...
    if (shapeIndex >= 0) {
      if (collisionFramePositionObj) {
        pybullet_internalSetVectord(collisionFramePositionObj,
                                    collisionFramePosition);
      }

      if (collisionFrameOrientationObj) {
        pybullet_internalSetVector4d(collisionFrameOrientationObj,
                                     collisionFrameOrientation);
      }
...
static PyObject* pybullet_createCollisionShapeArray(PyObject* self,
                                                    PyObject* args,
                                                    PyObject* keywds) {
  int physicsClientId = 0;
  b3PhysicsClientHandle sm = 0;
  ...
  static char* kwlist[] = {"shapeTypes",
                           ...
                           NULL};
  if (!PyArg_ParseTupleAndKeywords(
          args, keywds, "O|OOOOOOOOOi", kwlist, &shapeTypeArray, &radiusArray,
          &halfExtentsObjArray, &lengthArray, &fileNameArray,
          &meshScaleObjArray, &planeNormalObjArray, &flagsArray,
          &collisionFramePositionObjArray, &collisionFrameOrientationObjArray,
          &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
...

  {
    ...
    int s;
    PyObject* shapeTypeArraySeq =
        shapeTypeArray ? PySequence_Fast(shapeTypeArray,
                                         "expected a sequence of shape types")
                       : 0;
    PyObject* radiusArraySeq =
        radiusArray
            ? PySequence_Fast(radiusArray, "expected a sequence of radii")
            : 0;
    PyObject* halfExtentsArraySeq =
        halfExtentsObjArray
            ? PySequence_Fast(halfExtentsObjArray,
                              "expected a sequence of half extents")
            : 0;
    PyObject* lengthArraySeq =
        lengthArray
            ? PySequence_Fast(lengthArray, "expected a sequence of lengths")
            : 0;
    PyObject* fileNameArraySeq =
        fileNameArray
            ? PySequence_Fast(fileNameArray, "expected a sequence of filename")
            : 0;
    PyObject* meshScaleArraySeq =
        meshScaleObjArray ? PySequence_Fast(meshScaleObjArray,
                                            "expected a sequence of mesh scale")
                          : 0;
    PyObject* planeNormalArraySeq =
        planeNormalObjArray
            ? PySequence_Fast(planeNormalObjArray,
                              "expected a sequence of plane normal")
            : 0;
    PyObject* flagsArraySeq =
        flagsArray ? PySequence_Fast(flagsArray, "expected a sequence of flags")
                   : 0;
    PyObject* positionArraySeq =
        collisionFramePositionObjArray
            ? PySequence_Fast(
                  collisionFramePositionObjArray,
                  "expected a sequence of collision frame positions")
            : 0;
    PyObject* orientationArraySeq =
        collisionFrameOrientationObjArray
            ? PySequence_Fast(
                  collisionFrameOrientationObjArray,
                  "expected a sequence of collision frame orientations")
            : 0;
    ...
    numHalfExtents =
        halfExtentsArraySeq ? PySequence_Size(halfExtentsArraySeq) : 0;
    ...
    numPlaneNormals =
        planeNormalArraySeq ? PySequence_Size(planeNormalArraySeq) : 0;
    ...
    for (s = 0; s < numShapeTypes; s++) {
      ...
      if (shapeType >= GEOM_SPHERE) {
        ...
        if (shapeType == GEOM_SPHERE && s <= numRadius) {
          double radius =
              pybullet_internalGetFloatFromSequence(radiusArraySeq, s);
          if (radius > 0) {
            shapeIndex = b3CreateCollisionShapeAddSphere(commandHandle, radius);
          }
        }
        ...
        if (shapeType == GEOM_CAPSULE && s <= numRadius) {
          double radius =
              pybullet_internalGetFloatFromSequence(radiusArraySeq, s);
          double height =
              pybullet_internalGetFloatFromSequence(lengthArraySeq, s);
        ...
        if (shapeType == GEOM_MESH) {
          ...
          PyObject* meshScaleObj =
              meshScaleArraySeq ? PyList_GET_ITEM(meshScaleArraySeq, s) : 0;
          PyObject* fileNameObj =
              fileNameArraySeq ? PyList_GET_ITEM(fileNameArraySeq, s) : 0;
          const char* fileName = 0;
        ...
        if (shapeType == GEOM_PLANE) {
          PyObject* planeNormalObj =
              planeNormalArraySeq ? PyList_GET_ITEM(planeNormalArraySeq, s) : 0;
          double planeNormal[3];
          double planeConstant = 0;
          pybullet_internalSetVectord(planeNormalObj, planeNormal);
          shapeIndex = b3CreateCollisionShapeAddPlane(
              commandHandle, planeNormal, planeConstant);
        }
        if (flagsArraySeq) {
        ...
        if (positionArraySeq || orientationArraySeq) {
          PyObject* collisionFramePositionObj =
              positionArraySeq ? PyList_GET_ITEM(positionArraySeq, s) : 0;
          PyObject* collisionFrameOrientationObj =
              orientationArraySeq ? PyList_GET_ITEM(orientationArraySeq, s) : 0;
          double collisionFramePosition[3] = {0, 0, 0};
          double collisionFrameOrientation[4] = {0, 0, 0, 1};
          if (collisionFramePositionObj) {
            pybullet_internalSetVectord(collisionFramePositionObj,
    ...
    if (shapeTypeArraySeq) Py_DECREF(shapeTypeArraySeq);
    if (radiusArraySeq) Py_DECREF(radiusArraySeq);
    if (halfExtentsArraySeq) Py_DECREF(halfExtentsArraySeq);
    if (lengthArraySeq) Py_DECREF(lengthArraySeq);
    if (fileNameArraySeq) Py_DECREF(fileNameArraySeq);
    if (meshScaleArraySeq) Py_DECREF(meshScaleArraySeq);
    if (planeNormalArraySeq) Py_DECREF(planeNormalArraySeq);
    if (flagsArraySeq) Py_DECREF(flagsArraySeq);
    if (positionArraySeq) Py_DECREF(positionArraySeq);
    if (orientationArraySeq) Py_DECREF(orientationArraySeq);
...
static PyObject* pybullet_createVisualShape(PyObject* self, PyObject* args,
                                            PyObject* keywds) {
  int physicsClientId = 0;
  b3PhysicsClientHandle sm = 0;
  ...
  static char* kwlist[] = {"shapeType",
                           ...
                           NULL};
  if (!PyArg_ParseTupleAndKeywords(
          args, keywds, "i|dOdsOOiOOOOi", kwlist, &shapeType, &radius,
          &halfExtentsObj, &length, &fileName, &meshScaleObj, &planeNormalObj,
          &flags, &rgbaColorObj, &specularColorObj, &visualFramePositionObj,
          &visualFrameOrientationObj, &physicsClientId)) {
    return NULL;
  }
  ...
  if (shapeType >= GEOM_SPHERE) {
    ...
    int statusType;
    b3SharedMemoryCommandHandle commandHandle =
        b3CreateVisualShapeCommandInit(sm);
    int shapeIndex = -1;
    ...
    if (shapeType == GEOM_CYLINDER && radius > 0 && length >= 0) {
      shapeIndex =
          b3CreateVisualShapeAddCylinder(commandHandle, radius, length);
    }
    if (shapeType == GEOM_MESH && fileName) {
      pybullet_internalSetVectord(meshScaleObj, meshScale);
      shapeIndex =
          b3CreateVisualShapeAddMesh(commandHandle, fileName, meshScale);
    }
    if (shapeType == GEOM_PLANE) {
      double planeConstant = 0;
      pybullet_internalSetVectord(planeNormalObj, planeNormal);
      shapeIndex = b3CreateVisualShapeAddPlane(commandHandle, planeNormal,
                                               planeConstant);
    }
    if (shapeIndex >= 0 && flags) {
    ...
    if (shapeIndex >= 0) {
      ...
      b3CreateVisualShapeSetSpecularColor(commandHandle, shapeIndex,
                                          specularColor);

      if (visualFramePositionObj) {
        pybullet_internalSetVectord(visualFramePositionObj,
                                    visualFramePosition);
      }

      if (visualFrameOrientationObj) {
        pybullet_internalSetVector4d(visualFrameOrientationObj,
                                     visualFrameOrientation);
      }
      b3CreateVisualShapeSetChildTransform(commandHandle, shapeIndex,
                                           visualFramePosition,
...
static PyObject* pybullet_createMultiBody(PyObject* self, PyObject* args,
                                          PyObject* keywds) {
  int physicsClientId = 0;
  b3PhysicsClientHandle sm = 0;
  double baseMass = 0;
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, keywds, "|diiOOOOOOOOOOOOOOii", kwlist, &baseMass,
          &baseCollisionShapeIndex, &baseVisualShapeIndex, &basePosObj,
          &baseOrnObj, &baseInertialFramePositionObj,
          &baseInertialFrameOrientationObj, &linkMassesObj,
          &linkCollisionShapeIndicesObj, &linkVisualShapeIndicesObj,
          &linkPositionsObj, &linkOrientationsObj,
...

  {
    ...
    int numLinkCollisionShapes =
        linkCollisionShapeIndicesObj
            ? PySequence_Size(linkCollisionShapeIndicesObj)
            : 0;
    int numLinkVisualShapes = linkVisualShapeIndicesObj
                                  ? PySequence_Size(linkVisualShapeIndicesObj)
                                  : 0;
    int numLinkPositions =
        linkPositionsObj ? PySequence_Size(linkPositionsObj) : 0;
    int numLinkOrientations =
        linkOrientationsObj ? PySequence_Size(linkOrientationsObj) : 0;
    int numLinkParentIndices =
        linkParentIndicesObj ? PySequence_Size(linkParentIndicesObj) : 0;
    int numLinkJointTypes =
        linkJointTypesObj ? PySequence_Size(linkJointTypesObj) : 0;
    int numLinkJoinAxis =
        linkJointAxisObj ? PySequence_Size(linkJointAxisObj) : 0;
    int numLinkInertialFramePositions =
        linkInertialFramePositionObj
            ? PySequence_Size(linkInertialFramePositionObj)
            : 0;
    int numLinkInertialFrameOrientations =
        linkInertialFrameOrientationObj
            ? PySequence_Size(linkInertialFrameOrientationObj)
            : 0;

    PyObject* seqLinkMasses =
        linkMassesObj ? PySequence_Fast(linkMassesObj, "expected a sequence")
                      : 0;
    PyObject* seqLinkCollisionShapes =
        linkCollisionShapeIndicesObj
            ? PySequence_Fast(linkCollisionShapeIndicesObj,
                              "expected a sequence")
            : 0;
    PyObject* seqLinkVisualShapes =
        linkVisualShapeIndicesObj
            ? PySequence_Fast(linkVisualShapeIndicesObj, "expected a sequence")
            : 0;
    PyObject* seqLinkPositions =
        linkPositionsObj
            ? PySequence_Fast(linkPositionsObj, "expected a sequence")
            : 0;
    PyObject* seqLinkOrientations =
        linkOrientationsObj
            ? PySequence_Fast(linkOrientationsObj, "expected a sequence")
            : 0;
    PyObject* seqLinkParentIndices =
        linkParentIndicesObj
            ? PySequence_Fast(linkParentIndicesObj, "expected a sequence")
            : 0;
    PyObject* seqLinkJointTypes =
        linkJointTypesObj
            ? PySequence_Fast(linkJointTypesObj, "expected a sequence")
            : 0;
    PyObject* seqLinkJoinAxis =
        linkJointAxisObj
            ? PySequence_Fast(linkJointAxisObj, "expected a sequence")
            : 0;
    PyObject* seqLinkInertialFramePositions =
        linkInertialFramePositionObj
            ? PySequence_Fast(linkInertialFramePositionObj,
                              "expected a sequence")
            : 0;
    PyObject* seqLinkInertialFrameOrientations =
        linkInertialFrameOrientationObj
            ? PySequence_Fast(linkInertialFrameOrientationObj,
                              "expected a sequence")
            : 0;
    ...
    if ((numLinkMasses == numLinkCollisionShapes) &&
      ...
      int i;
      b3SharedMemoryCommandHandle commandHandle =
          b3CreateMultiBodyCommandInit(sm);
      ...
      pybullet_internalSetVectord(baseInertialFramePositionObj,
                                  baseInertialFramePosition);
      pybullet_internalSetVector4d(baseInertialFrameOrientationObj,
                                   baseInertialFrameOrientation);

      baseIndex = b3CreateMultiBodyBase(
          commandHandle, baseMass, baseCollisionShapeIndex,
          baseVisualShapeIndex, basePosition, baseOrientation,
          baseInertialFramePosition, baseInertialFrameOrientation);

      for (i = 0; i < numLinkMasses; i++) {
        double linkMass =
            pybullet_internalGetFloatFromSequence(seqLinkMasses, i);
        int linkCollisionShapeIndex =
            pybullet_internalGetIntFromSequence(seqLinkCollisionShapes, i);
        int linkVisualShapeIndex =
            pybullet_internalGetIntFromSequence(seqLinkVisualShapes, i);
        ...
        pybullet_internalGetVector3FromSequence(seqLinkInertialFramePositions,
                                                i, linkInertialFramePosition);
        pybullet_internalGetVector4FromSequence(
            linkInertialFrameOrientationObj, i, linkInertialFrameOrientation);
        pybullet_internalGetVector3FromSequence(seqLinkPositions, i,
                                                linkPosition);
        pybullet_internalGetVector4FromSequence(seqLinkOrientations, i,
                                                linkOrientation);
        pybullet_internalGetVector3FromSequence(seqLinkJoinAxis, i,
                                                linkJointAxis);
        linkParentIndex =
            pybullet_internalGetIntFromSequence(seqLinkParentIndices, i);
        linkJointType =
            pybullet_internalGetIntFromSequence(seqLinkJointTypes, i);

        b3CreateMultiBodyLink(commandHandle, linkMass, linkCollisionShapeIndex,
                              linkVisualShapeIndex, linkPosition,
                              linkOrientation, linkInertialFramePosition,
                              linkInertialFrameOrientation, linkParentIndex,
                              linkJointType, linkJointAxis);
      }

      if (seqLinkMasses) Py_DECREF(seqLinkMasses);
      if (seqLinkCollisionShapes) Py_DECREF(seqLinkCollisionShapes);
      if (seqLinkVisualShapes) Py_DECREF(seqLinkVisualShapes);
      if (seqLinkPositions) Py_DECREF(seqLinkPositions);
      if (seqLinkOrientations) Py_DECREF(seqLinkOrientations);
      if (seqLinkParentIndices) Py_DECREF(seqLinkParentIndices);
      if (seqLinkJointTypes) Py_DECREF(seqLinkJointTypes);
      if (seqLinkJoinAxis) Py_DECREF(seqLinkJoinAxis);
...
static PyObject* pybullet_createUserConstraint(PyObject* self, PyObject* args,
                                               PyObject* keywds) {
  b3SharedMemoryCommandHandle commandHandle;
  ...
  static char* kwlist[] = {
      ...
      "childFrameOrientation", "physicsClientId",    NULL};

  if (!PyArg_ParseTupleAndKeywords(
          args, keywds, "iiiiiOOO|OOi", kwlist, &parentBodyUniqueId,
          &parentLinkIndex, &childBodyUniqueId, &childLinkIndex, &jointType,
          &jointAxisObj, &parentFramePositionObj, &childFramePositionObj,
          &parentFrameOrientationObj, &childFrameOrientationObj,
          &physicsClientId)) {
    return NULL;
  }
  ...
  pybullet_internalSetVector4d(parentFrameOrientationObj,
  ...
  commandHandle = b3InitCreateUserConstraintCommand(
      sm, parentBodyUniqueId, parentLinkIndex, childBodyUniqueId,
      childLinkIndex, &jointInfo);
  statusHandle = b3SubmitClientCommandAndWaitStatus(sm, commandHandle);
  statusType = b3GetStatusType(statusHandle);
...
static PyObject* pybullet_getContactPointData(PyObject* self, PyObject* args,
  ...
  static char* kwlist[] = {"bodyA",      "bodyB",           "linkIndexA",
                           "linkIndexB", "physicsClientId", NULL};

  int physicsClientId = 0;
  b3PhysicsClientHandle sm = 0;
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "|iiiii", kwlist,
                                   &bodyUniqueIdA, &bodyUniqueIdB, &linkIndexA,
                                   &linkIndexB, &physicsClientId))
    return NULL;
...
/// Render an image from the current timestep of the simulation, width, height
/// are required, other args are optional
...
static PyObject* pybullet_getCameraImage(PyObject* self, PyObject* args,
                                         PyObject* keywds) {
  /// request an image from a simulated camera, using software or hardware
  /// renderer.
  ...
  // light color, light distance, shadow
  static char* kwlist[] = {"width",
                           "height",
                           "viewMatrix",
                           "projectionMatrix",
                           "lightDirection",
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, keywds, "ii|OOOOfifffiii", kwlist, &width, &height, &objViewMat,
          &objProjMat, &lightDirObj, &lightColorObj, &lightDist, &hasShadow,
          &lightAmbientCoeff, &lightDiffuseCoeff, &lightSpecularCoeff,
          &renderer, &flags, &physicsClientId)) {
    return NULL;
  }
  sm = getPhysicsClient(physicsClientId);
  if (sm == 0) {
  ...
  if (objViewMat && objProjMat &&
      pybullet_internalSetMatrix(objViewMat, viewMatrix) &&
    ...
    if (statusType == CMD_CAMERA_IMAGE_COMPLETED) {
      ...
      // TODO(hellojas): error handling if image size is 0
      {
        npy_intp rgb_dims[3] = {imageData.m_pixelHeight, imageData.m_pixelWidth,
                                bytesPerPixel};
        npy_intp dep_dims[2] = {imageData.m_pixelHeight,
                                imageData.m_pixelWidth};
        npy_intp seg_dims[2] = {imageData.m_pixelHeight,
                                imageData.m_pixelWidth};

        pyResultList = PyTuple_New(5);

        PyTuple_SetItem(pyResultList, 0,
                        PyInt_FromLong(imageData.m_pixelWidth));
        PyTuple_SetItem(pyResultList, 1,
                        PyInt_FromLong(imageData.m_pixelHeight));
        ...
        memcpy(
            PyArray_DATA(pyRGB), imageData.m_rgbColorData,
            imageData.m_pixelHeight * imageData.m_pixelWidth * bytesPerPixel);
        memcpy(
            PyArray_DATA(pyDep), imageData.m_depthValues,
...
static PyObject* pybullet_computeViewMatrix(PyObject* self, PyObject* args,
                                            PyObject* keywds) {
  ...
  static char* kwlist[] = {"cameraEyePosition", "cameraTargetPosition",
                           "cameraUpVector", "physicsClientId", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, keywds, "OOO|i", kwlist, &camEyeObj,
                                   &camTargetPositionObj, &camUpVectorObj,
                                   &physicsClientId)) {
    return NULL;
  }
  ...
  if (pybullet_internalSetVector(camEyeObj, camEye) &&
    ...
    PyObject* pyResultList = 0;
    int i;
    b3ComputeViewMatrixFromPositions(camEye, camTargetPosition, camUpVector,
                                     viewMatrix);
...
static PyObject* pybullet_computeViewMatrixFromYawPitchRoll(PyObject* self,
                                                            PyObject* args,
                                                            PyObject* keywds) {
  ...
  // set camera resolution, optionally view, projection matrix, light position
  static char* kwlist[] = {
      "cameraTargetPosition", "distance",        "yaw", "pitch", "roll",
      "upAxisIndex",          "physicsClientId", NULL};

  if (!PyArg_ParseTupleAndKeywords(
          args, keywds, "Offffi|i", kwlist, &cameraTargetPositionObj, &distance,
          &yaw, &pitch, &roll, &upAxisIndex, &physicsClientId)) {
    return NULL;
  }

  if (!pybullet_internalSetVector(cameraTargetPositionObj,
                                  cameraTargetPosition)) {
    PyErr_SetString(SpamError, "Cannot convert cameraTargetPosition.");
    return NULL;
  }

  b3ComputeViewMatrixFromYawPitchRoll(cameraTargetPosition, distance, yaw,
                                      pitch, roll, upAxisIndex, viewMatrix);
...
/// b3RequestCameraImageSetCameraMatrices
static PyObject* pybullet_computeProjectionMatrix(PyObject* self,
                                                  PyObject* args,
                                                  PyObject* keywds) {
  PyObject* pyResultList = 0;
  float left;
  ...
  // set camera resolution, optionally view, projection matrix, light position
  static char* kwlist[] = {"left",    "right",  "bottom",          "top",
                           "nearVal", "farVal", "physicsClientId", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, keywds, "ffffff|i", kwlist, &left,
                                   &right, &bottom, &top, &nearVal, &farVal,
                                   &physicsClientId)) {
    return NULL;
  }

  b3ComputeProjectionMatrix(left, right, bottom, top, nearVal, farVal,
                            projectionMatrix);
...
static PyObject* pybullet_computeProjectionMatrixFOV(PyObject* self,
                                                     PyObject* args,
                                                     PyObject* keywds) {
  ...
  static char* kwlist[] = {"fov",    "aspect",          "nearVal",
                           "farVal", "physicsClientId", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, keywds, "ffff|i", kwlist, &fov,
                                   &aspect, &nearVal, &farVal,
                                   &physicsClientId)) {
    return NULL;
  }
...
static PyObject* pybullet_applyExternalForce(PyObject* self, PyObject* args,
                                             PyObject* keywds) {
  {
    ...
    b3PhysicsClientHandle sm = 0;
    static char* kwlist[] = {
        "objectUniqueId", "linkIndex",       "forceObj", "posObj",
        "flags",          "physicsClientId", NULL};

    if (!PyArg_ParseTupleAndKeywords(args, keywds, "iiOOi|i", kwlist,
                                     &objectUniqueId, &linkIndex, &forceObj,
                                     &posObj, &flags, &physicsClientId)) {
      return NULL;
    }
    sm = getPhysicsClient(physicsClientId);
    if (sm == 0) {
...
static PyObject* pybullet_applyExternalTorque(PyObject* self, PyObject* args,
                                              PyObject* keywds) {
  {
    ...
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "iiOi|i", kwlist,
                                     &objectUniqueId, &linkIndex, &torqueObj,
                                     &flags, &physicsClientId)) {
      return NULL;
    }
    sm = getPhysicsClient(physicsClientId);
...
static PyObject* pybullet_getQuaternionFromEuler(PyObject* self, PyObject* args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "O|i", kwlist, &eulerObj,
                                   &physicsClientId)) {
    return NULL;
  }
...
static PyObject* pybullet_multiplyTransforms(PyObject* self, PyObject* args,
                                             PyObject* keywds) {
  PyObject* posAObj = 0;
  PyObject* ornAObj = 0;
  PyObject* posBObj = 0;
  ...
  static char* kwlist[] = {"positionA",    "orientationA",    "positionB",
                           "orientationB", "physicsClientId", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "OOOO|i", kwlist, &posAObj,
                                   &ornAObj, &posBObj, &ornBObj,
                                   &physicsClientId)) {
    return NULL;
  }
  ...
  PyErr_SetString(SpamError,
                  "Invalid input: expected positionA [x,y,z], orientationA "
                  "[x,y,z,w], positionB, orientationB.");
  return NULL;
}

static PyObject* pybullet_invertTransform(PyObject* self, PyObject* args,
                                          PyObject* keywds) {
  PyObject* posObj = 0;
  PyObject* ornObj = 0;
  double pos[3];
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "OO|i", kwlist, &posObj,
                                   &ornObj, &physicsClientId)) {
    return NULL;
  }
  ...
  PyErr_SetString(
      SpamError,
      "Invalid input: expected position [x,y,z] and orientation [x,y,z,w].");
  return NULL;
}
...
static PyObject* pybullet_loadPlugin(PyObject* self, PyObject* args,
                                     PyObject* keywds) {
  int physicsClientId = 0;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "s|si", kwlist, &pluginPath,
                                   &postFix, &physicsClientId)) {
    return NULL;
  }
...
static PyObject* pybullet_unloadPlugin(PyObject* self, PyObject* args,
                                       PyObject* keywds) {
  int physicsClientId = 0;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "i|i", kwlist, &pluginUniqueId,
                                   &physicsClientId)) {
    return NULL;
  }
...
static PyObject* pybullet_executePluginCommand(PyObject* self, PyObject* args,
                                               PyObject* keywds) {
  int physicsClientId = 0;
  int pluginUniqueId = -1;
  char* textArgument = 0;
  ...
  b3PhysicsClientHandle sm = 0;
  static char* kwlist[] = {"pluginUniqueId", "textArgument",    "intArgs",
                           "floatArgs",      "physicsClientId", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "i|sOOi", kwlist,
                                   &pluginUniqueId, &textArgument, &intArgs,
                                   &floatArgs, &physicsClientId)) {
    return NULL;
  }
  ...
  {
    PyObject* seqIntArgs =
        intArgs ? PySequence_Fast(intArgs, "expected a sequence") : 0;
    PyObject* seqFloatArgs =
        floatArgs ? PySequence_Fast(floatArgs, "expected a sequence") : 0;
...

{
  ...
  static char* kwlist[] = {"bodyUniqueId",    "endEffectorLinkIndex",
                           ...
                           "physicsClientId", NULL};
  if (!PyArg_ParseTupleAndKeywords(
          args, keywds, "iiO|OOOOOOii", kwlist, &bodyUniqueId,
          &endEffectorLinkIndex, &targetPosObj, &targetOrnObj, &lowerLimitsObj,
          &upperLimitsObj, &jointRangesObj, &restPosesObj, &jointDampingObj,
          &solver, &physicsClientId)) {
    // backward compatibility bodyIndex -> bodyUniqueId. don't update keywords,
    // people need to migrate to bodyUniqueId version
    static char* kwlist2[] = {"bodyIndex",
    ...
    PyErr_Clear();
    if (!PyArg_ParseTupleAndKeywords(
            args, keywds, "iiO|OOOOOOi", kwlist2, &bodyUniqueId,
            &endEffectorLinkIndex, &targetPosObj, &targetOrnObj,
            &lowerLimitsObj, &upperLimitsObj, &jointRangesObj, &restPosesObj,
  ...
  }
  {
    ...
    if (numJoints && (szLowerLimits == numJoints) &&
        (szUpperLimits == numJoints) && (szJointRanges == numJoints) &&
        (szRestPoses == numJoints)) {
    ...
    if (szJointDamping > 0) {
      ...
      if (szJointDamping < numJoints) {
        PyErr_SetString(SpamError,
                        "calculateInverseKinematics the size of input joint "
                        "damping values is smaller than the number of joints.");
        return NULL;
      } else {
        ...
        for (i = 0; i < szJointDamping; i++) {
          jointDamping[i] =
              pybullet_internalGetFloatFromSequence(jointDampingObj, i);
        }
        hasJointDamping = 1;
      }
    }
    ...
    if (hasPos) {
      ...
      if (hasNullSpace) {
        if (hasOrn) {
          b3CalculateInverseKinematicsPosOrnWithNullSpaceVel(
              command, numJoints, endEffectorLinkIndex, pos, ori, lowerLimits,
              upperLimits, jointRanges, restPoses);
        } else {
          b3CalculateInverseKinematicsPosWithNullSpaceVel(
              command, numJoints, endEffectorLinkIndex, pos, lowerLimits,
              upperLimits, jointRanges, restPoses);
        }
      } else {
        if (hasOrn) {
          b3CalculateInverseKinematicsAddTargetPositionWithOrientation(
              command, endEffectorLinkIndex, pos, ori);
        } else {
          b3CalculateInverseKinematicsAddTargetPurePosition(
              command, endEffectorLinkIndex, pos);
        }
      }

      if (hasJointDamping) {
        b3CalculateInverseKinematicsSetJointDamping(command, numJoints,
                                                    jointDamping);
      }
      free(jointDamping);
      ...
      result = b3GetStatusInverseKinematicsJointPositions(
          statusHandle, &resultBodyIndex, &numPos, 0);
      if (result && numPos) {
        int i;
        PyObject* pylist;
        double* ikOutPutJointPos = (double*)malloc(numPos * sizeof(double));
        result = b3GetStatusInverseKinematicsJointPositions(
            statusHandle, &resultBodyIndex, &numPos, ikOutPutJointPos);
        pylist = PyTuple_New(numPos);
        for (i = 0; i < numPos; i++) {
          PyTuple_SetItem(pylist, i, PyFloat_FromDouble(ikOutPutJointPos[i]));
        }

        free(ikOutPutJointPos);
        return pylist;
      } else {
        PyErr_SetString(SpamError, "Error in calculateInverseKinematics");
        return NULL;
      }
    } else {
      PyErr_SetString(
...
static PyObject* pybullet_calculateInverseDynamics(PyObject* self,
                                                   ...
                                                   PyObject* keywds) {
  {
    ...
    static char* kwlist[] = {"bodyUniqueId",    "objPositions",
                             ...
                             "physicsClientId", NULL};
    if (!PyArg_ParseTupleAndKeywords(
            args, keywds, "iOOO|i", kwlist, &bodyUniqueId, &objPositionsQ,
      ...
      PyErr_Clear();
      if (!PyArg_ParseTupleAndKeywords(
              args, keywds, "iOOO|i", kwlist2, &bodyUniqueId, &objPositionsQ,
              &objVelocitiesQdot, &objAccelerations, &physicsClientId)) {
        return NULL;
      }
    }
...

    {
      ...
      for (j = 0; j < nj; j++) {
        ...
        switch (info.m_jointType) {
          ...
          default: {
            // fixed joint has 0-dof and at the moment, we don't deal with
...
static PyObject* pybullet_calculateJacobian(PyObject* self, PyObject* args,
                                            PyObject* keywds) {
  {
    int bodyUniqueId;
    int linkIndex;
    ...
    static char* kwlist[] = {
        ...
        "objVelocities", "objAccelerations", "physicsClientId", NULL};
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "iiOOOO|i", kwlist,
                                     &bodyUniqueId, &linkIndex, &localPosition,
        ...
        }
        {
          ...
          b3SharedMemoryCommandHandle commandHandle =
              b3CalculateJacobianCommandInit(
                  sm, bodyUniqueId, linkIndex, localPoint, jointPositions,
          ...
          if (statusType == CMD_CALCULATED_JACOBIAN_COMPLETED) {
            ...
            if (dofCount) {
              ...
              b3GetStatusJacobian(statusHandle, NULL, linearJacobian,
                                  angularJacobian);
              if (linearJacobian) {
                int r;
                ...
                for (r = 0; r < 3; ++r) {
                  ...
                  for (c = 0; c < dofCount; ++c) {
                    ...
                    PyTuple_SetItem(
                        pyrow, c, PyFloat_FromDouble(linearJacobian[element]));
                  }
              ...
              if (angularJacobian) {
                ...
                for (r = 0; r < 3; ++r) {
                  ...
                  for (c = 0; c < dofCount; ++c) {
                    int element = r * dofCount + c;
                    PyTuple_SetItem(
                        pyrow, c, PyFloat_FromDouble(angularJacobian[element]));
                  }
                  PyTuple_SetItem(pymat, r, pyrow);
                }
            ...
            PyErr_SetString(SpamError, "Internal error in calculateJacobian");
          }
        }
        free(localPoint);
        free(jointPositions);
...
/// Given an object id, joint positions, joint velocities and joint
/// accelerations, compute the Jacobian
static PyObject* pybullet_calculateMassMatrix(PyObject* self, PyObject* args,
                                              PyObject* keywds) {
  {
    int bodyUniqueId;
    PyObject* objPositions;
    int physicsClientId = 0;
    b3PhysicsClientHandle sm = 0;
    static char* kwlist[] = {"bodyUniqueId", "objPositions", "physicsClientId",
                             NULL};
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "iO|i", kwlist,
                                     &bodyUniqueId, &objPositions,
                                     &physicsClientId)) {
      return NULL;
    }
    sm = getPhysicsClient(physicsClientId);
...

    {
      ...
      if (numJoints && (szObPos == numJoints)) {
        ...
        }
        {
          ...
          int statusType;
          b3SharedMemoryCommandHandle commandHandle =
              b3CalculateMassMatrixCommandInit(sm, bodyUniqueId,
                                               jointPositions);
          statusHandle = b3SubmitClientCommandAndWaitStatus(sm, commandHandle);
          statusType = b3GetStatusType(statusHandle);
...
static PyMethodDef SpamMethods[] = {
    ...
    {"setDefaultContactERP", (PyCFunction)pybullet_setDefaultContactERP,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"setRealTimeSimulation", (PyCFunction)pybullet_setRealTimeSimulation,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"setInternalSimFlags", (PyCFunction)pybullet_setInternalSimFlags,
     METH_VARARGS | METH_KEYWORDS,
     "This is for experimental purposes, use at own risk, magic may or not "
     "happen"},

    {"loadURDF", (PyCFunction)pybullet_loadURDF, METH_VARARGS | METH_KEYWORDS,
     "bodyUniqueId = loadURDF(fileName, basePosition=[0.,0.,0.], "
     "baseOrientation=[0.,0.,0.,1.], "
     "useMaximalCoordinates=0, useFixedBase=0, flags=0, globalScaling=1.0, "
     "physicsClientId=0)\n"
...
#ifndef SKIP_SOFT_BODY_MULTI_BODY_DYNAMICS_WORLD
    {"loadSoftBody", (PyCFunction)pybullet_loadSoftBody,
     METH_VARARGS | METH_KEYWORDS, "Load a softbody from an obj file."},
#endif
    {"loadBullet", (PyCFunction)pybullet_loadBullet,
     METH_VARARGS | METH_KEYWORDS, "Load a world from a .bullet file."},

    {"saveBullet", (PyCFunction)pybullet_saveBullet,
     METH_VARARGS | METH_KEYWORDS,
     "Save the full state of the world to a .bullet file."},

    {"restoreState", (PyCFunction)pybullet_restoreState,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"createCollisionShape", (PyCFunction)pybullet_createCollisionShape,
     METH_VARARGS | METH_KEYWORDS,
     "Create a collision shape. Returns a non-negative (int) unique id, if "
     "successfull, negative otherwise."},
    ...
    {"createVisualShape", (PyCFunction)pybullet_createVisualShape,
     METH_VARARGS | METH_KEYWORDS,
     "Create a visual shape. Returns a non-negative (int) unique id, if "
     "successfull, negative otherwise."},

    {"createMultiBody", (PyCFunction)pybullet_createMultiBody,
     METH_VARARGS | METH_KEYWORDS,
     "Create a multi body. Returns a non-negative (int) unique id, if "
     "successfull, negative otherwise."},

    {"createConstraint", (PyCFunction)pybullet_createUserConstraint,
     METH_VARARGS | METH_KEYWORDS,
     "Create a constraint between two bodies. Returns a (int) unique id, if "
     "successfull."},

    {"changeConstraint", (PyCFunction)pybullet_changeUserConstraint,
     METH_VARARGS | METH_KEYWORDS,
     "Change some parameters of an existing constraint, such as the child "
     "pivot or child frame orientation, using its unique id."},

    {"removeConstraint", (PyCFunction)pybullet_removeUserConstraint,
     METH_VARARGS | METH_KEYWORDS, "Remove a constraint using its unique id."},
    ...
    {"getNumBodies", (PyCFunction)pybullet_getNumBodies,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"getBodyInfo", (PyCFunction)pybullet_getBodyInfo,
     METH_VARARGS | METH_KEYWORDS,
     "Get the body info, given a body unique id."},

    {"removeBody", (PyCFunction)pybullet_removeBody,
     METH_VARARGS | METH_KEYWORDS, "Remove a body by its body unique id."},

    {"getNumConstraints", (PyCFunction)pybullet_getNumConstraints,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"getConstraintState", (PyCFunction)pybullet_getConstraintState,
     METH_VARARGS | METH_KEYWORDS,
     "Get the user-created constraint state (applied forces), given a "
     "constraint unique id."},

    {"getConstraintUniqueId", (PyCFunction)pybullet_getConstraintUniqueId,
     METH_VARARGS | METH_KEYWORDS,
     "Get the unique id of the constraint, given a integer index in range [0.. "
    ...
    {"getAABB", (PyCFunction)pybullet_getAABB, METH_VARARGS | METH_KEYWORDS,
    ...
    {"resetBaseVelocity", (PyCFunction)pybullet_resetBaseVelocity,
     METH_VARARGS | METH_KEYWORDS,
     "Reset the linear and/or angular velocity of the base of the object "
     " in world space coordinates. "
     "linearVelocity (x,y,z) and angularVelocity (x,y,z)."},

    {"getNumJoints", (PyCFunction)pybullet_getNumJoints,
     METH_VARARGS | METH_KEYWORDS, "Get the number of joints for an object."},

    {"getJointInfo", (PyCFunction)pybullet_getJointInfo,
     METH_VARARGS | METH_KEYWORDS,
     "Get the name and type info for a joint on a body."},

    {"getJointState", (PyCFunction)pybullet_getJointState,
     METH_VARARGS | METH_KEYWORDS,
     "Get the state (position, velocity etc) for a joint on a body."},

    {"getJointStates", (PyCFunction)pybullet_getJointStates,
     METH_VARARGS | METH_KEYWORDS,
     "Get the state (position, velocity etc) for multiple joints on a body."},

    {"getLinkState", (PyCFunction)pybullet_getLinkState,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"resetJointState", (PyCFunction)pybullet_resetJointState,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"setJointMotorControl2", (PyCFunction)pybullet_setJointMotorControl2,
     METH_VARARGS | METH_KEYWORDS,
     "Set a single joint motor control mode and desired target value. There is "
     "no immediate state change, stepSimulation will process the motors."},
    ...
    {"setJointMotorControlArray",
     ...
     "and optional targetPositions, "
     "targetVelocities, forces, kds and kps as lists"
     "Using setJointMotorControlArray has the benefit of lower calling "
     "overhead."},

    {"applyExternalForce", (PyCFunction)pybullet_applyExternalForce,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"applyExternalTorque", (PyCFunction)pybullet_applyExternalTorque,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"renderImage", pybullet_renderImageObsolete, METH_VARARGS,
     "obsolete, please use getCameraImage and getViewProjectionMatrices "
     "instead"},

    {"getCameraImage", (PyCFunction)pybullet_getCameraImage,
     METH_VARARGS | METH_KEYWORDS,
     "Render an image (given the pixel resolution width, height, camera "
     "viewMatrix "
     ", projectionMatrix, lightDirection, lightColor, lightDistance, shadow, "
     "lightAmbientCoeff, lightDiffuseCoeff, lightSpecularCoeff, and renderer), "
     "and return the "
    ...
    {"computeViewMatrix", (PyCFunction)pybullet_computeViewMatrix,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"computeProjectionMatrix", (PyCFunction)pybullet_computeProjectionMatrix,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"getContactPoints", (PyCFunction)pybullet_getContactPointData,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"getClosestPoints", (PyCFunction)pybullet_getClosestPointData,
     METH_VARARGS | METH_KEYWORDS,
     "Compute the closest points between two objects, if the distance is below "
     "a given threshold."
     "Input is two objects unique ids and distance threshold."},

    {"getOverlappingObjects", (PyCFunction)pybullet_getOverlappingObjects,
     METH_VARARGS | METH_KEYWORDS,
     "Return all the objects that have overlap with a given "
     "axis-aligned bounding box volume (AABB)."
     "Input are two vectors defining the AABB in world space "
     "[min_x,min_y,min_z],[max_x,max_y,max_z]."},

    {"addUserDebugLine", (PyCFunction)pybullet_addUserDebugLine,
     METH_VARARGS | METH_KEYWORDS,
     "Add a user debug draw line with lineFrom[3], lineTo[3], lineColorRGB[3], "
     "lineWidth, lifeTime. "
     "A lifeTime of 0 means permanent until removed. Returns a unique id for "
     "the user debug item."},

    {"addUserDebugText", (PyCFunction)pybullet_addUserDebugText,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"addUserDebugParameter", (PyCFunction)pybullet_addUserDebugParameter,
     METH_VARARGS | METH_KEYWORDS,
     "Add a user debug parameter, such as a slider, that can be controlled "
     "using a GUI."},
    {"readUserDebugParameter", (PyCFunction)pybullet_readUserDebugParameter,
     METH_VARARGS | METH_KEYWORDS,
     "Read the current value of a user debug parameter, given the user debug "
     "item unique id."},

    {"removeUserDebugItem", (PyCFunction)pybullet_removeUserDebugItem,
     METH_VARARGS | METH_KEYWORDS,
     "remove a user debug draw item, giving its unique id"},

    {"removeAllUserDebugItems", (PyCFunction)pybullet_removeAllUserDebugItems,
     METH_VARARGS | METH_KEYWORDS, "remove all user debug draw items"},

    {"setDebugObjectColor", (PyCFunction)pybullet_setDebugObjectColor,
     METH_VARARGS | METH_KEYWORDS,
     "Override the wireframe debug drawing color for a particular object "
     "unique id / link index."
     "If you ommit the color, the custom color will be removed."},

    {"getDebugVisualizerCamera", (PyCFunction)pybullet_getDebugVisualizerCamera,
     METH_VARARGS | METH_KEYWORDS,
     "Get information about the 3D visualizer camera, such as width, height, "
     "view matrix, projection matrix etc."},

    {"configureDebugVisualizer", (PyCFunction)pybullet_configureDebugVisualizer,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"getVisualShapeData", (PyCFunction)pybullet_getVisualShapeData,
     METH_VARARGS | METH_KEYWORDS,
     "Return the visual shape information for one object."},

    {"getCollisionShapeData", (PyCFunction)pybullet_getCollisionShapeData,
     METH_VARARGS | METH_KEYWORDS,
     "Return the collision shape information for one object."},

    {"changeVisualShape", (PyCFunction)pybullet_changeVisualShape,
     METH_VARARGS | METH_KEYWORDS,
     "Change part of the visual shape information for one object."},

    {"resetVisualShapeData", (PyCFunction)pybullet_changeVisualShape,
     METH_VARARGS | METH_KEYWORDS,
     "Obsolete method, kept for backward compatibility, use "
     "changeVisualShapeData instead."},

    {"loadTexture", (PyCFunction)pybullet_loadTexture,
     METH_VARARGS | METH_KEYWORDS, "Load texture file."},

    {"changeTexture", (PyCFunction)pybullet_changeTexture,
     METH_VARARGS | METH_KEYWORDS, "Change a texture file."},

    {"getQuaternionFromEuler", (PyCFunction)pybullet_getQuaternionFromEuler,
     METH_VARARGS | METH_KEYWORDS,
     "Convert Euler [roll, pitch, yaw] as in URDF/SDF convention, to "
     "quaternion [x,y,z,w]"},

    {"getEulerFromQuaternion", (PyCFunction)pybullet_getEulerFromQuaternion,
     METH_VARARGS | METH_KEYWORDS,
     "Convert quaternion [x,y,z,w] to Euler [roll, pitch, yaw] as in URDF/SDF "
     "convention"},

    {"multiplyTransforms", (PyCFunction)pybullet_multiplyTransforms,
     METH_VARARGS | METH_KEYWORDS,
     "Multiply two transform, provided as [position], [quaternion]."},

    {"invertTransform", (PyCFunction)pybullet_invertTransform,
     METH_VARARGS | METH_KEYWORDS,
     "Invert a transform, provided as [position], [quaternion]."},

    {"getMatrixFromQuaternion", (PyCFunction)pybullet_getMatrixFromQuaternion,
     METH_VARARGS | METH_KEYWORDS,
     "Compute the 3x3 matrix from a quaternion, as a list of 9 values "
     "(row-major)"},

    {"calculateInverseDynamics", (PyCFunction)pybullet_calculateInverseDynamics,
     METH_VARARGS | METH_KEYWORDS,
     "Given an object id, joint positions, joint velocities and joint "
     "accelerations, compute the joint forces using Inverse Dynamics"},

    {"calculateJacobian", (PyCFunction)pybullet_calculateJacobian,
     METH_VARARGS | METH_KEYWORDS,
     "linearJacobian, angularJacobian = calculateJacobian(bodyUniqueId, "
     "linkIndex, localPosition, objPositions, objVelocities, objAccelerations, "
     "physicsClientId=0)\n"
    ...
    {"calculateMassMatrix", (PyCFunction)pybullet_calculateMassMatrix,
     METH_VARARGS | METH_KEYWORDS,
     "massMatrix = calculateMassMatrix(bodyUniqueId, objPositions, "
     "physicsClientId=0)\n"
    ...
    {"getVREvents", (PyCFunction)pybullet_getVREvents,
     METH_VARARGS | METH_KEYWORDS,
     "Get Virtual Reality events, for example to track VR controllers "
     "position/buttons"},
    {"setVRCameraState", (PyCFunction)pybullet_setVRCameraState,
     METH_VARARGS | METH_KEYWORDS,
     ...
     "example)."},
    {"getKeyboardEvents", (PyCFunction)pybullet_getKeyboardEvents,
     METH_VARARGS | METH_KEYWORDS,
     "Get keyboard events, keycode and state (KEY_IS_DOWN, KEY_WAS_TRIGGERED, "
     "KEY_WAS_RELEASED)"},

    {"getMouseEvents", (PyCFunction)pybullet_getMouseEvents,
     METH_VARARGS | METH_KEYWORDS,
     "Get mouse events, event type and button state (KEY_IS_DOWN, "
     "KEY_WAS_TRIGGERED, KEY_WAS_RELEASED)"},

    {"startStateLogging", (PyCFunction)pybullet_startStateLogging,
     METH_VARARGS | METH_KEYWORDS,
     "Start logging of state, such as robot base position, orientation, joint "
     "positions etc. "
     "Specify loggingType (STATE_LOGGING_MINITAUR, "
     "STATE_LOGGING_GENERIC_ROBOT, STATE_LOGGING_VR_CONTROLLERS, "
     "STATE_LOGGING_CONTACT_POINTS, etc), "
     "fileName, optional objectUniqueId, maxLogDof, bodyUniqueIdA, "
     "bodyUniqueIdB, linkIndexA, linkIndexB. Function returns int "
     "loggingUniqueId"},
    {"stopStateLogging", (PyCFunction)pybullet_stopStateLogging,
     METH_VARARGS | METH_KEYWORDS,
     "Stop logging of robot state, given a loggingUniqueId."},
    {"rayTest", (PyCFunction)pybullet_rayTestObsolete,
     METH_VARARGS | METH_KEYWORDS,
     "Cast a ray and return the first object hit, if any. "
     "Takes two arguments (from_position [x,y,z] and to_position [x,y,z] in "
     "Cartesian world coordinates"},

    {"rayTestBatch", (PyCFunction)pybullet_rayTestBatch,
     METH_VARARGS | METH_KEYWORDS,
     "Cast a batch of rays and return the result for each of the rays (first "
     "object hit, if any. or -1) "
     "Takes two arguments (list of from_positions [x,y,z] and a list of "
     "to_positions [x,y,z] in Cartesian world coordinates"},

    {"loadPlugin", (PyCFunction)pybullet_loadPlugin,
     METH_VARARGS | METH_KEYWORDS,
     "Load a plugin, could implement custom commands etc."},

    {"unloadPlugin", (PyCFunction)pybullet_unloadPlugin,
     METH_VARARGS | METH_KEYWORDS,
     "Unload a plugin, given the pluginUniqueId."},

    {"executePluginCommand", (PyCFunction)pybullet_executePluginCommand,
     METH_VARARGS | METH_KEYWORDS,
     "Execute a command, implemented in a plugin."},

    {"submitProfileTiming", (PyCFunction)pybullet_submitProfileTiming,
     METH_VARARGS | METH_KEYWORDS,
     "Add a custom profile timing that will be visible in performance profile "
     "recordings on the physics server."
     "On the physics server (in GUI and VR mode) you can press 'p' to start "
     "and/or stop profile recordings"},

    {"setTimeOut", (PyCFunction)pybullet_setTimeOut,
     METH_VARARGS | METH_KEYWORDS,
...
#endif
{
  ...
  PyModule_AddIntConstant(m, "JOINT_POINT2POINT",
                          ePoint2PointType);            // user read
  PyModule_AddIntConstant(m, "JOINT_GEAR", eGearType);  // user read

  PyModule_AddIntConstant(m, "SENSOR_FORCE_TORQUE",
                          eSensorForceTorqueType);  // user read
  ...
  PyModule_AddIntConstant(m, "CONTACT_REPORT_EXISTING",
                          CONTACT_QUERY_MODE_REPORT_EXISTING_CONTACT_POINTS);
  PyModule_AddIntConstant(m, "CONTACT_RECOMPUTE_CLOSEST",
  ...
  PyModule_AddIntConstant(m, "VR_DEVICE_GENERIC_TRACKER",
                          VR_DEVICE_GENERIC_TRACKER);

  PyModule_AddIntConstant(m, "VR_CAMERA_TRACK_OBJECT_ORIENTATION",
  ...
  PyModule_AddIntConstant(m, "STATE_LOGGING_GENERIC_ROBOT",
                          STATE_LOGGING_GENERIC_ROBOT);
  PyModule_AddIntConstant(m, "STATE_LOGGING_VR_CONTROLLERS",
                          STATE_LOGGING_VR_CONTROLLERS);
  PyModule_AddIntConstant(m, "STATE_LOGGING_VIDEO_MP4",
                          STATE_LOGGING_VIDEO_MP4);
  PyModule_AddIntConstant(m, "STATE_LOGGING_CONTACT_POINTS",
                          STATE_LOGGING_CONTACT_POINTS);
  PyModule_AddIntConstant(m, "STATE_LOGGING_PROFILE_TIMINGS",
  ...
  PyModule_AddIntConstant(m, "COV_ENABLE_VR_TELEPORTING",
                          COV_ENABLE_VR_TELEPORTING);
  PyModule_AddIntConstant(m, "COV_ENABLE_RENDERING", COV_ENABLE_RENDERING);
  PyModule_AddIntConstant(m, "COV_ENABLE_TINY_RENDERER",
                          COV_ENABLE_TINY_RENDERER);
  PyModule_AddIntConstant(m, "COV_ENABLE_Y_AXIS_UP", COV_ENABLE_Y_AXIS_UP);

  PyModule_AddIntConstant(m, "COV_ENABLE_VR_RENDER_CONTROLLERS",
                          COV_ENABLE_VR_RENDER_CONTROLLERS);
  PyModule_AddIntConstant(m, "COV_ENABLE_KEYBOARD_SHORTCUTS",
                          COV_ENABLE_KEYBOARD_SHORTCUTS);
  PyModule_AddIntConstant(m, "COV_ENABLE_MOUSE_PICKING",
                          COV_ENABLE_MOUSE_PICKING);

  PyModule_AddIntConstant(m, "COV_ENABLE_RGB_BUFFER_PREVIEW",
                          COV_ENABLE_RGB_BUFFER_PREVIEW);
  PyModule_AddIntConstant(m, "COV_ENABLE_DEPTH_BUFFER_PREVIEW",
                          COV_ENABLE_DEPTH_BUFFER_PREVIEW);
  PyModule_AddIntConstant(m, "COV_ENABLE_SEGMENTATION_MARK_PREVIEW",
                          COV_ENABLE_SEGMENTATION_MARK_PREVIEW);

  PyModule_AddIntConstant(m, "ER_TINY_RENDERER", ER_TINY_RENDERER);
  PyModule_AddIntConstant(m, "ER_BULLET_HARDWARE_OPENGL",
                          ER_BULLET_HARDWARE_OPENGL);
  PyModule_AddIntConstant(m, "ER_SEGMENTATION_MASK_OBJECT_AND_LINKINDEX",
  ...
  PyModule_AddIntConstant(m, "IK_HAS_TARGET_ORIENTATION",
                          IK_HAS_TARGET_ORIENTATION);
  PyModule_AddIntConstant(m, "IK_HAS_NULL_SPACE_VELOCITY",
                          IK_HAS_NULL_SPACE_VELOCITY);
  PyModule_AddIntConstant(m, "IK_HAS_JOINT_DAMPING", IK_HAS_JOINT_DAMPING);

  PyModule_AddIntConstant(m, "URDF_USE_INERTIA_FROM_FILE",
                          URDF_USE_INERTIA_FROM_FILE);
  PyModule_AddIntConstant(m, "URDF_USE_IMPLICIT_CYLINDER",
                          URDF_USE_IMPLICIT_CYLINDER);
  PyModule_AddIntConstant(m, "URDF_GLOBAL_VELOCITIES_MB",
                          URDF_GLOBAL_VELOCITIES_MB);
  PyModule_AddIntConstant(m, "MJCF_COLORS_FROM_FILE", MJCF_COLORS_FROM_FILE);

  PyModule_AddIntConstant(m, "URDF_USE_SELF_COLLISION",
                          URDF_USE_SELF_COLLISION);
  PyModule_AddIntConstant(m, "URDF_USE_SELF_COLLISION_EXCLUDE_PARENT",
                          URDF_USE_SELF_COLLISION_EXCLUDE_PARENT);
  PyModule_AddIntConstant(m, "URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS",
                          URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS);

  PyModule_AddIntConstant(m, "MAX_RAY_INTERSECTION_BATCH_SIZE",
  ...
  PyModule_AddIntConstant(m, "GEOM_FORCE_CONCAVE_TRIMESH",
                          GEOM_FORCE_CONCAVE_TRIMESH);

  PyModule_AddIntConstant(m, "STATE_LOG_JOINT_MOTOR_TORQUES",
                          STATE_LOG_JOINT_MOTOR_TORQUES);
  PyModule_AddIntConstant(m, "STATE_LOG_JOINT_USER_TORQUES",
                          STATE_LOG_JOINT_USER_TORQUES);
  PyModule_AddIntConstant(
      m, "STATE_LOG_JOINT_TORQUES",
      STATE_LOG_JOINT_USER_TORQUES + STATE_LOG_JOINT_MOTOR_TORQUES);
