...
struct ImplicitClothExample : public CommonExampleInterface {
 ...
 public:
  ImplicitClothExample(struct GUIHelperInterface* helper, int option)
      : m_guiHelper(helper), m_option(option), m_cloth(0) {}
  virtual void initPhysics();
  virtual void exitPhysics();
  virtual void stepSimulation(float deltaTime);
  virtual void renderScene();
  virtual void physicsDebugDraw(
      int debugFlags);  // for now we reuse the flags in
...
void ImplicitClothExample::stepSimulation(float deltaTime) {
  ...
  m_cloth->cloth_gravity.y =
      -9.8;  //-9.8;//-9.8;//-9.8;//0;//-9.8;//0;//-9.8;//0;//-9.8;
...
void ImplicitClothExample::physicsDebugDraw(int debugFlags) {
  ...
  renderer->drawLines(&m_cloth->X[0].x, lineColor, total_points, sizeof(float3),
...
class CommonExampleInterface* ImplicitClothCreateFunc(
