...
int ClipPolyPoly(const float3 &normal, const float3 *clipper, int clipper_count,
  ...
  // work anyways. the first argument (normal) is the normal of polygon clipper.
...
float3x3 ShapeInertiaContrib(const float3 &cor, const float3 &position,
  ...
  // one. essentially returns the contribution of a subshape to the inertia of a
  // larger rigid body typical usage:
  ...
  // that is static, in which one easy way to implement this is to lock the
  // center of rotation and adjust the inertia accordingly. position &
  // orientation - is the current pose or transform of the shape. Obviously
  // position, orientation and cor are all described wrt the same reference
  // frame. shape_com and shape_inertia are the center of mass and the inertia
  // of the shape in the local coordinate system of that shape. To clarify, if a
  // shape happened to be located somewhere else then position or orientation
  // would be different, but com and inertia would be the same.
