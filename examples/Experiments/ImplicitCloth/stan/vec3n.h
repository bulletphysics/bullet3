...
// and excessing copying. Instead of a full Template Metaprogramming approach,
// we explicitly write classes to specifically handle all the expressions we are
// likely to use. Most applicable lines of code will be of the same handful of
// basic forms, but with different parameters for the operands. In the future,
// if we ever need a longer expression with more operands, then we will just add
// whatever additional building blocks that are necessary - not a big deal. This
// approach is much simpler to develop, debug and optimize (restrict keyword,
// simd etc) than template metaprogramming is.  We do not rely on the
// implementation of a particular compiler to be able to expand extensive nested
// inline codes. Additionally, we reliably get our optimizations even within a
// debug build. Therefore we believe that our Optimized Expressions are a good
// compromise that give us the best of both worlds. The code within those
// important algorithms, which use this library, can now remain clean and
// readable yet still execute quickly.
//
...
// _mm_malloc(n,64); } template <class T> void vec4<T>::operator delete[](void
// *a) { _mm_free(a); }
