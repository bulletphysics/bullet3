 ...
 public:
  class Spring {
   public:
    ...
    Spring() {}
    Spring(int _type, int _a, int _b, float _restlen)
        : type(_type), a(_a), b(_b), restlen(_restlen) {
      iab = iba = -1;
    }
  };
  ...
  float3Nx3N S;     // used for our constraints - contains only some diagonal
                    // blocks as needed S[i,i]
  int awake;
  float3 bmin, bmax;
  union {
    struct {
  ...
  Spring &CreateSpring(int type, int a, int b, float restlen) {
    return AddBlocks(springs.Add(Spring(type, a, b, restlen)));
  }
  Spring &CreateSpring(int type, int a, int b) {
    return CreateSpring(type, a, b, magnitude(X[b] - X[a]));
  }
