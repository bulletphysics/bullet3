...
// as is reasonable. However, if you choose to use any of this code, then you
// agree that I assume no financial liability should the software not meet your
// expectations. But do feel free to send any feedback.
//
// The core backward integration functionality has all been extracted into the
// SpringNetwork class. This makes it easy for you if you just want to look at
// or use the math and the algorithms. The remainder of the code builds a cloth
// system with basic render support, I/O, and manipulators, so its possible to
// make use of the technology within a 3D application. This code is separated
// from the SpringNetwork class in order to avoid pushing a particular style and
// prevent any dependancies of the algorithms onto unrelated systems. Feel free
// to adapt any of this into your own 3D engine/environment.
//
// Instead of having unique Hooke force and damping coefficients on each spring,
// the SpringNetwork code uses a spring 'type' that indexes a short list of
// shared named coefficients. This was just more practical for the typical
// application of this technology. Over-designed systems that are too general
// can be slower for the next guy to understand and more painful to use.
...
SpringNetwork::Spring &SpringNetwork::AddBlocks(Spring &s) {
  ...
  // them up each time a spring is applied All 3 matrices A,dFdX, and dFdV are
  // contstructed identically so the block array layout will be the same for
  // each.
...
void SpringNetwork::PreSolveSpring(const SpringNetwork::Spring &s) {
  ...
  // dFdX and dFdV One optimization would be premultiply dfdx by dt*dt and F and
  // dFdV by dt right here in this function. However, for educational purposes
  // we wont do that now and intead just follow the paper directly.
...
void SpringNetwork::Simulate(float dt) {
  ...
  UpdateLimits();
  awake =
      (dot(V, V) < cloth_sleepthreshold) ? awake - 1 : awake = cloth_sleepcount;
}
