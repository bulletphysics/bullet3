...
struct b3SoundEngineInternalData {
  ...
  b3SoundEngineInternalData()
      : m_useRealTimeDac(false), m_wavFileUidGenerator(123) {}
};

b3SoundEngine::b3SoundEngine() { m_data = new b3SoundEngineInternalData(); }
...
void b3SoundEngine::init(int maxNumSoundSources, bool useRealTimeDac) {
  ...
  if (useRealTimeDac) {
    RtAudioFormat format =
        (sizeof(double) == 8) ? RTAUDIO_FLOAT64 : RTAUDIO_FLOAT32;
    ...
    m_data->m_dac.openStream(
        &parameters, NULL, format, (unsigned int)sampleRate, &bufferFrames,
        &b3AudioListener::tick, (void*)m_data->m_listener.getTickData());
...
void b3SoundEngine::startSound(int soundSourceIndex, b3SoundMessage msg) {
  ...
  soundSource->setADSR(msg.m_attackRate, msg.m_decayRate, msg.m_sustainLevel,
                       msg.m_releaseRate);
