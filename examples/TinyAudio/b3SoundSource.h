...
class b3SoundSource {
 ...
 public:
  ...
  virtual bool computeSamples(double* sampleBuffer, int numSamples,
                              double sampleRate);
  ...
  void setADSR(double attackRate, double decayRate, double sustainLevel,
               double releaseRate);

  bool setWavFile(int oscillatorIndex, class b3ReadWavFile* wavFilePtr,
                  int sampleRate);
