...
static const char* pairsKernelsCL =
    "__kernel void moveObjectsKernel(__global float4* posOrnColors, int "
    "numObjects)\n"
    "{\n"
    ...
    "}\n"
    "__kernel void colorPairsKernel2(__global float4* posOrnColors, int "
    "numObjects, __global const int4* pairs, int indexOffset, int numPairs)\n"
    "{\n"
    ...
    "}\n"
    "__kernel void \n"
    "  sineWaveKernel( __global float4* posOrnColors, __global float* "
    "pBodyTimes,float timeStepPos, float mAmplitude,const int numNodes)\n"
    "{\n"
    ...
    "		position.x = native_cos(pBodyTimes[nodeID]*2.17f)*mAmplitude + "
    "native_sin(pBodyTimes[nodeID])*mAmplitude*0.5f;\n"
    "		position.y = native_cos(pBodyTimes[nodeID]*1.38f)*mAmplitude + "
    "native_sin(pBodyTimes[nodeID]*mAmplitude);\n"
    "		position.z = native_cos(pBodyTimes[nodeID]*2.17f)*mAmplitude + "
    "native_sin(pBodyTimes[nodeID]*0.777f)*mAmplitude;\n"
    "		\n"
    ...
    "	int	uw;\n"
    "} b3AABBCL;\n"
    "__kernel void updateAabbSimple( __global float4* posOrnColors, const int "
    "numNodes, __global b3AABBCL* pAABB)\n"
    "{\n"
    "	int nodeId = get_global_id(0);\n"
    "	if( nodeId < numNodes )\n"
    "	{\n"
    ...
    "		\n"
    "		float4 position = posOrnColors[nodeId];\n"
    "		float4 argAabbMinVec = "
    "(float4)(orgAabbMin.fx,orgAabbMin.fy,orgAabbMin.fz,0.f);\n"
    "		float4 argAabbMaxVec = "
    "(float4)(orgAabbMax.fx,orgAabbMax.fy,orgAabbMax.fz,0.f);\n"
