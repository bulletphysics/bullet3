...
bool CMainApplication::BInit() {
  ...
  SDL_GL_CONTEXT_PROFILE_COMPATIBILITY ); SDL_GL_SetAttribute(
  ...
  */
  m_app = new SimpleOpenGL3App("SimpleOpenGL3App", m_nWindowWidth,
                               m_nWindowHeight, true);
  ...
  /*	if (m_pWindow == NULL)
     ...
     __FUNCTION__, SDL_GetError() ); return false;
          }
          */
  ...
  glewGetErrorString( nGlewError ) ); return false;
  }
...
bool CMainApplication::BInitGL() {
  if (m_bDebugOpenGL) {
    ...
    // nullptr, GL_TRUE ); glEnable(GL_DEBUG_OUTPUT_SYNCHRONOUS);
  }
...
void CMainApplication::Shutdown() {
  ...
  if (m_hasContext) {
    if (m_glSceneVertBuffer) {
      // glDebugMessageControl( GL_DONT_CARE, GL_DONT_CARE, GL_DONT_CARE, 0,
      // nullptr, GL_FALSE ); glDebugMessageCallback(nullptr, nullptr);
...
bool CMainApplication::CreateAllShaders() {
  ...
  m_unLensProgramID = CompileGLShader(
      ...
      "void main()\n"
      "{\n"
      "	float fBoundsCheck = ( (dot( vec2( lessThan( v2UVgreen.xy, vec2(0.05, "
      "0.05)) ), vec2(1.0, 1.0))+dot( vec2( greaterThan( v2UVgreen.xy, vec2( "
      "0.95, 0.95)) ), vec2(1.0, 1.0))) );\n"
      "	if( fBoundsCheck > 1.0 )\n"
