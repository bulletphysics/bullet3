...
// you can execute. You can call ImGui::ShowTestWindow() in your code to learn
// about various features of ImGui. Everything in this file will be stripped out
// by the linker if you don't call ImGui::ShowTestWindow().
...
void ImGui::ShowTestWindow(bool* p_open) {
  ...
  if (ImGui::CollapsingHeader("Widgets")) {
    ...
    if (ImGui::TreeNode("UTF-8 Text")) {
      ...
      // Studio you need to save your file as 'UTF-8 without signature') However
      // for the sake for maximum portability here we are *not* including raw
      // UTF-8 character in this source file, instead we encode the string with
      // hexadecimal constants. In your own application be reasonable and use
      // UTF-8 in source or retrieve the data from file system! Note that
      // characters values are preserved even if the font cannot be displayed,
      // so you can safely copy & paste garbled characters into another
      // application.
      ImGui::TextWrapped(
    ...
    for (int i = 0; i < 7; i++) {
      ...
      ImGui::PushStyleColor(ImGuiCol_Button,
                            ImColor::HSV(i / 7.0f, 0.6f, 0.6f));
      ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                            ImColor::HSV(i / 7.0f, 0.7f, 0.7f));
      ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                            ImColor::HSV(i / 7.0f, 0.8f, 0.8f));
      ImGui::Button("Click");
    ...
    ImGui::Combo(
        "combo", &item,
        "aaaa\0bbbb\0cccc\0dddd\0eeee\0\0");  // Combo using values packed in a
                                              // single constant string (for
                                              // really quick combo)
    ...
    ImGui::Combo(
        ...
        IM_ARRAYSIZE(items));  // Combo using proper array. You can also pass a
                               // callback to retrieve array value, no need to
                               // create/copy an array just for that.
    ...
    if (ImGui::TreeNode("Vertical Sliders")) {
      ...
      for (int i = 0; i < 7; i++) {
        ...
        ImGui::PushStyleColor(ImGuiCol_FrameBg,
                              ImColor::HSV(i / 7.0f, 0.5f, 0.5f));
        ImGui::PushStyleColor(ImGuiCol_FrameBgHovered,
                              ImColor::HSV(i / 7.0f, 0.6f, 0.5f));
        ImGui::PushStyleColor(ImGuiCol_FrameBgActive,
                              ImColor::HSV(i / 7.0f, 0.7f, 0.5f));
        ImGui::PushStyleColor(ImGuiCol_SliderGrab,
                              ImColor::HSV(i / 7.0f, 0.9f, 0.9f));
  ...
  if (ImGui::CollapsingHeader("Graphs widgets")) {
    ...
    if (animate) {
      static float refresh_time =
          ImGui::GetTime();  // Create dummy data at fixed 60 hz rate for the
                             // demo
      for (; ImGui::GetTime() > refresh_time + 1.0f / 60.0f;
    ...
    if (ImGui::TreeNode("Horizontal Scrolling")) {
      ...
      for (int line = 0; line < lines; line++) {
        ...
        for (int n = 0; n < num_buttons; n++) {
          ...
          ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                                ImColor::HSV(hue, 0.7f, 0.7f));
          ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                                ImColor::HSV(hue, 0.8f, 0.8f));
          ImGui::Button(label,
  ...
  if (ImGui::CollapsingHeader("Columns")) {
    ...
    */
    ...
    ImGui::SameLine();
    ShowHelpMarker(
        "NB: Tree node must be poped before ending the cell.\nThere's no "
  ...
  if (ImGui::CollapsingHeader("Keyboard, Mouse & Focus")) {
    ...
    if (ImGui::TreeNode("Dragging")) {
      ...
      if (ImGui::IsItemActive()) {
        ...
        draw_list->AddLine(ImGui::CalcItemRectClosestPoint(
                               ImGui::GetIO().MousePos, true, -2.0f),
    ...
    if (ImGui::TreeNode("Keyboard & Mouse State")) {
      ...
      ImGui::Text("KeyMods: %s%s%s%s", io.KeyCtrl ? "CTRL " : "",
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref) {
      ...
      for (int i = 0; i < ImGuiCol_COUNT; i++) {
        ...
        if (!output_only_modified ||
            memcmp(&col, (ref ? &ref->Colors[i] : &default_style.Colors[i]),
                   sizeof(ImVec4)) != 0)
          ImGui::LogText(
              "style.Colors[ImGuiCol_%s]%*s= ImVec4(%.2ff, %.2ff, %.2ff, "
              "%.2ff);" IM_NEWLINE,
  ...
  if (ImGui::TreeNode("Fonts", "Fonts (%d)",
    ...
    for (int i = 0; i < atlas->Fonts.Size; i++) {
      ...
      if (ImGui::TreeNode("Details")) {
        ...
        ImGui::Text("Fallback character: '%c' (%d)", font->FallbackChar,
                    font->FallbackChar);
        ImGui::Text("Texture surface: %d pixels (approx)",
...
struct ExampleAppConsole {
  ...
  ExampleAppConsole() {
    ...
    Commands.push_back(
        "CLASSIFY");  // "classify" is here to provide an example of "C"+[tab]
    ...
    for (int i = 0; i < Items.Size; i++) {
      ...
      ImVec4 col =
          ImVec4(1.0f, 1.0f, 1.0f,
                 1.0f);  // A better implementation may store a type per-item.
  ...
  void ExecCommand(const char* command_line) {
    ...
    if (Stricmp(command_line, "CLEAR") == 0) {
      ...
      for (int i = History.Size >= 10 ? History.Size - 10 : 0; i < History.Size;
           i++)
        AddLog("%3d: %s\n", i, History[i]);
    } else {
