...
examples/opengl2_example/ first as it is the simplest. you may be able to grab
...
https://github.com/ocornut/imgui/issues The list below consist mostly of ideas
noted down before they are requested/discussed by users (at which point it
usually moves to the github)
...
// to a different context by calling ImGui::SetCurrentContext() ImGui is
// currently not thread-safe because of this variable. If you want thread-safety
// to allow N threads to access N different contexts, you might work around it
// by:
...
void ImGui::NewFrame() {
  ...
  if ((g.IO.MousePos.x < 0 && g.IO.MousePos.y < 0) ||
      (g.IO.MousePosPrev.x < 0 &&
       g.IO.MousePosPrev.y <
           0))  // if mouse just appeared or disappeared (negative coordinate)
                // we cancel out movement in MouseDelta
...
void ImGui::EndFrame() {
  ...
  IM_ASSERT(g.FrameCountEnded !=
            g.FrameCount);  // ImGui::EndFrame() called multiple times, or
  ...
  if (values_count > 0) {
    ...
    ImVec2 tp0 = ImVec2(
        t0, 1.0f - ImSaturate((v0 - scale_min) /
                              (scale_max -
                               scale_min)));  // Point in the normalized space
  ...
  if (focus_requested || user_clicked || user_scrolled) {
    if (g.ActiveId != id) {
      ...
      edit_state.InitialText.resize(
          buf_size + 1);  // UTF-8. we use +1 to make sure that .Data isn't NULL
                          // so it doesn't crash.
      ...
      edit_state.CurLenA =
          (int)(buf_end -
                buf);  // We can't get the result from ImFormatString() above
                       // because it is not UTF-8 aware. Here we'll cut off
                       // malformed UTF-8.
...
void ImGui::EndGroup() {
  ...
  group_bb.Max.y -=
      g.Style.ItemSpacing.y;  // Cancel out last vertical spacing because we are
      ...
      if (held) {
        if (g.ActiveIdIsJustActivated)
          g.ActiveIdClickOffset.x -=
              4;  // Store from center of column line (we used a 8 wide rect for
                  // columns clicking)
    ...
    for (int column_index = 0; column_index < columns_count + 1;
      ...
      const float t = window->DC.StateStorage->GetFloat(
          ...
          default_t);  // Cheaply store our floating point value inside the
                       // integer (could store a union into the map?)
      window->DC.ColumnsData[column_index].OffsetNorm = t;
    }
  ...
  if (ImGui::Begin("ImGui Metrics", p_open)) {
    ...
    if (ImGui::TreeNode("Basic state")) {
      ...
      ImGui::Text(
          ...
          g.HoveredIdPreviousFrame);  // Data is "in-flight" so depending on
