...
void V_URLEncodeInternal(char *pchDest, int nDestLen, const char *pchSource,
  ...
  for (int i = 0; i < nSourceLen; ++i) {
    ...
    // through unescaped. These are the characters allowed by both the original
    // RFC 1738 and the latest RFC 3986. Current specs also allow '~', but that
    // is forbidden under original RFC 1738.
...
size_t V_URLDecodeInternal(char *pchDecodeDest, int nDecodeDestLen,
  ...
  // data, but if we do go ahead and do it as this can avoid bugs.
...
//          + for space as is the standard in HTML form encoding, despite it not
//          being part of the RFC.
//
