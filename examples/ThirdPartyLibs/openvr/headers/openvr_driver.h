...
VR_CAMERA_DECL_ALIGN(8) struct CameraVideoStreamFrame_t {
  ...
  double
      m_flFrameCaptureTime_ClientRelative;  // At the client, relative to when
                                            // the frame was exposed/captured.

  double m_flSyncMarkerError;

  TrackedDevicePose_t
      m_StandingTrackedDevicePose;  // Supplied by HMD layer when used as a
                                    // tracked camera
...
struct DriverPose_t {
   ...
   * measured, and is instead calculated from successive samples of angular
   * velocity. This leads to a noisy angular acceleration values, which are also
...
class ITrackedDeviceServerDriver {
 public:
   ...
   * processor use by the ITrackedDeviceServerDriver object should be kept to a
   * minimum until it is activated. The pose listener is guaranteed to be valid
   * until Deactivate is called, but should not be used after that point. */
...
//Chances are you don't 			need to implement this component in your driver.
// ----------------------------------------------------------------------------------------------
class IVRDriverDirectModeComponent {
 public:
   ...
   * eye.  Textures must be created using CreateSwapTextureSet and should be
   * alternated per frame.  Call Present once all layers have been submitted. */
...
class IVRDriverContext {
 public:
   ...
   * will return NULL and fill out the error. */
...
class IServerTrackedDeviceProvider {
 public:
   ...
   *called. If Init returns anything other than VRInitError_None the driver DLL
   *will be unloaded.
   *
   * pDriverHost will never be NULL, and will always be a pointer to a
   *IServerDriverHost interface
   *
   * pchUserDriverConfigDir - The absolute path of the directory where the
   *driver should store user config files. pchDriverInstallDir - The absolute
   *path of the root directory for the driver.
   */
   ...
   * itself into whatever sort of low-power state it has. */
  virtual void EnterStandby() = 0;

  /** Called when the system is leaving Standby mode. The driver should switch
  itself back to full operation. */
...
class CVRPropertyHelpers {
 public:
   ...
   * property value type does not match, this function will return false. */
   ...
   * valid or the property is not a string type this function will return an
   * empty string. */
 ...
 * is not a string type this function will return 0. Otherwise it returns the
 * length of the number of bytes necessary to hold this string including the
 * trailing null. Strings will always fit in buffers of
 * k_unMaxPropertyStringSize characters. */
 ...
 * or the property is not a string type this function will return an empty
 * string. */
...
class IVRServerDriverHost {
 public:
   ...
   * returns true the server will call Activate on the device. If it returns
   * false some kind of error
   ...
   * the device. This is only permitted on devices of the HMD class. */
   ...
   * tracking space which is uniquely defined by each driver providing poses for
   * its devices.  It is up to clients of this function to correlate poses
   * across different drivers.  Poses are indexed by their device id, and their
...
class IVRWatchdogHost {
 public:
   ...
   * signal from hardware that should cause SteamVR to start */
