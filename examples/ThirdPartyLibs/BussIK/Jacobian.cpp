...
void Jacobian::CalcDeltaThetasDLSwithNullspace(const VectorRn& desiredV) {
  ...
  // method with clamped error vector e. CalcdTClampedFromdS(); VectorRn
  // dTextra(3*m_nEffector); U.Solve( dT, &dTextra ); J.MultiplyTranspose(
  // dTextra, dTheta );
...
void Jacobian::UpdatedSClampValue(VectorR3* targets) {
  ...
  while (n) {
    if (n->IsEffector()) {
      ...
      // Compute the delta S value (differences from end effectors to target
      // positions. While we are at it, also update the clamping values in
      // dSclamp;
      temp = targetPos;
      temp -= n->GetS();
      double normSi =
...
void Jacobian::CalcDeltaThetasSDLSrev2()
{
    ...
    assert(J.DebugCheckSVD(U, w , V));
        ...
        //	Delta target values are the dS values
        int nRows = J.GetNumRows();
        int numEndEffectors = tree->GetNumEffector();		// Equals the
number of rows of J divided by three int nCols = J.GetNumColumns();
        dTheta.SetZero();
        ...
        for ( i=0; i<nRows; i++ ) {
                ...
                double N = 0.0;
// N is the quasi-1-norm of the i-th column of U double alpha = 0.0;
...
according to the i-th column of V double P = 0.0; double *vx =
V.GetColumnPtr(i); jnx = Jnorms.GetPtr(); for ( j=nCols; j>0; j-- ) { double
accum=0.0; for ( long k=numEndEffectors; k>0; k-- ) { accum += *(jnx++);
                        }
                        P += fabs((*(vx++)))*accum;
                }
                ...
                double lambda = 1.0;
                if ( N<P ) {
                        lambda -= N/P;				// Scale back
maximum permissable joint angle
                }
                lambda *= lambda;
