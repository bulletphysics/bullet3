...
class MatrixRmn {
 public:
  ...
  // (0,0). The "Tranpose" versions load the transpose of A.
  ...
  void Solve(const VectorRn& b, VectorRn* x)
      const;  // Solves the equation   (*this)*x = b;    Uses row operations.
              // Assumes *this is invertible.
  ...
  void
  ConvertToRefNoFree();  // Converts the matrix in place to row echelon form --
                         // assumption is no free variables will be found
  void ConvertToRef(
      int numVars);  // Converts the matrix in place to row echelon form --
                     // numVars is number of columns to work with.
  void ConvertToRef(
      int numVars,
