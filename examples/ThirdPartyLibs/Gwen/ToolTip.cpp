...
void RenderToolTip(Skin::Base* skin) {
  ...
  Gwen::Rect rOffset =
      Gwen::Rect(MousePos.x - Bounds.w * 0.5f, MousePos.y - Bounds.h - 10,
                 Bounds.w, Bounds.h);
  rOffset =
      Utility::ClampRectToRect(rOffset, g_ToolTip->GetCanvas()->GetBounds());
...
void ControlDeleted(Controls::Base* pControl) { Disable(pControl); }
}  // namespace ToolTip
