...
Gwen::Font* Text::GetFont() { return m_Font; }
...
void Text::SetString(const String& str) {
  SetString(Gwen::Utility::StringToUnicode(str));
}
...
void Text::Layout(Skin::Base* /*skin*/) { RefreshSize(); }
...
Gwen::Point Text::GetCharacterPosition(int iChar) {
  ...
  if (p.y >= m_Font->size) p.y -= m_Font->size;
...
void Text::OnScaleChanged() { Invalidate(); }