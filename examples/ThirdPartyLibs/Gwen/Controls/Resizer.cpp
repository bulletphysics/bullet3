...
void Resizer::OnMouseMoved(int x, int y, int /*deltaX*/, int /*deltaY*/) {
  ...
  if (m_iResizeDir & Pos::Right) {
    ...
    // the control But we need to move the HoldPos with the window movement.
    // Yikes. I actually think this might be a big hack around the way this
    // control works with regards to the holdpos being on the parent panel.
