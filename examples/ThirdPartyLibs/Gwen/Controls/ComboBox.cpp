...
class GWEN_EXPORT DownArrow : public Controls::Base {
 public:
  ...
  void Render(Skin::Base* skin) { skin->DrawArrowDown(this->m_Bounds); }
...
MenuItem* ComboBox::AddItem(const UnicodeString& strLabel,
                            const String& strName,
                            Gwen::Event::Handler* pHandler,
                            Gwen::Event::Handler::Function fn) {
  ...
  // Default
  if (m_SelectedItem == NULL) OnItemSelected(pItem);

  return pItem;
}

void ComboBox::Render(Skin::Base* skin) { skin->DrawComboBox(this); }

void ComboBox::OpenButtonPressed(Controls::Base* /*pControl*/) { OnPress(); }
...
void ComboBox::OnLostKeyboardFocus() { SetTextColor(Color(0, 0, 0, 255)); }
...
Gwen::Controls::Label* ComboBox::GetSelectedItem() { return m_SelectedItem; }

bool ComboBox::IsMenuOpen() { return m_Menu && !m_Menu->Hidden(); }
...
bool ComboBox::OnKeyUp(bool bDown) {
  if (bDown) {
    ...
    Base::List::reverse_iterator it =
        std::find(children.rbegin(), children.rend(), m_SelectedItem);
...
bool ComboBox::OnKeyDown(bool bDown) {
  if (bDown) {
    ...
    Base::List::iterator it =
        std::find(children.begin(), children.end(), m_SelectedItem);
