...
TabControl* DockBase::GetTabControl() { return m_DockedTabControl; }
...
int DockBase::GetDroppedTabDirection(int x, int y) {
  ...
  float minimum =
      GwenUtil_Min(GwenUtil_Min(GwenUtil_Min(top, left), right), bottom);
...
bool DockBase::DragAndDrop_CanAcceptPackage(
    Gwen::DragAndDrop::Package* pPackage) {
  // A TAB button dropped
  if (pPackage->name == "TabButtonMove") return true;

  // a TAB window dropped
  if (pPackage->name == "TabWindowMove") return true;
...
bool DockBase::DragAndDrop_HandleDrop(Gwen::DragAndDrop::Package* pPackage,
                                      int x, int y) {
  ...
  if (pPackage->name == "TabWindowMove") {
    DockedTabControl* pTabControl =
        DragAndDrop::SourceControl->DynamicCastDockedTabControl();
...
void DockBase::DragAndDrop_HoverEnter(Gwen::DragAndDrop::Package* /*pPackage*/,
                                      int /*x*/, int /*y*/) {
  m_bDrawHover = true;
}

void DockBase::DragAndDrop_HoverLeave(
    Gwen::DragAndDrop::Package* /*pPackage*/) {
  m_bDrawHover = false;
}

void DockBase::DragAndDrop_Hover(Gwen::DragAndDrop::Package* /*pPackage*/,
                                 int x, int y) {
  Gwen::Point pPos = CanvasPosToLocal(Gwen::Point(x, y));
  int dir = GetDroppedTabDirection(pPos.x, pPos.y);
