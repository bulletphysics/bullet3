...
Base::~Base() {
  {
    ...
    if (canvas) canvas->PreDelete(this);
  }
  ...
  for (AccelMap::iterator accelIt = m_Accelerators.begin();
       accelIt != m_Accelerators.end(); ++accelIt) {
...
int Base::GetDock() { return m_iDock; }

bool Base::Hidden() const { return m_bHidden; }
...
void Base::InvalidateChildren(bool bRecursive) {
  for (Base::List::iterator it = Children.begin(); it != Children.end(); ++it) {
    ...
    if (bRecursive) (*it)->InvalidateChildren(bRecursive);
  }

  if (m_InnerPanel) {
    for (Base::List::iterator it = m_InnerPanel->Children.begin();
         it != m_InnerPanel->Children.end(); ++it) {
      (*it)->Invalidate();
...
Controls::Base* Base::FindChildByName(const Gwen::String& name,
                                      bool bRecursive) {
...
void Base::BringNextToControl(Controls::Base* pChild, bool bBehind) {
  ...
  Base::List::iterator it =
      std::find(m_Parent->Children.begin(), m_Parent->Children.end(), pChild);
  if (it == m_Parent->Children.end()) return BringToFront();
...
void Base::OnChildAdded(Base* /*pChild*/) { Invalidate(); }

void Base::OnChildRemoved(Base* /*pChild*/) { Invalidate(); }
...
void Base::MoveTo(int x, int y) {
  if (m_bRestrictToParent && GetParent()) {
    ...
    if (x + Width() + GetPadding().right >
        pParent->Width() - pParent->GetMargin().right)
      x = pParent->Width() - pParent->GetMargin().right - Width() -
          GetPadding().right;
    if (y + Height() + GetPadding().bottom >
        pParent->Height() - pParent->GetMargin().bottom)
      y = pParent->Height() - pParent->GetMargin().bottom - Height() -
          GetPadding().bottom;
  }
...
void Base::SetPos(int x, int y) { SetBounds(x, y, Width(), Height()); }

bool Base::SetSize(int w, int h) { return SetBounds(X(), Y(), w, h); }
...
bool Base::SetBounds(int x, int y, int w, int h) {
  if (m_Bounds.x == x && m_Bounds.y == y && m_Bounds.w == w && m_Bounds.h == h)
    return false;
...
void Base::OnBoundsChanged(Gwen::Rect oldBounds) {
  ...
  if (m_Bounds.w != oldBounds.w || m_Bounds.h != oldBounds.h) {
    if (GetParent()) GetParent()->OnChildBoundsChanged(oldBounds, this);
    Invalidate();
  }
...
void Base::OnChildBoundsChanged(Gwen::Rect /*oldChildBounds*/,
                                Base* /*pChild*/) {}

void Base::Render(Gwen::Skin::Base* /*skin*/) {}

void Base::DoCacheRender(Gwen::Skin::Base* skin,
                         Gwen::Controls::Base* pMaster) {
  ...
  if (m_bCacheTextureDirty && render->ClipRegionVisible()) {
    ...
    if (ShouldCacheToTexture()) cache->SetupCacheTexture(this);
...
bool Base::OnMouseWheeled(int iDelta) {
  if (m_ActualParent) return m_ActualParent->OnMouseWheeled(iDelta);
...
void Base::OnMouseLeave() {
  ...
  if (GetToolTip()) ToolTip::Disable(this);
}

bool Base::IsHovered() { return Gwen::HoveredControl == this; }

bool Base::ShouldDrawHover() {
  return Gwen::MouseFocus == this || Gwen::MouseFocus == NULL;
}

bool Base::HasFocus() { return Gwen::KeyboardFocus == this; }

void Base::Focus() {
  if (Gwen::KeyboardFocus == this) return;

  if (Gwen::KeyboardFocus) Gwen::KeyboardFocus->OnLostKeyboardFocus();
...
void Base::Touch() {
  if (GetParent()) GetParent()->OnChildTouched(this);
}

void Base::OnChildTouched(Controls::Base* /*pChild*/) { Touch(); }
...
Base* Base::GetControlAt(int x, int y) {
  ...
  if (!GetMouseInputEnabled()) return NULL;
...
void Base::RecurseLayout(Skin::Base* skin) {
  ...
  for (Base::List::iterator iter = Children.begin(); iter != Children.end();
    ...
    if (iDock & Pos::Right) {
      ...
      pChild->SetBounds(
          (rBounds.x + rBounds.w) - pChild->Width() - margin.right,
          rBounds.y + margin.top, pChild->Width(),
          rBounds.h - margin.top - margin.bottom);
...
void Base::UpdateCursor() { Platform::SetCursor(m_Cursor); }

DragAndDrop::Package* Base::DragAndDrop_GetPackage(int /*x*/, int /*y*/) {
  return m_DragAndDrop_Package;
}

bool Base::DragAndDrop_HandleDrop(Gwen::DragAndDrop::Package* /*pPackage*/,
                                  int /*x*/, int /*y*/) {
...
void Base::DragAndDrop_SetPackage(bool bDraggable, const String& strName,
                                  void* pUserData) {
  if (!m_DragAndDrop_Package) {
...
void Base::DragAndDrop_StartDragging(Gwen::DragAndDrop::Package* pPackage,
                                     int x, int y) {
...
void Base::SetMargin(const Margin& margin) {
  if (m_Margin.top == margin.top && m_Margin.left == margin.left &&
      m_Margin.bottom == margin.bottom && m_Margin.right == margin.right)
    return;
...
bool Base::HandleAccelerator(Gwen::UnicodeString& accelerator) {
  ...
  for (Base::List::iterator it = Children.begin(); it != Children.end(); ++it) {
    if ((*it)->HandleAccelerator(accelerator)) return true;
  }
  return false;
}
...
bool Base::OnKeyPress(int iKey, bool bPress) {
  ...
  if (!bHandled && GetParent()) GetParent()->OnKeyPress(iKey, bPress);

  return bHandled;
}

bool Base::OnKeyRelease(int iKey) { return OnKeyPress(iKey, false); }
...
void Base::Anim_WidthIn(float fLength, float fDelay, float fEase) {
  Gwen::Anim::Add(this, new Gwen::Anim::Size::Width(0, Width(), fLength, false,
                                                    fDelay, fEase));
  SetWidth(0);
}

void Base::Anim_HeightIn(float fLength, float fDelay, float fEase) {
  Gwen::Anim::Add(this, new Gwen::Anim::Size::Height(0, Height(), fLength,
                                                     false, fDelay, fEase));
  SetHeight(0);
}

void Base::Anim_WidthOut(float fLength, bool bHide, float fDelay, float fEase) {
  Gwen::Anim::Add(this, new Gwen::Anim::Size::Width(Width(), 0, fLength, bHide,
                                                    fDelay, fEase));
}

void Base::Anim_HeightOut(float fLength, bool bHide, float fDelay,
                          float fEase) {
  Gwen::Anim::Add(this, new Gwen::Anim::Size::Height(Height(), 0, fLength,
                                                     bHide, fDelay, fEase));
}
