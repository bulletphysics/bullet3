...
void TreeControl::Render(Skin::Base* skin) {
  if (ShouldDrawBackground()) skin->DrawTreeControl(this);
}
...
void TreeControl::OnChildBoundsChanged(Gwen::Rect /*oldChildBounds*/,
                                       Base* /*pChild*/) {}

void TreeControl::Clear() { m_ScrollControl->Clear(); }
...
bool TreeControl::OnKeyUp(bool bDown) {
  if (bDown) {
    ...
    if (curItem > 0) {
      ...
      float viewSize =
          m_ScrollControl->m_VerticalScrollBar->getViewableContentSize();
      float contSize = m_ScrollControl->m_VerticalScrollBar->getContentSize();

      float curAmount =
          m_ScrollControl->m_VerticalScrollBar->GetScrolledAmount();
      ...
      if (contSize != viewSize) {
        {
          ...
          if (newAmount < curAmount) {
            m_ScrollControl->m_VerticalScrollBar->SetScrolledAmount(newAmount,
                                                                    true);
          }
        }
        {
          ...
          float newAmount =
              float((curItem - numItems) * 16) / (contSize - viewSize);
