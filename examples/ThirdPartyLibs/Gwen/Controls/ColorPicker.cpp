...
void ColorPicker::NumericTyped(Gwen::Controls::Base* control) {
  ...
  if (box->GetName().find("Red") != Gwen::String::npos) SetRed(textValue);

  if (box->GetName().find("Green") != Gwen::String::npos) SetGreen(textValue);

  if (box->GetName().find("Blue") != Gwen::String::npos) SetBlue(textValue);

  if (box->GetName().find("Alpha") != Gwen::String::npos) SetAlpha(textValue);
...
void ColorPicker::UpdateColorControls(Gwen::String name, Gwen::Color col,
                                      int sliderVal) {
  ...
  HorizontalSlider* slider =
      FindChildByName(name + "Slider", true)->DynamicCastHorizontalSlider();
  slider->SetValue(sliderVal);

  TextBoxNumeric* box =
      FindChildByName(name + "Box", true)->DynamicCastTextBoxNumeric();
...
void ColorPicker::UpdateControls() {
  ...
  UpdateColorControls("Alpha", Color(255, 255, 255, GetColor().a),
                      GetColor().a);

  ColorDisplay* disp =
      FindChildByName("Result", true)->DynamicCastColorDisplay();
...
void ColorPicker::Layout(Skin::Base* skin) {
  ...
  GroupBox* groupBox =
      FindChildByName("ResultGroupBox", true)->DynamicCastGroupBox();
  if (groupBox)
    groupBox->SetPos(groupBox->X(),
                     Height() * 0.5f - groupBox->Height() * 0.5f);

  UpdateControls();
}

void ColorPicker::Render(Skin::Base* skin) { skin->DrawBackground(this); }
...
Gwen::String ColorPicker::GetColorFromName(Gwen::String name) {
  if (name.find("Red") != Gwen::String::npos) return "Red";
  if (name.find("Green") != Gwen::String::npos) return "Green";
  if (name.find("Blue") != Gwen::String::npos) return "Blue";
...
void ColorPicker::SetAlphaVisible(bool visible) {
  GroupBox* groupBox =
      FindChildByName("Alphagroupbox", true)->DynamicCastGroupBox();
