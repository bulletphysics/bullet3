...
Canvas::Canvas(Gwen::Skin::Base* pSkin)
    : BaseClass(NULL), m_bAnyDelete(false), m_fScale(-1) {
...
void Canvas::Render(Gwen::Skin::Base* /*pRender*/) { m_bNeedsRedraw = false; }
...
void Canvas::DoThink() {
  ...
  if (NextTab == NULL) NextTab = FirstTab;
...
void Canvas::SetScale(float f) {
  ...
  if (m_Skin && m_Skin->GetRender()) m_Skin->GetRender()->SetScale(m_fScale);
...
void Canvas::ProcessDelayedDeletes() {
  while (m_bAnyDelete) {
    ...
    for (Gwen::Controls::Base::List::iterator it = deleteList.begin();
...
bool Canvas::InputCharacter(Gwen::UnicodeChar chr) {
  ...
  // Handle Accelerators
  if (Gwen::Input::HandleAccelerator(this, chr)) return true;
