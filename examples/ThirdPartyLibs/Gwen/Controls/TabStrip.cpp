...
bool TabStrip::DragAndDrop_HandleDrop(Gwen::DragAndDrop::Package* /*pPackage*/,
                                      int x, int y) {
  ...
  TabControl* pTabControl =
      GetParent() ? GetParent()->DynamicCastTabControl() : 0;
  ...
  if (DroppedOn) {
    ...
    DragAndDrop::SourceControl->BringNextToControl(
        DroppedOn, DropPos.x > DroppedOn->Width() / 2);
  } else {
...
bool TabStrip::DragAndDrop_CanAcceptPackage(
    Gwen::DragAndDrop::Package* pPackage) {
  if (!m_bAllowReorder) return false;
...
void TabStrip::DragAndDrop_HoverEnter(Gwen::DragAndDrop::Package* /*pPackage*/,
                                      int /*x*/, int /*y*/) {
  if (m_TabDragControl) {
...
void TabStrip::DragAndDrop_HoverLeave(
    Gwen::DragAndDrop::Package* /*pPackage*/) {
  delete m_TabDragControl;
  m_TabDragControl = NULL;
}

void TabStrip::DragAndDrop_Hover(Gwen::DragAndDrop::Package* /*pPackage*/,
                                 int x, int y) {
  Gwen::Point LocalPos = CanvasPosToLocal(Gwen::Point(x, y));
...
void TabStrip::SetTabPosition(int iPos) { Dock(iPos); }