...
GWEN_CONTROL_CONSTRUCTOR(RichLabel) { m_bNeedsRebuild = false; }
...
void RichLabel::AddText(const Gwen::TextObject& text, Gwen::Color color,
                        Gwen::Font* font) {
...
void RichLabel::SplitLabel(const Gwen::UnicodeString& text, Gwen::Font* pFont,
                           const DividedText& txt, int& x, int& y,
                           int& lineheight) {
  Gwen::Utility::Strings::UnicodeList lst;
  ...
  for (size_t i = 0; i < lst.size(); i++) {
    Gwen::Point WordSize =
        GetSkin()->GetRender()->MeasureText(pFont, strNewString + lst[i]);
    if (WordSize.x > iSpaceLeft) {
...
void RichLabel::CreateLabel(const Gwen::UnicodeString& text,
                            const DividedText& txt, int& x, int& y,
  ...
  pLabel->SetText(
      x == 0 ? Gwen::Utility::Strings::TrimLeft<Gwen::UnicodeString>(text, L" ")
             : text);
...
void RichLabel::Rebuild() {
  ...
  int y = 0;
  int lineheight = -1;
  for (DividedText::List::iterator it = m_TextBlocks.begin();
       it != m_TextBlocks.end(); ++it) {
