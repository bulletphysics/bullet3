...
GWEN_CONTROL_CONSTRUCTOR(HorizontalScrollBar) {
  ...
  m_ScrollButton[SCROLL_BUTTON_LEFT]->onPress.Add(
      this, &HorizontalScrollBar::NudgeLeft);

  m_ScrollButton[SCROLL_BUTTON_RIGHT]->SetDirectionRight();
  m_ScrollButton[SCROLL_BUTTON_RIGHT]->onPress.Add(
      this, &HorizontalScrollBar::NudgeRight);
...
void HorizontalScrollBar::Layout(Skin::Base* skin) {
  ...
  float barWidth = (m_fViewableContentSize / m_fContentSize) *
                   (Width() - (GetButtonSize() * 2));

  if (barWidth < GetButtonSize() * 0.5) barWidth = GetButtonSize() * 0.5;
...
void HorizontalScrollBar::ScrollToLeft() { SetScrolledAmount(0, true); }
void HorizontalScrollBar::ScrollToRight() { SetScrolledAmount(1, true); }
...
float HorizontalScrollBar::CalculateScrolledAmount() {
  return (float)(m_Bar->X() - GetButtonSize()) /
         (float)(Width() - m_Bar->Width() - (GetButtonSize() * 2));
}
...
bool HorizontalScrollBar::SetScrolledAmount(float amount, bool forceUpdate) {
  ...
  if (forceUpdate) {
    int newX = GetButtonSize() +
               (amount * ((Width() - m_Bar->Width()) - (GetButtonSize() * 2)));
