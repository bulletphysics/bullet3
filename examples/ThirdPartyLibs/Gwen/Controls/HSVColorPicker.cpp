...
GWEN_CONTROL_CONSTRUCTOR(HSVColorPicker) {
  ...
  m_ColorSlider->onSelectionChanged.Add(this,
                                        &HSVColorPicker::ColorSliderChanged);
...
void HSVColorPicker::UpdateControls(Gwen::Color color) {
  TextBoxNumeric* redBox =
      FindChildByName("RedBox", false)->DynamicCastTextBoxNumeric();
  if (redBox) redBox->SetText(Gwen::Utility::ToString((int)color.r), false);

  TextBoxNumeric* greenBox =
      FindChildByName("GreenBox", false)->DynamicCastTextBoxNumeric();
  if (greenBox) greenBox->SetText(Gwen::Utility::ToString((int)color.g), false);

  TextBoxNumeric* blueBox =
      FindChildByName("BlueBox", false)->DynamicCastTextBoxNumeric();
...
void HSVColorPicker::SetColor(Gwen::Color color, bool onlyHue, bool reset) {
  ...
  if (reset) m_Before->SetColor(color);
...
Gwen::Color HSVColorPicker::GetColor() { return m_LerpBox->GetSelectedColor(); }
...
void HSVColorPicker::ColorSliderChanged(Gwen::Controls::Base* /*pControl*/) {
  if (m_LerpBox) m_LerpBox->SetColor(m_ColorSlider->GetSelectedColor(), true);
  Invalidate();
}