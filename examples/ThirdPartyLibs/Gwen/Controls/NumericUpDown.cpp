...
GWEN_CONTROL_CONSTRUCTOR(NumericUpDown) {
  ...
  NumericUpDownButton_Down* pButtonDown =
      new NumericUpDownButton_Down(pSplitter);
...
void NumericUpDown::SyncNumberFromText() { SetValue((int)GetFloatFromText()); }

void NumericUpDown::SetMin(int i) { m_iMin = i; }

void NumericUpDown::SetMax(int i) { m_iMax = i; }
...
void NumericUpDown::OnChange() { onChanged.Call(this); }
