...
void CrossSplitter::UpdateVSplitter() {
  m_VSplitter->MoveTo(m_VSplitter->X(),
                      (Height() - m_VSplitter->Height()) * (m_fVVal));
}
void CrossSplitter::UpdateHSplitter() {
  m_HSplitter->MoveTo((Width() - m_HSplitter->Width()) * (m_fHVal),
                      m_HSplitter->Y());
}
...
void CrossSplitter::Layout(Skin::Base* /*skin*/) {
  ...
  if (m_iZoomedSection == -1) {
    ...
    if (m_Sections[1])
      m_Sections[1]->SetBounds(m_HSplitter->X() + m_fBarSize, 0,
                               Width() - (m_HSplitter->X() + m_fBarSize),
                               m_VSplitter->Y());

    if (m_Sections[2])
      m_Sections[2]->SetBounds(0, m_VSplitter->Y() + m_fBarSize,
                               m_HSplitter->X(),
...
void CrossSplitter::SetPanel(int index, Controls::Base* pPanel) {
  Debug::AssertCheck(index >= 0 && index <= 3,
                     "CrossSplitter::SetPanel out of range");
...
Controls::Base* CrossSplitter::GetPanel(int i) { return m_Sections[i]; }
...
void CrossSplitter::Zoom(int section) {
  ...
  if (m_Sections[section]) {
    for (int i = 0; i < 4; i++) {
      if (i != section && m_Sections[i]) m_Sections[i]->SetHidden(true);
    }
...
void CrossSplitter::UnZoom() {
  ...
  for (int i = 0; i < 4; i++) {
    if (m_Sections[i]) m_Sections[i]->SetHidden(false);
  }
