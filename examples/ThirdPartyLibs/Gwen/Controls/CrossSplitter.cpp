...
{
	m_VSplitter->MoveTo(m_VSplitter->X(), int((float)(Height() - m_VSplitter->Height()) * (m_fVVal)));
}
void CrossSplitter::UpdateHSplitter()
{
	m_HSplitter->MoveTo(int((float)(Width() - m_HSplitter->Width()) * (m_fHVal)), m_HSplitter->Y());
}
...
{
	m_CSplitter->MoveTo(int((float)(Width() - m_CSplitter->Width()) * (m_fHVal)), int((float)(Height() - m_CSplitter->Height()) * (m_fVVal)));
}
