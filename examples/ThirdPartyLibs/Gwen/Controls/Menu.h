...
class GWEN_EXPORT Menu : public ScrollControl {
 public:
  ...
  virtual MenuItem* AddItem(const Gwen::UnicodeString& strName,
                            const UnicodeString& strIconName,
                            Gwen::Event::Handler* pHandler = NULL,
                            Gwen::Event::Handler::Function fn = NULL);
  ...
  virtual MenuItem* AddItem(const Gwen::String& strName,
...
class GWEN_EXPORT MenuDivider : public Base {
 public:
  GWEN_CONTROL_INLINE(MenuDivider, Base) { SetHeight(1); }
