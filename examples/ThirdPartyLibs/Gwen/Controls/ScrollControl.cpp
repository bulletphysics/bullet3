...
void ScrollControl::SetInnerSize(int w, int h) { m_InnerPanel->SetSize(w, h); }

void ScrollControl::VBarMoved(Controls::Base* /*control*/) { Invalidate(); }
  ...
  if (CanScrollV() && m_VerticalScrollBar->Visible()) {
    if (m_VerticalScrollBar->SetScrolledAmount(
            m_VerticalScrollBar->GetScrolledAmount() -
                m_VerticalScrollBar->GetNudgeAmount() * ((float)iDelta / 60.0f),
            true))
      return true;
  }
...
void ScrollControl::UpdateScrollBars() {
  ...
  for (Base::List::iterator iter = m_InnerPanel->Children.begin();
       iter != m_InnerPanel->Children.end(); ++iter) {
    Base* pChild = *iter;
  ...
  m_InnerPanel->SetSize(Utility::Max(Width(), childrenWidth),
                        Utility::Max(Height(), childrenHeight));

  float hg = (float)(childrenWidth + (m_VerticalScrollBar->Hidden()
                                          ? 0
                                          : m_VerticalScrollBar->Width()));
  if (hg == 0.f) hg = 0.00001f;
  float wPercent = (float)Width() / hg;
  hg = (float)(childrenHeight + (m_HorizontalScrollBar->Hidden()
                                     ? 0
...
void ScrollControl::SetVScrollRequired(bool req) {
  if (req) {
    ...
    if (m_bAutoHideBars) m_VerticalScrollBar->SetHidden(true);
  } else {
...
void ScrollControl::SetHScrollRequired(bool req) {
  if (req) {
    ...
    if (m_bAutoHideBars) m_HorizontalScrollBar->SetHidden(true);
  } else {
...
void ScrollControl::Clear() { m_InnerPanel->RemoveAllChildren(); }