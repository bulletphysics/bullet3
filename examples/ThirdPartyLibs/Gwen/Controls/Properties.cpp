...
int Properties::GetSplitWidth() { return m_SplitterBar->X(); }

PropertyRow* Properties::Add(const UnicodeString& text,
...
PropertyRow* Properties::Add(const String& text, const String& value) {
  return Add(Gwen::Utility::StringToUnicode(text),
             Gwen::Utility::StringToUnicode(value));
}
...
void Properties::Clear() {
  ...
  for (Base::List::iterator it = ChildListCopy.begin();
       it != ChildListCopy.end(); ++it) {
