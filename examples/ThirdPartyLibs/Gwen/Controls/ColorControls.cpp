...
Gwen::Color LerpColor(Gwen::Color& toColor, Gwen::Color& fromColor,
                      float amount) {
...
void ColorLerpBox::OnMouseMoved(int x, int y, int /*deltaX*/, int /*deltaY*/) {
  if (m_bDepressed) {
    ...
    if (cursorPos.x > Width()) cursorPos.x = Width();
...
void ColorSlider::Render(Gwen::Skin::Base* skin) {
  ...
  skin->GetRender()->DrawFilledRect(
      Gwen::Rect(Width() - 4, drawHeight + 1, 3, 3));
}
