...
void TreeNode::Render(Skin::Base* skin) {
  ...
  skin->DrawTreeNode(this, m_InnerPanel->Visible(), IsSelected(),
                     ...
                     m_ToggleButton->Y() + m_ToggleButton->Height() * 0.5,
                     iBottom, GetParent() == m_TreeControl);
}
...
void TreeNode::Open() {
  ...
  Invalidate();
  if (m_TreeControl) m_TreeControl->ForceUpdateScrollBars();
}
...
void TreeNode::iterate(int action, int* curIndex, int* targetIndex) {
    ...
    switch (action) {
      case ITERATE_ACTION_DESELECT_INDEX: {
        if (targetIndex && curIndex) {
          if (*targetIndex == *curIndex) SetSelected(false);
        }
        break;
      }
  ...
  if (action == ITERATE_ACTION_FIND_SELECTED_INDEX ||
      action == ITERATE_ACTION_SELECT ||
      action == ITERATE_ACTION_DESELECT_INDEX) {
    if (m_ToggleButton && !m_ToggleButton->GetToggleState()) {
