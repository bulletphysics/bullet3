...
void Label::Layout(Skin::Base* /*skin*/)
{
	...
	if (iAlign & Pos::CenterH) x = int((float)(m_rTextPadding.left + m_Padding.left) + ((float)(Width() - m_Text->Width()) * 0.5f) - (float)m_rTextPadding.right - (float)m_Padding.right);

	if (iAlign & Pos::CenterV) y = int((float)(m_rTextPadding.top + m_Padding.top) + ((float)(Height() - m_Text->Height()) * 0.5f) - (float)m_rTextPadding.bottom - (float)m_Padding.bottom);
	if (iAlign & Pos::Bottom) y = int(Height() - m_Text->Height() - m_rTextPadding.bottom - m_Padding.bottom);
