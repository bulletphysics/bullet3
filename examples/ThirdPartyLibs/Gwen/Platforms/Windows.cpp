...
#endif
    {IDC_ARROW,    IDC_IBEAM,   IDC_SIZENS, IDC_SIZEWE, IDC_SIZENWSE,
     IDC_SIZENESW, IDC_SIZEALL, IDC_NO,     IDC_WAIT,   IDC_HAND};
...
float Gwen::Platform::GetTimeInSeconds() {
  ...
  float fSecondsDifference =
      (double)(thistime - iLastTime) * GetPerformanceFrequency();
...
bool Gwen::Platform::FileOpen(const String& Name, const String& StartPath,
  ...
  char FilterBuffer[512];
  {
    ...
    memcpy(FilterBuffer, Extension.c_str(),
           GwenUtil_Min(Extension.size(), 512));
    for (int i = 0; i < 512; i++) {
  ...
  opf.lpfnHook = NULL;
  opf.lCustData = 0;
  opf.Flags = (OFN_PATHMUSTEXIST | OFN_OVERWRITEPROMPT | OFN_NOCHANGEDIR) &
