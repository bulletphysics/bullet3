...
inline void Center(Controls::Base* ctrl) {
  ...
  ctrl->SetPos(parent->GetPadding().left +
                   (((parent->Width() - parent->GetPadding().left -
                      parent->GetPadding().right) -
                     ctrl->Width()) /
                    2),
...
inline void AlignRight(Controls::Base* ctrl) {
  ...
  ctrl->SetPos(parent->Width() - ctrl->Width() - parent->GetPadding().right,
               ctrl->Y());
}

inline void AlignTop(Controls::Base* ctrl) { ctrl->SetPos(ctrl->X(), 0); }
...
inline void PlaceBelow(Controls::Base* ctrl, Controls::Base* below,
                       int iBorder = 0) {
