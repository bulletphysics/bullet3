 ...
 public:
  ...
  virtual void DrawRadioButton(Gwen::Controls::Base* control, bool bSelected,
    ...
    m_Render->DrawFilledRect(
        Gwen::Rect(rect.x + 2, rect.y + 2, rect.w - 4, rect.h - 4));
    m_Render->DrawFilledRect(
        Gwen::Rect(rect.x + 2, rect.y + 2, rect.w * 0.3f, rect.h - 4));
    m_Render->DrawFilledRect(
        Gwen::Rect(rect.x + 2, rect.y + 2, rect.w - 4, rect.h * 0.3f));
  ...
  virtual void DrawGroupBox(Gwen::Controls::Base* control, int textStart,
    ...
    m_Render->DrawFilledRect(Gwen::Rect(rect.x + 1 + textStart + textWidth,
                                        rect.y + 1,
                                        rect.w - textStart + textWidth - 2, 1));
    m_Render->DrawFilledRect(
        Gwen::Rect(rect.x + 1, (rect.y + rect.h) - 1, rect.w - 2, 1));
  ...
  virtual void DrawTextBox(Gwen::Controls::Base* control) {
    ...
    m_Render->DrawFilledRect(
        Gwen::Rect(rect.x + 1, (rect.y + rect.h) - 1, rect.w - 2, 1));
    m_Render->DrawFilledRect(
        Gwen::Rect((rect.x + rect.w) - 1, rect.y + 1, 1, rect.h - 2));
  ...
  virtual void DrawTabButton(Gwen::Controls::Base* control, bool bActive) {
    ...
    if (bActive) {
      m_Render->SetDrawColor(m_colControl);
      m_Render->DrawFilledRect(
          Gwen::Rect(rect.x + 1, rect.y + 1, rect.w - 2, rect.h - 1));
    } else {
      if (bHovered)
        m_Render->SetDrawColor(m_colControlBright);
      else
      ...
      m_Render->DrawFilledRect(
          Gwen::Rect(1, rect.h * 0.5, rect.w - 2, rect.h * 0.5 - 1));
  ...
  virtual void DrawWindow(Gwen::Controls::Base* control, int topHeight,
    ...
    if (inFocus)
      ...
      m_Render->SetDrawColor(
          Gwen::Color(87 * 0.70, 164 * 0.70, 232 * 0.70, 230));

    int iBorderSize = 5;
    m_Render->DrawFilledRect(
        Gwen::Rect(rect.x + 1, rect.y + 1, rect.w - 2, topHeight - 1));
    m_Render->DrawFilledRect(Gwen::Rect(rect.x + 1, rect.y + topHeight - 1,
                                        iBorderSize, rect.h - 2 - topHeight));
    m_Render->DrawFilledRect(Gwen::Rect(rect.x + rect.w - iBorderSize,
                                        rect.y + topHeight - 1, iBorderSize,
                                        rect.h - 2 - topHeight));
    m_Render->DrawFilledRect(Gwen::Rect(
        rect.x + 1, rect.y + rect.h - iBorderSize, rect.w - 2, iBorderSize));

    // Main inner
    m_Render->SetDrawColor(Gwen::Color(m_colControlDark.r, m_colControlDark.g,
                                       m_colControlDark.b, 230));
    m_Render->DrawFilledRect(Gwen::Rect(
        rect.x + iBorderSize + 1, rect.y + topHeight,
        rect.w - iBorderSize * 2 - 2, rect.h - topHeight - iBorderSize - 1));

    // Light inner border
    m_Render->SetDrawColor(Gwen::Color(255, 255, 255, 100));
    m_Render->DrawShavedCornerRect(
        Gwen::Rect(rect.x + 1, rect.y + 1, rect.w - 2, rect.h - 2));
    ...
    // Inside border
    m_Render->SetDrawColor(m_colControlOutlineNormal);
    m_Render->DrawLinedRect(Gwen::Rect(rect.x + iBorderSize,
                                       rect.y + topHeight - 1, rect.w - 10,
  ...
  virtual void DrawProgressBar(Gwen::Controls::Base* control, bool isHorizontal,
    ...
    if (isHorizontal) {
      ...
      // Top half
      m_Render->SetDrawColor(FillColour);
      m_Render->DrawFilledRect(Gwen::Rect(1, 1 + (rect.h * (1 - progress)),
  ...
  virtual void DrawTreeButton(Controls::Base* control, bool bOpen) {
    ...
    if (!bOpen)  // ! because the button shows intention, not the current state
      m_Render->DrawFilledRect(
          Gwen::Rect(rect.x + rect.w / 2, rect.y + 2, 1, rect.h - 4));

    m_Render->DrawFilledRect(
        Gwen::Rect(rect.x + 2, rect.y + rect.h / 2, rect.w - 4, 1));
  }
  ...
  void DrawTreeNode(Controls::Base* ctrl, bool bOpen, bool bSelected,
    ...
    if (bSelected) {
      ...
      m_Render->DrawLinedRect(
          Gwen::Rect(17, 0, iLabelWidth + 2, iLabelHeight - 1));
    }

    m_Render->SetDrawColor(Color(0, 0, 0, 50));

    if (!bIsRoot) m_Render->DrawFilledRect(Gwen::Rect(9, iHalfWay, 16 - 9, 1));

    if (!bOpen) return;

    m_Render->DrawFilledRect(Gwen::Rect(14 + 8, iLabelHeight, 1,
                                        iLastBranch + iHalfWay - iLabelHeight));
  }
  ...
  virtual void DrawPropertyTreeNode(Controls::Base* control, int BorderLeft,
    ...
    m_Render->DrawFilledRect(Gwen::Rect(rect.x + BorderLeft, rect.y,
