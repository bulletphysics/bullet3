...
int main(int argc, char* argv[]) {
  ...
  b3ConfigureOpenGLVisualizerSetVisualizationFlags(command, COV_ENABLE_SHADOWS,
                                                   0);
  b3SubmitClientCommandAndWaitStatus(kPhysClient, command);
  ...
  // load test
  command =
      b3LoadUrdfCommandInit(kPhysClient, "TwoJointRobot_wo_fixedJoints.urdf");
  ...
  for (int i = 0; i < numJoints; ++i) {
    ...
    command = b3JointControlCommandInit2(kPhysClient, twojoint,
  ...
  while (b3CanSubmitCommand(kPhysClient)) {
    ...
    command =
        b3JointControlCommandInit2(kPhysClient, twojoint, CONTROL_MODE_TORQUE);
    b3JointControlSetDesiredForceTorque(command, jointInfo.m_uIndex,
                                        0.5 * sin(10 * simTimeS));
    statusHandle = b3SubmitClientCommandAndWaitStatus(kPhysClient, command);
    ...
    statusHandle = b3SubmitClientCommandAndWaitStatus(kPhysClient, command);
    b3GetJointState(kPhysClient, statusHandle, jointNameToId["joint_1"],
                    &state);
    q[0] = state.m_jointPosition;
    v[0] = state.m_jointVelocity;
    b3GetJointState(kPhysClient, statusHandle, jointNameToId["joint_2"],
                    &state);
    q[1] = state.m_jointPosition;
    v[1] = state.m_jointVelocity;

    statusHandle = b3SubmitClientCommandAndWaitStatus(
        kPhysClient, b3InitStepSimulationCommand(kPhysClient));
