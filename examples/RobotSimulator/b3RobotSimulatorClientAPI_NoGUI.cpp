...
void b3RobotSimulatorClientAPI_NoGUI::setGravity(
    const b3Vector3& gravityAcceleration) {
  if (!isConnected()) {
    b3Warning("Not connected");
    return;
  }
...
b3Quaternion b3RobotSimulatorClientAPI_NoGUI::getQuaternionFromEuler(
    const b3Vector3& rollPitchYaw) {
  b3Quaternion q;
  q.setEulerZYX(rollPitchYaw[2], rollPitchYaw[1], rollPitchYaw[0]);
  return q;
}

b3Vector3 b3RobotSimulatorClientAPI_NoGUI::getEulerFromQuaternion(
    const b3Quaternion& quat) {
...
int b3RobotSimulatorClientAPI_NoGUI::loadURDF(
...
bool b3RobotSimulatorClientAPI_NoGUI::loadMJCF(
...
bool b3RobotSimulatorClientAPI_NoGUI::loadBullet(
    const std::string& fileName, b3RobotSimulatorLoadFileResults& results) {
  if (!isConnected()) {
    b3Warning("Not connected");
    return false;
...
bool b3RobotSimulatorClientAPI_NoGUI::loadSDF(
    const std::string& fileName, b3RobotSimulatorLoadFileResults& results,
...
bool b3RobotSimulatorClientAPI_NoGUI::getBodyInfo(int bodyUniqueId,
...
bool b3RobotSimulatorClientAPI_NoGUI::getBasePositionAndOrientation(
    int bodyUniqueId, b3Vector3& basePosition,
    b3Quaternion& baseOrientation) const {
  if (!isConnected()) {
    b3Warning("Not connected");
    return false;
  }
...
bool b3RobotSimulatorClientAPI_NoGUI::resetBasePositionAndOrientation(
    int bodyUniqueId, b3Vector3& basePosition, b3Quaternion& baseOrientation) {
  if (!isConnected()) {
    b3Warning("Not connected");
    return false;
  }
...
bool b3RobotSimulatorClientAPI_NoGUI::getBaseVelocity(
    int bodyUniqueId, b3Vector3& baseLinearVelocity,
    b3Vector3& baseAngularVelocity) const {
  if (!isConnected()) {
    b3Warning("Not connected");
    return false;
  }
...
bool b3RobotSimulatorClientAPI_NoGUI::resetBaseVelocity(
    int bodyUniqueId, const b3Vector3& linearVelocity,
    const b3Vector3& angularVelocity) const {
  if (!isConnected()) {
    b3Warning("Not connected");
    return false;
  }
...
void b3RobotSimulatorClientAPI_NoGUI::setRealTimeSimulation(
...
bool b3RobotSimulatorClientAPI_NoGUI::getJointInfo(int bodyUniqueId,
                                                   int jointIndex,
...
int b3RobotSimulatorClientAPI_NoGUI::createConstraint(int parentBodyIndex,
...
int b3RobotSimulatorClientAPI_NoGUI::changeConstraint(int constraintId,
                                                      b3JointInfo* jointInfo) {
  if (!isConnected()) {
    b3Warning("Not connected");
    return -1;
...
bool b3RobotSimulatorClientAPI_NoGUI::getJointState(
...
bool b3RobotSimulatorClientAPI_NoGUI::getJointStates(int bodyUniqueId,
...
bool b3RobotSimulatorClientAPI_NoGUI::resetJointState(int bodyUniqueId,
                                                      int jointIndex,
                                                      double targetValue) {
  if (!isConnected()) {
    b3Warning("Not connected");
...
void b3RobotSimulatorClientAPI_NoGUI::setJointMotorControl(
...
void b3RobotSimulatorClientAPI_NoGUI::setNumSolverIterations(
    int numIterations) {
  if (!isConnected()) {
    b3Warning("Not connected");
    return;
...
void b3RobotSimulatorClientAPI_NoGUI::setContactBreakingThreshold(
    double threshold) {
  if (!isConnected()) {
    b3Warning("Not connected");
    return;
...
void b3RobotSimulatorClientAPI_NoGUI::setNumSimulationSubSteps(
    int numSubSteps) {
  if (!isConnected()) {
    b3Warning("Not connected");
    return;
...
bool b3RobotSimulatorClientAPI_NoGUI::getBodyJacobian(
...
bool b3RobotSimulatorClientAPI_NoGUI::getLinkState(int bodyUniqueId,
                                                   int linkIndex,
                                                   b3LinkState* linkState) {
  bool computeLinkVelocity = true;
  bool computeForwardKinematics = true;

  return getLinkState(bodyUniqueId, linkIndex, computeLinkVelocity,
                      computeForwardKinematics, linkState);
}
...
void b3RobotSimulatorClientAPI_NoGUI::configureDebugVisualizer(
...
void b3RobotSimulatorClientAPI_NoGUI::getVREvents(b3VREventsData* vrEventsData,
...
void b3RobotSimulatorClientAPI_NoGUI::getKeyboardEvents(
...
int b3RobotSimulatorClientAPI_NoGUI::startStateLogging(
    b3StateLoggingType loggingType, const std::string& fileName,
    const b3AlignedObjectArray<int>& objectUniqueIds, int maxLogDof) {
  if (!isConnected()) {
    b3Warning("Not connected");
    return -1;
  }
...
void b3RobotSimulatorClientAPI_NoGUI::stopStateLogging(
...
void b3RobotSimulatorClientAPI_NoGUI::resetDebugVisualizerCamera(
    double cameraDistance, double cameraPitch, double cameraYaw,
    const b3Vector3& targetPos) {
  if (!isConnected()) {
    b3Warning("Not connected");
    return;
  }
...
void b3RobotSimulatorClientAPI_NoGUI::submitProfileTiming(
    const std::string& profileName, int durationInMicroSeconds) {
  if (!isConnected()) {
    b3Warning("Not connected");
    return;
  }
...
void b3RobotSimulatorClientAPI_NoGUI::loadSoftBody(const std::string& fileName,
...
void b3RobotSimulatorClientAPI_NoGUI::getMouseEvents(
...
bool b3RobotSimulatorClientAPI_NoGUI::calculateInverseDynamics(
...
bool b3RobotSimulatorClientAPI_NoGUI::getDynamicsInfo(
...
bool b3RobotSimulatorClientAPI_NoGUI::changeDynamics(
...
int b3RobotSimulatorClientAPI_NoGUI::addUserDebugParameter(char* paramName,
                                                           double rangeMin,
                                                           double rangeMax,
                                                           double startValue) {
  b3PhysicsClientHandle sm = m_data->m_physicsClientHandle;
...
double b3RobotSimulatorClientAPI_NoGUI::readUserDebugParameter(
    int itemUniqueId) {
  b3PhysicsClientHandle sm = m_data->m_physicsClientHandle;
  if (sm == 0) {
    b3Warning("Not connected to physics server.");
...
int b3RobotSimulatorClientAPI_NoGUI::addUserDebugText(
    char* text, double* posXYZ,
  ...
  commandHandle = b3InitUserDebugDrawAddText3D(
      sm, text, posXYZ, &args.m_colorRGB[0], args.m_size, args.m_lifeTime);
...
int b3RobotSimulatorClientAPI_NoGUI::addUserDebugLine(
...
int b3RobotSimulatorClientAPI_NoGUI::addUserDebugLine(
    b3Vector3& fromXYZ, b3Vector3& toXYZ,
    struct b3RobotSimulatorAddUserDebugLineArgs& args) {
  double dfromXYZ[3];
  double dtoXYZ[3];
...
bool b3RobotSimulatorClientAPI_NoGUI::setJointMotorControlArray(
...
bool b3RobotSimulatorClientAPI_NoGUI::setPhysicsEngineParameter(
...
bool b3RobotSimulatorClientAPI_NoGUI::applyExternalForce(int objectUniqueId,
                                                         int linkIndex,
                                                         double* force,
                                                         double* position,
                                                         int flags) {
...
bool b3RobotSimulatorClientAPI_NoGUI::applyExternalTorque(int objectUniqueId,
                                                          int linkIndex,
                                                          double* torque,
                                                          int flags) {
  b3PhysicsClientHandle sm = m_data->m_physicsClientHandle;
...
bool b3RobotSimulatorClientAPI_NoGUI::enableJointForceTorqueSensor(
...
bool b3RobotSimulatorClientAPI_NoGUI::getDebugVisualizerCamera(
...
bool b3RobotSimulatorClientAPI_NoGUI::getContactPoints(
...
bool b3RobotSimulatorClientAPI_NoGUI::getClosestPoints(
...
bool b3RobotSimulatorClientAPI_NoGUI::getOverlappingObjects(
...
bool b3RobotSimulatorClientAPI_NoGUI::getOverlappingObjects(
    b3Vector3& aabbMin, b3Vector3& aabbMax,
...
bool b3RobotSimulatorClientAPI_NoGUI::getAABB(int bodyUniqueId, int linkIndex,
                                              double* aabbMin,
                                              double* aabbMax) {
  b3PhysicsClientHandle sm = m_data->m_physicsClientHandle;
  if (sm == 0) {
...
int b3RobotSimulatorClientAPI_NoGUI::createCollisionShape(
...
int b3RobotSimulatorClientAPI_NoGUI::createMultiBody(
  ...
  scalarToDouble4(args.m_baseInertialFrameOrientation.m_floats,
  ...
  for (int i = 0; i < args.m_numLinks; i++) {
    ...
    b3Quaternion linkInertialFrameOrientation =
    ...
    scalarToDouble4(linkInertialFrameOrientation.m_floats,
...
void b3RobotSimulatorClientAPI_NoGUI::setGuiHelper(
