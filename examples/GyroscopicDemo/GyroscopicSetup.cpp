...
struct GyroscopicSetup : public CommonRigidBodyBase
{
	...
	void resetCamera()
	{
		...
		float pitch = -16;
		float yaw = 180;
		float targetPos[3] = {-2.4f, 0.4f, -0.24f};
...
void GyroscopicSetup::initPhysics()
{
	...
	for (int i = 0; i < 4; i++)
	{
		btCylinderShapeZ* pin = new btCylinderShapeZ(btVector3(btScalar(0.1), btScalar(0.1), btScalar(0.2)));
		btBoxShape* box = new btBoxShape(btVector3(1, btScalar(0.1), btScalar(0.1)));
		box->setMargin(btScalar(0.01));
		pin->setMargin(btScalar(0.01));
		btCompoundShape* compound = new btCompoundShape();
		compound->addChildShape(btTransform::getIdentity(), pin);
		btTransform offsetBox(btMatrix3x3::getIdentity(), btVector3(0, 0, btScalar(0.2)));
		...
		body->setAngularVelocity(btVector3(0, btScalar(0.1), 10));  //51));
