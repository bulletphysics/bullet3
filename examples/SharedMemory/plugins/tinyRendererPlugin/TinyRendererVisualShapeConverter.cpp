...
void convertURDFToVisualShape(
    ...
    b3VisualShapeData& visualShapeOut) {
  ...
  if (visual->m_geometry.m_hasLocalMaterial) {
    visualShapeOut.m_rgbaColor[0] =
        visual->m_geometry.m_localMaterial.m_matColor.m_rgbaColor[0];
    visualShapeOut.m_rgbaColor[1] =
        visual->m_geometry.m_localMaterial.m_matColor.m_rgbaColor[1];
    visualShapeOut.m_rgbaColor[2] =
        visual->m_geometry.m_localMaterial.m_matColor.m_rgbaColor[2];
    visualShapeOut.m_rgbaColor[3] =
        visual->m_geometry.m_localMaterial.m_matColor.m_rgbaColor[3];
  }
...
void TinyRendererVisualShapeConverter::convertVisualShapes(
  ...
  if (linkPtr) {
    ...
    for (int v1 = 0; v1 < cnt; v1++) {
      ...
      int colorIndex =
          linkIndex;  // colObj? colObj->getBroadphaseHandle()->getUid() & 3 :
                      // 0;
      ...
      if (visualsPtr == 0) {
        m_data->m_swRenderInstances.insert(collisionObjectUniqueId,
...

      {
        ...
        convertURDFToVisualShape(vis, pathPrefix,
                                 localInertiaFrame.inverse() * childTrans,
                                 vertices, indices, textures, visualShape);
      }
