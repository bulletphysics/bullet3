...
B3_SHARED_API int preTickPluginCallback(struct b3PluginContext* context) {
...

  {
    b3SharedMemoryCommandHandle commandHandle =
        b3RequestVREventsCommandInit(context->m_physClient);
    ...
    if (statusType == CMD_REQUEST_VR_EVENTS_DATA_COMPLETED) {
      ...
      if (vrEvents.m_numControllerEvents) {
        // this is only for a test, normally you wouldn't print to the console
        // at each simulation substep!
  ...
  }
  {
    ...
    b3SharedMemoryCommandHandle commandHandle =
        b3RequestKeyboardEventsCommandInit(context->m_physClient);
    ...
    if (keyboardEventsData.m_numKeyboardEvents) {
      // this is only for a test, normally you wouldn't print to the console at
      // each simulation substep!
...

  {
    ...
    b3SharedMemoryCommandHandle commandHandle =
        b3RequestMouseEventsCommandInit(context->m_physClient);
    b3SubmitClientCommandAndWaitStatus(context->m_physClient, commandHandle);
