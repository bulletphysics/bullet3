...
struct UrdfLinkNameMapUtil {
  ...
  virtual ~UrdfLinkNameMapUtil() { delete m_memSerializer; }
};
...
#else
            fread(&cmd->m_updateFlags, sizeof(int), 1, m_file);
            fread(&cmd->m_sdfRequestInfoArgs, sizeof(SdfRequestInfoArgs), 1,
                  m_file);
#endif
            result = true;
            break;
...
#else
            fread(&cmd->m_updateFlags, sizeof(int), 1, m_file);
            fread(&cmd->m_requestVisualShapeDataArguments,
                  sizeof(RequestVisualShapeDataArgs), 1, m_file);
#endif
            result = true;
            break;
...
#else
            fread(&cmd->m_updateFlags, sizeof(int), 1, m_file);
            fread(&cmd->m_requestActualStateInformationCommandArgument,
                  sizeof(RequestActualStateArgs), 1, m_file);
#endif
            result = true;
            break;
...
#else
            fread(&cmd->m_updateFlags, sizeof(int), 1, m_file);
            fread(&cmd->m_sendDesiredStateCommandArgument,
                  sizeof(SendDesiredStateArgs), 1, m_file);
...
#else
            fread(&cmd->m_updateFlags, sizeof(int), 1, m_file);
            fread(&cmd->m_requestContactPointArguments,
                  sizeof(RequestContactDataArgs), 1, m_file);
...
struct MinitaurStateLogger : public InternalStateLogger {
  ...
  virtual void logState(btScalar timeStep) {
    if (m_logFileHandle) {
      ...
      // (potential delay) better to fill a huge memory buffer and once in a
      // while write it to disk
...
#ifdef STATIC_LINK_VR_PLUGIN
      m_pluginManager.registerStaticLinkedPlugin(
          "vrSyncPlugin", initPlugin_vrSyncPlugin, exitPlugin_vrSyncPlugin,
          executePluginCommand_vrSyncPlugin, preTickPluginCallback_vrSyncPlugin,
          0, 0);
#endif  // STATIC_LINK_VR_PLUGIN

#ifndef SKIP_STATIC_TINYRENDERER_PLUGIN
      int renderPluginId = m_pluginManager.registerStaticLinkedPlugin(
          "tinyRendererPlugin", initPlugin_tinyRendererPlugin,
          exitPlugin_tinyRendererPlugin,
          executePluginCommand_tinyRendererPlugin, 0, 0,
          getRenderInterface_tinyRendererPlugin);
...
void PhysicsServerCommandProcessor::processCollisionForces(btScalar timeStep) {
  ...
  // will be exposed in C-API/pybullet. audio will go into a wav file, as well as
  // real-time output to speakers/headphones using RtAudio/DAC.
...
struct ProgrammaticUrdfInterface : public URDFImporterInterface {
  ...
  ProgrammaticUrdfInterface(const b3CreateMultiBodyArgs& bodyArgs,
      ...
      : m_bodyUniqueId(-1), m_createBodyArgs(bodyArgs), m_data(data) {}
    ...
    if (m_createBodyArgs.m_linkVisualShapeUniqueIds[linkIndex] >= 0) {
      ...
      if (visHandle) {
        if (visHandle->m_visualShape.m_geometry.m_hasLocalMaterial) {
          matCol =
              visHandle->m_visualShape.m_geometry.m_localMaterial.m_matColor;
          return true;
        }
      }
    }
...
void PhysicsServerCommandProcessor::createEmptyDynamicsWorld() {
  ...
  // m_data->m_broadphase = new btSimpleBroadphase(maxProxies,
  // m_data->m_pairCache);
  ...
  // todo: islands/constraints are buggy in btMultiBodyDynamicsWorld!
  // (performance + see slipping grasp)
...
#ifndef SKIP_SOFT_BODY_MULTI_BODY_DYNAMICS_WORLD
    for (i = m_data->m_dynamicsWorld->getSoftBodyArray().size() - 1; i >= 0;
         i--) {
  ...
  for (int m = 0; m < u2b.getNumModels(); m++) {
    ...
    u2b.setBodyUniqueId(bodyUniqueId);
    {
      ...
      u2b.getMassAndInertia(urdfLinkIndex, mass, localInertiaDiagonal,
                            bodyHandle->m_rootLocalInertialFrame);
    }

    // todo: move these internal API called inside the 'ConvertURDF2Bullet'
    // call, hidden from the user int rootLinkIndex = u2b.getRootLinkIndex();
...
bool PhysicsServerCommandProcessor::loadMjcf(const char* fileName,
  ...
  BulletMJCFImporter u2b(m_data->m_guiHelper,
                         m_data->m_pluginManager.getRenderInterface(), flags);
...
int PhysicsServerCommandProcessor::createBodyInfoStream(
  ...
  if (mb) {
    ...
    util->m_mb = mb;
    util->m_memSerializer = new btDefaultSerializer(
        bufferSizeInBytes, (unsigned char*)bufferServerToClient);
    ...
    if (mb->getBaseName()) {
      util->m_memSerializer->registerNameForPointer(mb->getBaseName(),
                                                    mb->getBaseName());
    }
    ...
    for (int i = 0; i < mb->getNumLinks(); i++) {
      // disable serialization of the collision objects
      util->m_memSerializer->m_skipPointers.insert(mb->getLink(i).m_collider,
                                                   0);
      util->m_memSerializer->registerNameForPointer(mb->getLink(i).m_linkName,
                                                    mb->getLink(i).m_linkName);
      util->m_memSerializer->registerNameForPointer(mb->getLink(i).m_jointName,
    ...
    const char* structType =
        mb->serialize(chunk->m_oldPtr, util->m_memSerializer);
    util->m_memSerializer->finalizeChunk(chunk, structType, BT_MULTIBODY_CODE,
                                         mb);
    ...
    if (rb) {
      ...
      util->m_memSerializer = new btDefaultSerializer(
          bufferSizeInBytes, (unsigned char*)bufferServerToClient);
      util->m_memSerializer->startSerialization();
      util->m_memSerializer->registerNameForPointer(
          bodyHandle->m_rigidBody, bodyHandle->m_bodyName.c_str());
      // disable serialization of the collision objects (they are too big, and
      // the client likely doesn't need them);
      for (int i = 0; i < bodyHandle->m_rigidBodyJoints.size(); i++) {
        ...
        util->m_memSerializer->registerNameForPointer(
            con, bodyHandle->m_rigidBodyJointNames[i].c_str());
        util->m_memSerializer->registerNameForPointer(
            &con->getRigidBodyB(), bodyHandle->m_rigidBodyLinkNames[i].c_str());
        ...
        btChunk* chunk = util->m_memSerializer->allocate(len, 1);
        const char* structType =
            con->serialize(chunk->m_oldPtr, util->m_memSerializer);
        util->m_memSerializer->finalizeChunk(chunk, structType,
                                             BT_CONSTRAINT_CODE, (void*)con);
      }
    ...
    if ((clientCmd.m_updateFlags & ER_BULLET_HARDWARE_OPENGL) != 0) {
      ...
      if (numPixelsCopied > 0) {
        ...
        m_data->m_guiHelper->debugDisplayCameraImageData(
            viewMat, projMat, pixelRGBA, numRequestedPixels, depthBuffer,
            numRequestedPixels, 0, numRequestedPixels, startPixelIndex, width,
...
bool PhysicsServerCommandProcessor::processSaveWorldCommand(
  ...
  /// authoring many todo's, for example save the state of motor controllers etc.
  ...
  for (int i = 0; i < clientCmd.m_createUserShapeArgs.m_numUserShapes; i++) {
    ...
    switch (clientCmd.m_createUserShapeArgs.m_shapes[i].m_type) {
      ...
      case GEOM_MESH: {
        ...
        pathPrefix[0] = 0;
        if (b3ResourcePath::findResourcePath(fileName.c_str(), relativeFileName,
                                             1024)) {
        ...
        bool foundFile = findExistingMeshFile(pathPrefix, relativeFileName,
        ...
        if (foundFile) {
          ...
          if (out_type == UrdfGeometry::FILE_OBJ) {
            ...
            if (clientCmd.m_createUserShapeArgs.m_shapes[i].m_collisionFlags &
              ...
              std::string err =
                  tinyobj::LoadObj(shapes, out_found_filename.c_str());

              // shape = createConvexHullFromShapes(shapes,
              // collision->m_geometry.m_meshScale); static btCollisionShape*
              ...
              for (int s = 0; s < (int)shapes.size(); s++) {
                ...
                for (int f = 0; f < faceCount; f += 3) {
                  btVector3 pt;
                  pt.setValue(
                      shape.mesh.positions[shape.mesh.indices[f] * 3 + 0],
...
bool PhysicsServerCommandProcessor::processCreateVisualShapeCommand(
  ...
  if (clientCmd.m_createUserShapeArgs.m_numUserShapes == 1) {
    ...
    u2b.convertURDFToVisualShapeInternal(
        &visualShape, pathPrefix, localInertiaFrame.inverse() * childTrans,
        ...
        int graphicsIndex = -1;
        {
          ...
          if (graphicsIndex >= 0) {
            int visualShapeUniqueId =
                m_data->m_userVisualShapeHandles.allocHandle();
            ...
            // tinyrenderer doesn't separate shape versus instance, so create it
            // when creating the multibody instance store needed info for
            // tinyrenderer
...
bool PhysicsServerCommandProcessor::processUserDebugDrawCommand(
  ...
  if (clientCmd.m_updateFlags & USER_DEBUG_HAS_TEXT) {
    ...
    // textColorRGB[3], double size, double lifeTime, int trackingObjectUniqueId,
    // int optionFlags){return -1;}
...
bool PhysicsServerCommandProcessor::processRequestRaycastIntersectionsCommand(
  ...
  for (int ray = 0; ray < clientCmd.m_requestRaycastIntersections.m_numRays;
       ray++) {
    btVector3 rayFromWorld(
        clientCmd.m_requestRaycastIntersections.m_rayFromPositions[ray][0],
    ...
    btVector3 rayToWorld(
        clientCmd.m_requestRaycastIntersections.m_rayToPositions[ray][0],
    ...
    btCollisionWorld::ClosestRayResultCallback rayResultCallback(rayFromWorld,
                                                                 rayToWorld);
    m_data->m_dynamicsWorld->rayTest(rayFromWorld, rayToWorld,
                                     rayResultCallback);
    ...
    if (rayResultCallback.hasHit()) {
      ...
      serverStatusOut.m_raycastHits
          .m_rayHits[serverStatusOut.m_raycastHits.m_numRaycastHits]
          .m_hitObjectUniqueId = -1;
      serverStatusOut.m_raycastHits
          .m_rayHits[serverStatusOut.m_raycastHits.m_numRaycastHits]
          .m_hitObjectLinkIndex = -1;
      serverStatusOut.m_raycastHits.m_rayHits[rayHits].m_hitPositionWorld[0] =
          0;
      serverStatusOut.m_raycastHits.m_rayHits[rayHits].m_hitPositionWorld[1] =
          0;
...
bool PhysicsServerCommandProcessor::processSyncBodyInfoCommand(
  ...
  for (int i = 0; i < usedHandles.size(); i++) {
    ...
    if (body && (body->m_multiBody || body->m_rigidBody)) {
      serverStatusOut.m_sdfLoadedArgs.m_bodyUniqueIds[actualNumBodies++] =
          usedHandle;
    }
  }
  ...
  for (int i = 0; i < usz; i++) {
    ...
    //						int uid =
    //m_data->m_userConstraints.getAtIndex(i)->m_userConstraintData.m_userConstraintUniqueId;
                    ...
                    if (linearLowerLimit.isZero() &&
                      ...
                      switch (clientCmd.m_sendDesiredStateCommandArgument
                        ...
                        case CONTROL_MODE_POSITION_VELOCITY_PD: {
                          if (hasDesiredPosOrVel) {
                            ...
                            // position. the maximum velocity is limited by
                            // maxMotorForce
                      ...
                      switch (clientCmd.m_sendDesiredStateCommandArgument
                        ...
                        case CONTROL_MODE_VELOCITY: {
                          ...
                          // this is max motor force impulse
                          btScalar torqueImpulse =
                              torque * m_data->m_dynamicsWorld->getSolverInfo()
                                           .m_timeStep;
  ...
  if (body && body->m_multiBody) {
    ...
    for (int l = 0; l < mb->getNumLinks(); l++) {
      ...
      for (int d = 0; d < mb->getLink(l).m_dofCount; d++) {
        serverCmd.m_sendActualStateArgs
            .m_actualStateQdot[totalDegreeOfFreedomU++] =
      ...
      if (0 == mb->getLink(l).m_jointFeedback) {
        ...
        serverCmd.m_sendActualStateArgs.m_jointReactionForces[l * 6 + 0] =
            sensedForce[0];
        serverCmd.m_sendActualStateArgs.m_jointReactionForces[l * 6 + 1] =
            sensedForce[1];
        serverCmd.m_sendActualStateArgs.m_jointReactionForces[l * 6 + 2] =
            sensedForce[2];

        serverCmd.m_sendActualStateArgs.m_jointReactionForces[l * 6 + 3] =
            sensedTorque[0];
        serverCmd.m_sendActualStateArgs.m_jointReactionForces[l * 6 + 4] =
            sensedTorque[1];
        serverCmd.m_sendActualStateArgs.m_jointReactionForces[l * 6 + 5] =
            sensedTorque[2];
      }
      ...
      if (supportsJointMotor(mb, l)) {
        ...
        if (motor && m_data->m_physicsDeltaTime > btScalar(0)) {
          ...
          serverCmd.m_sendActualStateArgs.m_jointMotorForce[l] = force;
          // if (force>0)
          //{
          //   b3Printf("force = %f\n", force);
          //}
      ...
      serverCmd.m_sendActualStateArgs.m_linkState[l * 7 + 0] =
          linkCOMOrigin.getX();
      serverCmd.m_sendActualStateArgs.m_linkState[l * 7 + 1] =
          linkCOMOrigin.getY();
      serverCmd.m_sendActualStateArgs.m_linkState[l * 7 + 2] =
          linkCOMOrigin.getZ();
      serverCmd.m_sendActualStateArgs.m_linkState[l * 7 + 3] =
          linkCOMRotation.x();
      serverCmd.m_sendActualStateArgs.m_linkState[l * 7 + 4] =
          linkCOMRotation.y();
      serverCmd.m_sendActualStateArgs.m_linkState[l * 7 + 5] =
          linkCOMRotation.z();
      serverCmd.m_sendActualStateArgs.m_linkState[l * 7 + 6] =
          linkCOMRotation.w();
      ...
      serverCmd.m_sendActualStateArgs.m_linkWorldVelocities[l * 6 + 0] =
          worldLinVel[0];
      serverCmd.m_sendActualStateArgs.m_linkWorldVelocities[l * 6 + 1] =
          worldLinVel[1];
      serverCmd.m_sendActualStateArgs.m_linkWorldVelocities[l * 6 + 2] =
          worldLinVel[2];
      serverCmd.m_sendActualStateArgs.m_linkWorldVelocities[l * 6 + 3] =
          worldAngVel[0];
      serverCmd.m_sendActualStateArgs.m_linkWorldVelocities[l * 6 + 4] =
          worldAngVel[1];
      serverCmd.m_sendActualStateArgs.m_linkWorldVelocities[l * 6 + 5] =
          worldAngVel[2];

      serverCmd.m_sendActualStateArgs.m_linkLocalInertialFrames[l * 7 + 0] =
          linkLocalInertialOrigin.getX();
      serverCmd.m_sendActualStateArgs.m_linkLocalInertialFrames[l * 7 + 1] =
          linkLocalInertialOrigin.getY();
      serverCmd.m_sendActualStateArgs.m_linkLocalInertialFrames[l * 7 + 2] =
          linkLocalInertialOrigin.getZ();

      serverCmd.m_sendActualStateArgs.m_linkLocalInertialFrames[l * 7 + 3] =
          linkLocalInertialRotation.x();
      serverCmd.m_sendActualStateArgs.m_linkLocalInertialFrames[l * 7 + 4] =
          linkLocalInertialRotation.y();
      serverCmd.m_sendActualStateArgs.m_linkLocalInertialFrames[l * 7 + 5] =
          linkLocalInertialRotation.z();
      serverCmd.m_sendActualStateArgs.m_linkLocalInertialFrames[l * 7 + 6] =
    ...
    if (body && body->m_rigidBody) {
      ...
      // base angular velocity (in world space, carthesian)
      serverCmd.m_sendActualStateArgs.m_actualStateQdot[3] =
          rb->getAngularVelocity()[0];
      serverCmd.m_sendActualStateArgs.m_actualStateQdot[4] =
          rb->getAngularVelocity()[1];
      serverCmd.m_sendActualStateArgs.m_actualStateQdot[5] =
          rb->getAngularVelocity()[2];
      totalDegreeOfFreedomU += 6;  // 3 linear and 3 angular DOF
      ...
      case CONTACT_QUERY_MODE_REPORT_EXISTING_CONTACT_POINTS: {
        ...
        for (int i = 0; i < numContactManifolds; i++) {
          ...
          for (int p = 0; p < manifold->getNumContacts(); p++) {
            ...
            //                                    pt.m_linearFrictionForce =
            //                                    srcPt.m_appliedImpulseLateral1;
...

        {
          ...
          struct MyContactResultCallback
            ...
            virtual bool needsCollision(btBroadphaseProxy* proxy0) const {
              ...
              // m_collisionFilterMask) != 0; collides = collides &&
              // (m_collisionFilterGroup & proxy0->m_collisionFilterMask); return
              // collides;
              return true;
            }
...
bool PhysicsServerCommandProcessor::processCreateMultiBodyCommand(
  ...
  if (clientCmd.m_createMultiBodyArgs.m_baseLinkIndex >= 0) {
    ...
    int flags = 0;
    bool ok =
        processImportedObjects("memory", bufferServerToClient,
                               bufferSizeInBytes, useMultiBody, flags, u2b);
...
bool PhysicsServerCommandProcessor::processLoadSoftBodyCommand(
...

  {
    ...
    char pathPrefix[1024];
    pathPrefix[0] = 0;
    if (b3ResourcePath::findResourcePath(loadSoftBodyArgs.m_fileName,
    ...
    int out_type;
    bool foundFile =
        findExistingMeshFile(pathPrefix, relativeFileName, error_message_prefix,
                             &out_found_filename, &out_type);
    std::vector<tinyobj::shape_t> shapes;
    ...
    if (shapes.size() > 0) {
      ...
      if (numTris > 0) {
        btSoftBody* psb = btSoftBodyHelpers::CreateFromTriMesh(
            m_data->m_softBodyWorldInfo, &vertices[0], &indices[0], numTris);
...
bool PhysicsServerCommandProcessor::processForwardDynamicsCommand(
  ...
  if (m_data->m_numSimulationSubSteps > 0) {
    m_data->m_dynamicsWorld->stepSimulation(
  ...
  if (body && body->m_multiBody) {
    ...
    if (body && body->m_rigidBody) {
      if (clientCmd.m_updateFlags & CHANGE_DYNAMICS_INFO_SET_LINEAR_DAMPING) {
        ...
        body->m_rigidBody->setDamping(
            clientCmd.m_changeDynamicsInfoArgs.m_linearDamping, angDamping);
      }
      if (clientCmd.m_updateFlags & CHANGE_DYNAMICS_INFO_SET_ANGULAR_DAMPING) {
        btScalar linDamping = body->m_rigidBody->getLinearDamping();
        body->m_rigidBody->setDamping(
            linDamping, clientCmd.m_changeDynamicsInfoArgs.m_angularDamping);
      }

      if (clientCmd.m_updateFlags &
          CHANGE_DYNAMICS_INFO_SET_CONTACT_STIFFNESS_AND_DAMPING) {
        body->m_rigidBody->setContactStiffnessAndDamping(
            clientCmd.m_changeDynamicsInfoArgs.m_contactStiffness,
            clientCmd.m_changeDynamicsInfoArgs.m_contactDamping);
      }
      if (clientCmd.m_updateFlags & CHANGE_DYNAMICS_INFO_SET_RESTITUTION) {
      ...
      if (clientCmd.m_updateFlags & CHANGE_DYNAMICS_INFO_SET_FRICTION_ANCHOR) {
        if (clientCmd.m_changeDynamicsInfoArgs.m_frictionAnchor) {
          body->m_rigidBody->setCollisionFlags(
              body->m_rigidBody->getCollisionFlags() |
              btCollisionObject::CF_HAS_FRICTION_ANCHOR);
        } else {
          body->m_rigidBody->setCollisionFlags(
              body->m_rigidBody->getCollisionFlags() &
      ...
      if (clientCmd.m_updateFlags & CHANGE_DYNAMICS_INFO_SET_MASS) {
        ...
        if (body->m_rigidBody->getCollisionShape()) {
          body->m_rigidBody->getCollisionShape()->calculateLocalInertia(
              mass, localInertia);
        }
...
bool PhysicsServerCommandProcessor::processInitPoseCommand(
  ...
  if (body && body->m_multiBody) {
    ...
    if (clientCmd.m_updateFlags & INIT_POSE_HAS_JOINT_STATE) {
      ...
      for (int i = 0; i < mb->getNumLinks(); i++) {
        if ((clientCmd.m_initPoseArgs.m_hasInitialStateQ[posVarCountIndex]) &&
            (mb->getLink(i).m_dofCount == 1)) {
          mb->setJointPos(
              i, clientCmd.m_initPoseArgs.m_initialStateQ[posVarCountIndex]);
          mb->setJointVel(i, 0);  // backwards compatibility
        }
        if ((clientCmd.m_initPoseArgs.m_hasInitialStateQdot[uDofIndex]) &&
            (mb->getLink(i).m_dofCount == 1)) {
...
bool PhysicsServerCommandProcessor::processInverseDynamicsCommand(
  ...
  if (bodyHandle && bodyHandle->m_multiBody) {
    ...
    if (tree) {
      ...
      btInverseDynamics::vecx nu(num_dofs + baseDofs),
          qdot(num_dofs + baseDofs), q(num_dofs + baseDofs),
          joint_force(num_dofs + baseDofs);
      for (int i = 0; i < num_dofs; i++) {
        q[i + baseDofs] =
            clientCmd.m_calculateInverseDynamicsArguments.m_jointPositionsQ[i];
        qdot[i + baseDofs] = clientCmd.m_calculateInverseDynamicsArguments
                                 .m_jointVelocitiesQdot[i];
        nu[i + baseDofs] = clientCmd.m_calculateInverseDynamicsArguments
            ...
            if (m_data->m_pluginManager.getRenderInterface()) {
              m_data->m_pluginManager.getRenderInterface()->removeVisualShape(
                  bodyHandle->m_multiBody->getLink(link)
                      .m_collider->getBroadphaseHandle()
                      ->getUid());
            }
            m_data->m_dynamicsWorld->removeCollisionObject(
...
bool PhysicsServerCommandProcessor::processCalculateInverseKinematicsCommand(
  ...
  if (bodyHandle && bodyHandle->m_multiBody) {
    ...
    int endEffectorLinkIndex =
        clientCmd.m_calculateInverseKinematicsArguments.m_endEffectorLinkIndex;
    ...
    if (ikHelperPtr &&
      ...
      if (tree && ((numDofs + baseDofs) == tree->numDoFs())) {
        ...
        btInverseDynamics::vecx nu(numDofs + baseDofs),
            qdot(numDofs + baseDofs), q(numDofs + baseDofs),
        ...
        for (int i = 0; i < bodyHandle->m_multiBody->getNumLinks(); ++i) {
          if (bodyHandle->m_multiBody->getLink(i).m_jointType >= 0 &&
              bodyHandle->m_multiBody->getLink(i).m_jointType <=
                  2) {  // 0, 1, 2 represent revolute, prismatic, and spherical
                        // joint types respectively. Skip the fixed joints.
        ...
        if ((clientCmd.m_updateFlags & IK_HAS_TARGET_ORIENTATION) &&
          ...
          // Nullspace task only works with DLS now. TODO: add nullspace task to
          // SDLS.
        ...
        if (clientCmd.m_updateFlags & IK_HAS_NULL_SPACE_VELOCITY) {
          ...
          for (int i = 0; i < numDofs; ++i) {
            lower_limit[i] =
                clientCmd.m_calculateInverseKinematicsArguments.m_lowerLimit[i];
            upper_limit[i] =
                clientCmd.m_calculateInverseKinematicsArguments.m_upperLimit[i];
            joint_range[i] =
                clientCmd.m_calculateInverseKinematicsArguments.m_jointRange[i];
            rest_pose[i] =
        ...
        btTransform endEffectorTransformWorld =
            bodyHandle->m_multiBody->getLink(endEffectorLinkIndex)
                .m_cachedWorldTransform *
            bodyHandle->m_linkLocalInertialFrames[endEffectorLinkIndex]
                .inverse();

        btVector3DoubleData endEffectorWorldPosition;
        btQuaternionDoubleData endEffectorWorldOrientation;

        btVector3 endEffectorPosWorldOrg =
            endEffectorTransformWorld.getOrigin();
        btQuaternion endEffectorOriWorldOrg =
            endEffectorTransformWorld.getRotation();
        ...
        btVector4 endEffectorOri(
            endEffectorOriBaseCoord.x(), endEffectorOriBaseCoord.y(),
            endEffectorOriBaseCoord.z(), endEffectorOriBaseCoord.w());
        ...
        btVector3 targetPosWorld(
            clientCmd.m_calculateInverseKinematicsArguments.m_targetPosition[0],
        ...
        btQuaternion targetOrnWorld(
            ...
            clientCmd.m_calculateInverseKinematicsArguments
                .m_targetOrientation[1],
            clientCmd.m_calculateInverseKinematicsArguments
                .m_targetOrientation[2],
            clientCmd.m_calculateInverseKinematicsArguments
                .m_targetOrientation[3]);
...
bool PhysicsServerCommandProcessor::processRequestVisualShapeInfoCommand(
  ...
  if (m_data->m_pluginManager.getRenderInterface()) {
    ...
    int remain =
        ...
        clientCmd.m_requestVisualShapeDataArguments.m_startingVisualShapeIndex;
    int shapeIndex =
        clientCmd.m_requestVisualShapeDataArguments.m_startingVisualShapeIndex;
          ...
          if (linkIndex < bodyHandle->m_multiBody->getNumLinks()) {
            if (bodyHandle->m_multiBody->getLink(linkIndex).m_collider) {
              ...
              if (clientCmd.m_updateFlags & CMD_UPDATE_VISUAL_SHAPE_TEXTURE) {
                if (texHandle) {
                  int shapeIndex =
                      m_data->m_guiHelper->getShapeIndexFromInstance(
                          graphicsIndex);
                  m_data->m_guiHelper->replaceTexture(
                      shapeIndex, texHandle->m_openglTextureId);
                }
...
bool PhysicsServerCommandProcessor::processLoadTextureCommand(
  ...
  if (b3ResourcePath::findResourcePath(
    ...
    if (texH) {
      ...
      int uid = -1;
      if (m_data->m_pluginManager.getRenderInterface()) {
        m_data->m_pluginManager.getRenderInterface()->loadTextureFile(
            relativeFileName);
      }
      if (uid >= 0) {
...

      {
        ...
        unsigned char* imageData =
            stbi_load(relativeFileName, &width, &height, &n, 3);
...
bool PhysicsServerCommandProcessor::processRestoreStateCommand(
  ...
  if (clientCmd.m_loadStateArguments.m_stateId >= 0) {
    ...
    for (int i = 0; !f && i < numPrefixes; i++) {
      sprintf(relativeFileName, "%s%s", prefix[i],
...
#ifndef SKIP_SOFT_BODY_MULTI_BODY_DYNAMICS_WORLD
      for (int i = 0; i < m_data->m_dynamicsWorld->getSoftBodyArray().size();
           i++) {
        btSoftBody* psb =
            (btSoftBody*)m_data->m_dynamicsWorld->getSoftBodyArray()[i];
        if (m_data->m_dynamicsWorld->getDebugDrawer() &&
          ...
          btSoftBodyHelpers::Draw(psb,
                                  m_data->m_dynamicsWorld->getDebugDrawer(),
                                  m_data->m_dynamicsWorld->getDrawFlags());
        }
      }
#endif
    }
  ...
  if (rayCallback.hasHit()) {
    ...
    if (body) {
      ...
      if (multiCol && multiCol->m_multiBody) {
        ...
        // velocities, simulation 'explodes' see also
        // http://www.bulletphysics.org/Bullet/phpBB3/viewtopic.php?f=4&t=949 so
        // we try to avoid it by clamping the maximum impulse (force) that the
        // mouse pick can apply it is not satisfying, hopefully we find a better
        // solution (higher order integrator, using joint friction using a
        // zero-velocity target motor with limited force etc?)
        btScalar scaling = 1;
...
void PhysicsServerCommandProcessor::stepSimulationRealTime(
  ...
  if ((m_data->m_useRealTimeSimulation) && m_data->m_guiHelper) {
    ...
    int numSteps = m_data->m_dynamicsWorld->stepSimulation(
        dtInSec * simTimeScalingFactor, maxSteps, gSubStep);
