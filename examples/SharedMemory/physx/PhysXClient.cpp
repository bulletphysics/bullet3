...
struct BodyJointInfoCache2
{
	...
	btHashMap<btHashInt, PhysXUserDataCache> m_jointToUserDataMap;
...
struct PhysXDirectInternalData
{
...
// return true if connection succesfull, can also check 'isConnected'
bool PhysXClient::connect(struct GUIHelperInterface* guiHelper)
{
	bool connected = m_data->m_commandProcessor->connect();
...
void PhysXClient::debugDraw(int debugDrawMode)
{
	m_data->m_commandProcessor->physicsDebugDraw(debugDrawMode);
}

////todo: rename to 'disconnect'
void PhysXClient::disconnectSharedMemory()
{
	m_data->m_commandProcessor->disconnect();
	m_data->m_commandProcessor->setGuiHelper(0);
}

bool PhysXClient::isConnected() const
{
	return m_data->m_commandProcessor->isConnected();
}

// return non-null if there is a status, nullptr otherwise
const SharedMemoryStatus* PhysXClient::processServerStatus()
{
...
SharedMemoryCommand* PhysXClient::getAvailableSharedMemoryCommand()
{
	return &m_data->m_command;
}

bool PhysXClient::canSubmitCommand() const
{
	return m_data->m_commandProcessor->isConnected();
}

bool PhysXClient::processDebugLines(const struct SharedMemoryCommand& orgCommand)
{
...
bool PhysXClient::processVisualShapeData(const struct SharedMemoryCommand& orgCommand)
{
	SharedMemoryCommand command = orgCommand;
	const SharedMemoryStatus& serverCmd = m_data->m_serverStatus;
...
bool PhysXClient::processOverlappingObjects(const struct SharedMemoryCommand& orgCommand)
{
	SharedMemoryCommand command = orgCommand;
...
bool PhysXClient::processContactPointData(const struct SharedMemoryCommand& orgCommand)
{
	SharedMemoryCommand command = orgCommand;
...
bool PhysXClient::processCamera(const struct SharedMemoryCommand& orgCommand)
{
	SharedMemoryCommand command = orgCommand;
...
void PhysXClient::processBodyJointInfo(int bodyUniqueId, const SharedMemoryStatus& serverCmd)
{
...
void PhysXClient::processAddUserData(const struct SharedMemoryStatus& serverCmd)
{
	
}

void PhysXClient::postProcessStatus(const struct SharedMemoryStatus& serverCmd)
{
...
bool PhysXClient::submitClientCommand(const struct SharedMemoryCommand& command)
{
...
int PhysXClient::getNumBodies() const
{
	return m_data->m_bodyJointMap.size();
}

void PhysXClient::removeCachedBody(int bodyUniqueId)
{
...
int PhysXClient::getNumUserConstraints() const
{
	return m_data->m_userConstraintInfoMap.size();
}

int PhysXClient::getUserConstraintInfo(int constraintUniqueId, struct b3UserConstraint& info) const
{
...
int PhysXClient::getUserConstraintId(int serialIndex) const
{
...
int PhysXClient::getBodyUniqueId(int serialIndex) const
{
...
bool PhysXClient::getBodyInfo(int bodyUniqueId, struct b3BodyInfo& info) const
{
	BodyJointInfoCache2** bodyJointsPtr = m_data->m_bodyJointMap[bodyUniqueId];
	if (bodyJointsPtr && *bodyJointsPtr)
	{
...
int PhysXClient::getNumJoints(int bodyIndex) const
{
...
bool PhysXClient::getJointInfo(int bodyIndex, int jointIndex, struct b3JointInfo& info) const
{
	BodyJointInfoCache2** bodyJointsPtr = m_data->m_bodyJointMap[bodyIndex];
	if (bodyJointsPtr && *bodyJointsPtr)
	{
...
void PhysXClient::setSharedMemoryKey(int key)
{
}

void PhysXClient::uploadBulletFileToSharedMemory(const char* data, int len)
{
...
void PhysXClient::uploadRaysToSharedMemory(struct SharedMemoryCommand& command, const double* rayFromWorldArray, const double* rayToWorldArray, int numRays)
{
...
int PhysXClient::getNumDebugLines() const
{
	return m_data->m_debugLinesFrom.size();
}

const float* PhysXClient::getDebugLinesFrom() const
{
	if (getNumDebugLines())
	{
...
const float* PhysXClient::getDebugLinesTo() const
{
	if (getNumDebugLines())
	{
...
const float* PhysXClient::getDebugLinesColor() const
{
	if (getNumDebugLines())
	{
...
void PhysXClient::getCachedCameraImage(b3CameraImageData* cameraData)
{
	if (cameraData)
	{
...
void PhysXClient::getCachedContactPointInformation(struct b3ContactInformation* contactPointData)
{
...
void PhysXClient::getCachedOverlappingObjects(struct b3AABBOverlapData* overlappingObjects)
{
...
void PhysXClient::getCachedVisualShapeInformation(struct b3VisualShapeInformation* visualShapesInfo)
{
...
void PhysXClient::getCachedCollisionShapeInformation(struct b3CollisionShapeInformation* collisionShapesInfo)
{
...
void PhysXClient::getCachedVREvents(struct b3VREventsData* vrEventsData)
{
...
void PhysXClient::getCachedKeyboardEvents(struct b3KeyboardEventsData* keyboardEventsData)
{
...
void PhysXClient::getCachedMouseEvents(struct b3MouseEventsData* mouseEventsData)
{
...
void PhysXClient::getCachedRaycastHits(struct b3RaycastInformation* raycastHits)
{
...
void PhysXClient::getCachedMassMatrix(int dofCountCheck, double* massMatrix)
{
...
void PhysXClient::setTimeOut(double timeOutInSeconds)
{
	m_data->m_timeOutInSeconds = timeOutInSeconds;
}

double PhysXClient::getTimeOut() const
{
	return m_data->m_timeOutInSeconds;
}


void PhysXClient::pushProfileTiming(const char* timingName)
{
...
void PhysXClient::popProfileTiming()
{
