...
/// made in the structures my convention is year/month/day/rev
...
struct b3JointSensorState {
  ...
  double m_jointForceTorque[6]; /* note to roboticists: this is NOT the motor
...
struct b3VRControllerEvent {
  ...
  float m_auxAnalogAxis[MAX_VR_ANALOG_AXIS *
                        2];  // store x,y per axis, only valid if
...
struct b3VisualShapeData {
  ...
  double m_localVisualFrame[7];  // pos[3], orn[4]
                                 // todo: add more data if necessary (material
                                 // color etc, although material can be in asset
                                 // file .obj file)
...
/// coordinates center of mass (COM) of the link, relative to the world reference
/// frame. Orientation is a quaternion x,y,z,w Note: to compute the URDF link
/// frame (which equals the joint frame at joint position 0) use URDF link frame
/// = link COM frame * inertiaFrame.inverse()
struct b3LinkState {
