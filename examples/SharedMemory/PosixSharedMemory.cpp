...
PosixSharedMemory::~PosixSharedMemory() { delete m_internalData; }
...
void PosixSharedMemory::releaseSharedMemory(int key, int size) {
  ...
  if (0 == seg) {
    b3Error(
        "PosixSharedMemory::releaseSharedMemory: shared memory key not found");
    return;
  }

  if (seg->m_sharedMemoryId < 0) {
    b3Error(
        "PosixSharedMemory::releaseSharedMemory: shared memory id is not set");
  } else {
    if (seg->m_createdSharedMemory) {
      ...
      if (result == -1) {
        ...
        b3Printf(
            "PosixSharedMemory::releaseSharedMemory removed shared memory");
      }
