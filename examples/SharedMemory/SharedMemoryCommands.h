...
/// position No motors or controls are needed to initialize the pose. It is
/// similar to moving a robot to a starting place, while it is switched off. It
/// is only called at the start of a robot control session. All velocities and
/// control forces are cleared to zero.
struct InitPoseArgs {
  int m_bodyUniqueId;
...
/// controllers. The control mode determines the state variables used for motor
/// control.
struct SendDesiredStateArgs {
  ...
  // CONTROL_MODE_TORQUE) or or the maximum applied force/torque for the
  // PD/motor/constraint to reach the desired velocity in CONTROL_MODE_VELOCITY
  // and CONTROL_MODE_POSITION_VELOCITY_PD mode indexed by degree of freedom, 6
  // dof base, and then dofs for each link
