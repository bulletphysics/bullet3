...
struct b3PluginManagerInternalData {
  ...
  b3PluginManagerInternalData() : m_activeRendererPluginUid(-1) {}
};
...
int b3PluginManager::loadPlugin(const char* pluginPath,
  ...
  // for now, automatically select the loaded plugin as active renderer. If
  // wanted, we can add some 'select' mechanism.
  if (pluginUniqueId >= 0) {
    b3PluginHandle* plugin = m_data->m_plugins.getHandle(pluginUniqueId);
...
int b3PluginManager::registerStaticLinkedPlugin(
    const char* pluginPath, PFN_INIT initFunc, PFN_EXIT exitFunc,
    PFN_EXECUTE executeCommandFunc, PFN_TICK preTickFunc, PFN_TICK postTickFunc,
    PFN_GET_RENDER_INTERFACE getRendererFunc) {
  b3Plugin orgPlugin;
