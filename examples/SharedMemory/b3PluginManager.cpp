...
int b3PluginManager::loadPlugin(const char* pluginPath, const char* postFixStr)
{
	...
	else
	{
		...
		if (pluginHandle)
		{
			...
			*(void**)&plugin->m_initFunc = (void*)B3_DYNLIB_IMPORT(pluginHandle, initStr.c_str());
			*(void**)&plugin->m_exitFunc = (void*)B3_DYNLIB_IMPORT(pluginHandle, exitStr.c_str());
			*(void**)&plugin->m_executeCommandFunc = (void*)B3_DYNLIB_IMPORT(pluginHandle, executePluginCommandStr.c_str());
			*(void**)&plugin->m_preTickFunc = (void*)B3_DYNLIB_IMPORT(pluginHandle, preTickPluginCallbackStr.c_str());
			*(void**)&plugin->m_postTickFunc = (void*)B3_DYNLIB_IMPORT(pluginHandle, postTickPluginCallback.c_str());
			*(void**)&plugin->m_processNotificationsFunc = (void*)B3_DYNLIB_IMPORT(pluginHandle, processNotificationsStr.c_str());

			if (plugin->m_processNotificationsFunc)
			{
				m_data->m_numNotificationPlugins++;
			}
			*(void**)&plugin->m_processClientCommandsFunc = (void*)B3_DYNLIB_IMPORT(pluginHandle, processClientCommandsStr.c_str());

			*(void**)&plugin->m_getRendererFunc = (void*)B3_DYNLIB_IMPORT(pluginHandle, getRendererStr.c_str());
			*(void**)&plugin->m_getCollisionFunc = (void*)B3_DYNLIB_IMPORT(pluginHandle, getCollisionStr.c_str());
			*(void**)&plugin->m_getFileIOFunc = (void*)B3_DYNLIB_IMPORT(pluginHandle, getFileIOStr.c_str());
