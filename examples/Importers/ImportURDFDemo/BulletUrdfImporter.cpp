      ...
      switch (collision->m_geometry.m_meshFileType) {
        case UrdfGeometry::FILE_OBJ:
          if (collision->m_flags & URDF_FORCE_CONCAVE_TRIMESH) {
            ...
            glmesh = LoadMeshFromObj(
                collision->m_geometry.m_meshFileName.c_str(), pathPrefix);
          } else {
            std::vector<tinyobj::shape_t> shapes;
            std::string err = tinyobj::LoadObj(
                shapes, collision->m_geometry.m_meshFileName.c_str());
            // create a convex hull for each shape, and store it in a
            // btCompoundShape

            shape = createConvexHullFromShapes(
                shapes, collision->m_geometry.m_meshScale);
...
void BulletURDFImporter::convertURDFToVisualShapeInternal(
    ...
    btAlignedObjectArray<BulletURDFTexture>& texturesOut) const {
...
int BulletURDFImporter::convertLinkVisualShapes(
  ...
  if (vertices.size() && indices.size()) {
    ...
    //vertices.size(), &indices[0], indices.size()); graphicsIndex  =
    // m_data->m_guiHelper->registerGraphicsShape(&vertices[0].xyzw[0],
