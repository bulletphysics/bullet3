...
/// URDF-based robots. There is an implementation in
...
struct UrdfRenderingInterface {
  ...
  /// (loading graphics meshes, textures etc) use the collisionObjectUid as a
  /// unique identifier to synchronize the world transform and to remove the
  /// visual shape.
  virtual void convertVisualShapes(int linkIndex, const char* pathPrefix,
                                   const btTransform& localInertiaFrame,
                                   const UrdfLink* linkPtr,
                                   const UrdfModel* model,
                                   int collisionObjectUid,
                                   int bodyUniqueId) = 0;
