...
public:
    ...
    MyURDF2Bullet(my_shared_ptr<ModelInterface> robot,struct GUIHelperInterface* helper)
    :m_robot(robot),
    m_guiHelper(helper),
    m_bulletMultiBody(0)
    {
    ...
    virtual int convertLinkVisualShapes(int linkIndex, const char* pathPrefix, const btTransform& inertialFrame) const
    {
        ...
        {
            graphicsIndex  = m_guiHelper->registerGraphicsShape(&vertices[0].xyzw[0], vertices.size(), &indices[0], indices.size());
        }
...
void URDFvisual2BulletCollisionShape(my_shared_ptr<const Link> link, struct GUIHelperInterface* helper, const btTransform& parentTransformInWorldSpace, btMultiBodyDynamicsWorld* world1, URDF2BulletMappings& mappings, const char* pathPrefix)
{
    //btCollisionShape* shape = 0;
...

    {
...

		{
			...
			if (vertices.size() && indices.size())
			{
				graphicsIndex  = helper->registerGraphicsShape(&vertices[0].xyzw[0], vertices.size(), &indices[0], indices.size());
			}
			...
			if (compoundShape)
			{
				...
				if (!mappings.m_createMultiBody)
				{
					...
					helper->createRigidBodyGraphicsObject(body, color);
				...
				if (mappings.m_createMultiBody)
				{
					if (compoundShape->getNumChildShapes()>0)
					{
						...
						helper->createCollisionObjectGraphicsObject(col,color);
    ...
    for (std::vector<my_shared_ptr<Link> >::const_iterator child = link->child_links.begin(); child != link->child_links.end(); child++)
    {
        if (*child)
        {
            URDFvisual2BulletCollisionShape(*child,helper, linkTransformInWorldSpace, world1,mappings,pathPrefix);
...
void ImportUrdfSetup::initPhysics()
{
    ...
    MyURDF2Bullet u2b(robot,m_guiHelper);
    printTree(u2b, 0,0);
    ...
    
    {
        ...
        {
            mappings.m_createMultiBody = m_useMultiBody;
            ...
            URDFvisual2BulletCollisionShape(root_link, m_guiHelper, identityTrans,m_dynamicsWorld,mappings,pathPrefix);
        ...
        } else
        {
            ...
            ConvertURDF2Bullet(u2b,identityTrans,m_dynamicsWorld,m_useMultiBody,pathPrefix);
