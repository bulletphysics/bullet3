...
struct SDFAudioSource {
  ...
  double m_gain;  // normalized volume in range [0..1] where 0 is silent, 1 is
                  // most loud
  ...
  double m_collisionForceThreshold;  // force that will trigger the audio, in
                                     // Newton. If < 0, audio source is invalid
