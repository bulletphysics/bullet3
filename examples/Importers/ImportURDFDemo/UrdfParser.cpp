...
UrdfParser::UrdfParser()
    : m_parseSDF(false), m_activeSdfModel(-1), m_urdfScaling(1) {
  m_urdf2Model.m_sourceFile =
      "IN_MEMORY_STRING";  // if loadUrdf() called later, source file name will
...
bool UrdfParser::parseMaterial(UrdfMaterial& material, TiXmlElement* config,
                               ErrorLogger* logger) {
...
bool UrdfParser::parseTransform(btTransform& tr, TiXmlElement* xml,
...
bool UrdfParser::parseInertia(UrdfInertia& inertia, TiXmlElement* config,
                              ErrorLogger* logger) {
...
bool UrdfParser::parseGeometry(UrdfGeometry& geom, TiXmlElement* g,
                               ErrorLogger* logger) {
  //	btAssert(g);
  if (g == 0) return false;
...
bool UrdfParser::parseCollision(UrdfCollision& collision, TiXmlElement* config,
                                ErrorLogger* logger) {
...
bool UrdfParser::parseVisual(UrdfModel& model, UrdfVisual& visual,
                             TiXmlElement* config, ErrorLogger* logger) {
...
bool UrdfParser::parseLink(UrdfModel& model, UrdfLink& link,
                           TiXmlElement* config, ErrorLogger* logger) {
...
// //http://sdformat.org/spec?ver=1.6&elem=link
    ...
    if (ci) {
      ...
      if (TiXmlElement* attack_rate_xml =
              ci->FirstChildElement("attack_rate")) {
      ...
      if (TiXmlElement* forceThreshold_xml =
...
bool UrdfParser::parseJointLimits(UrdfJoint& joint, TiXmlElement* config,
                                  ErrorLogger* logger) {
...
bool UrdfParser::parseJointDynamics(UrdfJoint& joint, TiXmlElement* config,
                                    ErrorLogger* logger) {
...
bool UrdfParser::parseJoint(UrdfJoint& joint, TiXmlElement* config,
                            ErrorLogger* logger) {
  // Get Joint Name
