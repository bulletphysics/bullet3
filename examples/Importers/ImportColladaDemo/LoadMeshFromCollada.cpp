...
void readFloatArray(TiXmlElement* source,
  ...
  if (array) {
    ...
    if (source->FirstChildElement("technique_common")
            ...
            ->QueryIntAttribute("stride", &stride) != TIXML_NO_ATTRIBUTE) {
...
void readLibraryGeometries(
  ...
  for (TiXmlElement* geometry = doc.RootElement()
    ...
    for (TiXmlElement* mesh = geometry->FirstChildElement("mesh");
         (mesh != NULL); mesh = mesh->NextSiblingElement("mesh")) {
      ...
      VertexSource vs;
      for (TiXmlElement* input = vertices2->FirstChildElement("input");
      ...
      for (TiXmlElement* primitive = mesh->FirstChildElement("triangles");
        ...
        int numIndices = 0;
        {
          for (TiXmlElement* input = primitive->FirstChildElement("input");
               input != NULL; input = input->NextSiblingElement("input")) {
            const char* sem = input->Attribute("semantic");
            std::string semName(sem);
...
void readNodeHierarchy(
    TiXmlElement* node, btHashMap<btHashString, int>& name2Shape,
  ...
  {
    for (TiXmlElement* transElem = node->FirstChildElement("matrix"); transElem;
  ...
  {
    for (TiXmlElement* transElem = node->FirstChildElement("translate");
  ...
  for (TiXmlElement* instanceGeom =
           node->FirstChildElement("instance_geometry");
       instanceGeom != 0;
...
void readVisualSceneInstanceGeometries(
  ...
  TiXmlElement* scene = 0;
  {
    ...
    if (scenes) {
      TiXmlElement* instanceSceneReference =
  ...
  if (scene) {
    for (TiXmlElement* node = scene->FirstChildElement("node"); node != NULL;
