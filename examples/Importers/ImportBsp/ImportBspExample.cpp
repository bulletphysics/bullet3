...
char* makeExeToBspFilename(const char* lpCmdLine) {
  ...
  // DOM which expects all paths to be URI's.  This block of code does some
  // conversion to try and make the input compliant without breaking the ability
  // to accept a properly formatted URI.  Right now this only displays the first
  // filename
