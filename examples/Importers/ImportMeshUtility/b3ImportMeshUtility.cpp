...
bool b3ImportMeshUtility::loadAndRegisterMeshFromFileInternal(
  ...
  if (b3ResourcePath::findResourcePath(fileName.c_str(), relativeFileName,
    ...
    std::vector<tinyobj::shape_t> shapes;
    {
      ...
      std::string err =
          LoadFromCachedOrFromObj(shapes, relativeFileName, pathPrefix);
      // std::string err = tinyobj::LoadObj(shapes, relativeFileName,
      // pathPrefix);
    }

    GLInstanceGraphicsShape* gfxShape =
        btgCreateGraphicsShapeFromWavefrontObj(shapes);
    {
      ...
      for (int i = 0; meshData.m_textureImage1 == 0 && i < shapes.size(); i++) {
        ...
        if (shape.material.diffuse_texname.length() > 0) {
          ...
          for (int i = 0; !image && i < numprefix; i++) {
            ...
            if (b3ResourcePath::findResourcePath(relativeFileName,
                                                 relativeFileName2, 1024)) {
