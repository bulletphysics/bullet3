...
struct BulletMJCFImporterInternalData {
  ...
  void parseAssets(TiXmlElement* root_xml, MJCFErrorLogger* logger) {
    ...
    for (TiXmlElement* child_xml = root_xml->FirstChildElement(); child_xml;
  ...
  bool parseDefaults(MyMJCFDefaults& defaults, TiXmlElement* root_xml,
                     MJCFErrorLogger* logger) {
    bool handled = false;
  ...
  bool parseRootLevel(MyMJCFDefaults& defaults, TiXmlElement* root_xml,
                      MJCFErrorLogger* logger) {
    for (TiXmlElement* rootxml = root_xml->FirstChildElement(); rootxml;
  ...
  bool parseJoint(MyMJCFDefaults& defaults, TiXmlElement* link_xml,
    ...
    // the diagonal of the inertia matrix in generalized coordinates. They make
    // the simulation more stable, and often increase physical realism. This is
    // because when a motor is attached to the system with a transmission that
    // amplifies the motor force by c, the inertia of the rotor (i.e. the moving
    // part of the motor) is amplified by c*c. The same holds for gears in the
    // early stages of planetary gear boxes. These extra inertias often dominate
    // the inertias of the robot parts that are represented explicitly in the
  ...
  bool parseGeom(MyMJCFDefaults& defaults, TiXmlElement* link_xml,
                 int modelIndex, int linkIndex, MJCFErrorLogger* logger,
        ...
        if (meshStr) {
          ...
          if (assetPtr) {
            ...
            bool exists = findExistingMeshFile(
                m_sourceFileName, assetPtr->m_fileName,
  ...
  bool parseBody(MyMJCFDefaults& defaults, TiXmlElement* link_xml,
                 int modelIndex, int orgParentLinkIndex,
                 MJCFErrorLogger* logger) {
    ...
    for (TiXmlElement* xml = link_xml->FirstChildElement(); xml;
...
bool BulletMJCFImporter::loadMJCF(const char* fileName, MJCFErrorLogger* logger,
  ...
  bool fileFound =
      (b3ResourcePath::findResourcePath(fileName, relativeFileName, 1024) > 0);
...
bool BulletMJCFImporter::parseMJCFString(const char* xmlText,
  ...
  for (TiXmlElement* link_xml = mujoco_xml->FirstChildElement("default");
       link_xml; link_xml = link_xml->NextSiblingElement("default")) {
    m_data->parseDefaults(m_data->m_globalDefaults, link_xml, logger);
  }

  for (TiXmlElement* link_xml = mujoco_xml->FirstChildElement("compiler");
       link_xml; link_xml = link_xml->NextSiblingElement("compiler")) {
    m_data->parseCompiler(link_xml, logger);
  }

  for (TiXmlElement* link_xml = mujoco_xml->FirstChildElement("asset");
       link_xml; link_xml = link_xml->NextSiblingElement("asset")) {
    m_data->parseAssets(link_xml, logger);
  }

  for (TiXmlElement* link_xml = mujoco_xml->FirstChildElement("body"); link_xml;
       link_xml = link_xml->NextSiblingElement("body")) {
    m_data->parseRootLevel(m_data->m_globalDefaults, link_xml, logger);
  }

  for (TiXmlElement* link_xml = mujoco_xml->FirstChildElement("worldbody");
...
// implement a custom collision filter instead (contype1 & conaffinity2) ||
...
void BulletMJCFImporter::convertLinkVisualShapes2(
  ...
  if (m_data->m_customVisualShapesConverter) {
    ...
    m_data->m_customVisualShapesConverter->convertVisualShapes(
        ...
        colObj->getBroadphaseHandle()->getUid(), objectIndex);
  }
}
...
static btCollisionShape* MjcfCreateConvexHullFromShapes(
    std::vector<tinyobj::shape_t>& shapes, const btVector3& geomScale,
      ...
      switch (col->m_geometry.m_type) {
        ...
        case URDF_GEOM_MESH: {
          ...
          if (childShape) {
            ...
            // glmesh->m_numvertices,fullPath); int shapeId =
