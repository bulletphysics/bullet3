...
class GwenUserInterface {
 ...
 public:
  ...
  void init(int width, int height, Gwen::Renderer::Base* gwenRenderer,
            float retinaScale);
  void exit();
  void setFocus();
  ...
  void registerComboBox2(int buttonId, int numItems, const char** items,
                         int startItem = 0);
  ...
  GwenInternalData* getInternalData() { return m_data; }
};
