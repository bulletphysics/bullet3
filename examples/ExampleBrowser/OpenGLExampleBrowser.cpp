...
struct MyMenuItemHander : public Gwen::Event::Handler {
  ...
  void onButtonC(Gwen::Controls::Base* pControl) {
    ...
    GetText(); Gwen::String laa = Gwen::Utility::UnicodeToString(la); const
    char* ha = laa.c_str();
  ...
  void onButtonD(Gwen::Controls::Base* pControl) {
    /*		Gwen::Controls::Label* label = (Gwen::Controls::Label*)
       pControl; Gwen::UnicodeString la = label->GetText();//
       node->GetButton()->GetName();// GetText(); Gwen::String laa =
       Gwen::Utility::UnicodeToString(la); const char* ha = laa.c_str();
                    */
...
bool OpenGLExampleBrowser::init(int argc, char* argv[]) {
  ...
  /// GTX 680 with up-to-date drivers you could give it a try Note that several
  /// old OpenCL physics examples still have to be ported over to this new
  /// Example Browser
  ...
  if (gui2) {
    ...
    for (int d = 0; d < numDemos; d++) {
              ...
              d))  // was test for gAllExamples[d].m_menuLevel==1
      {
        ...
        //&MyMenuItemHander::onButtonD); 		pNode->onNamePress.Add(handler,
        //&MyMenuItemHander::onButtonD);
        //			pNode->onKeyboardPressed.Add(handler,
        //&MyMenuItemHander::onButtonD); 			pNode->OnKeyPress
      } else {
