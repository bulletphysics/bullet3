...
struct MyHashShape {
  ...
  bool equals(const MyHashShape& other) const {
    ...
    return sameShapeType && sameSphere0 && sameSphere1 && sameRadius0 &&
           sameRadius1 && sameTransform && sameUpAxis && sameHalfHeight;
  }
  // to our success
...
void OpenGLGuiHelper::copyCameraImageData(
  ...
  int sourceWidth = m_data->m_glApp->m_window->getWidth() *
                    m_data->m_glApp->m_window->getRetinaScale();
  int sourceHeight = m_data->m_glApp->m_window->getHeight() *
                     m_data->m_glApp->m_window->getRetinaScale();
