...
static btScalar gPendulaQty =
    2;  // TODO: This would actually be an Integer, but the Slider does not like
...
struct MultiPendulumExample : public CommonRigidBodyBase {
  ...
  virtual void createMultiPendulum(
      btSphereShape* colShape, btScalar pendulaQty, const btVector3& position,
      btScalar length,
      btScalar mass);  // create a multi pendulum at the indicated x and y
                       // position, the specified number of pendula formed into
                       // a chain, each with indicated length and mass
...
static MultiPendulumExample* mex =
    NULL;  // Handle to the example to access it via functions. Do not use this
           // in your simulation!
...
void MultiPendulumExample::createMultiPendulum(btSphereShape* colShape,
  ...
  btRigidBody* parentSphere =
      topSphere;  // set the top sphere as the parent sphere for the next sphere
                  // to be created
  ...
  for (int i = 0; i < pendulaQty; i++) {  // produce the number of pendula
    ...
    // this is represented by the constraint pivot in the local frames of
    // reference of both constrained spheres furthermore we need to rotate the
    // constraint appropriately to orient it correctly in space
    ...
    // dimensions, therefore we set both limits directly to the current position
    // of the parentSphere
