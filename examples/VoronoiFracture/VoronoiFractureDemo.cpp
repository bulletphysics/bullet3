...
struct pointCmp {
  bool operator()(const btVector3& p1, const btVector3& p2) const {
    ...
    // ((btScalar)(p2-curVoronoiPoint).length2()); apparently result0 is not
    // always result1, because extended precision used in registered is different
    // from precision when values are stored in memory
    return result0;
  }
};
...
void VoronoiFractureDemo::voronoiBBShatter(
  ...
  for (i = 0; i < numpoints; i++) {
    ...
    // produce some visible cracks. Use the mesh in convexHC for visual display
    // or to perform boolean operations with.
