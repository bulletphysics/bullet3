...
struct b3ParamsGridBroadphaseCL {
  ...
  int getMaxBodiesPerCell() const { return m_gridSize[3]; }

  void setMaxBodiesPerCell(int maxOverlap) { m_gridSize[3] = maxOverlap; }
};
...
class b3GpuGridBroadphase : public b3GpuBroadphaseInterface {
 ...
 public:
  ...
  static b3GpuBroadphaseInterface* CreateFunc(cl_context ctx,
                                              cl_device_id device,
                                              cl_command_queue q) {
