            ...
            for (int j = prevMinIndex; j != curMinIndex; j += stepMin) {
              ...
              if (otherIndex != i) {
                ...
                if (otherIsMax) {
                  ...
                  // b3Vector3&)m_allAabbsCPU[otherIndex].m_max); bool
                  // prevOverlap = TestAabbAgainstAabb2((const
...
void b3GpuSapBroadphase::writeAabbsToGpu() {
  ...
  m_allAabbsGPU.copyFromHost(
      m_allAabbsCPU);  // might not be necessary, the 'setupGpuAabbsFull'
