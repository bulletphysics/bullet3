...
void b3GpuParallelLinearBvh::build(
  ...
  // AABBs, it is necessary to separate the AABBs so that the large AABBs will
  // not degrade the quality of the BVH.
  {
  ...
  if (numLeaves < 2) {
    ...
    // testRaysAgainstBvhAabbs(), so it does not matter if numLeaves == 0 and
    // rootNodeIndex == -1
    ...
    // algorithm, m_mortonCodesAndAabbIndicies.m_value is used to map a sorted
    // AABB index to the unsorted AABB index instead of directly moving the
    // AABBs. It needs to be set for the ray cast traversal kernel to work. (
  ...
  // dimension).
  {
    ...
    m_mergedAabb.copyFromOpenCLArray(
        m_leafNodeAabbs);  // Need to make a copy since the kernel modifies the
                           // array
    ...
    for (int numAabbsNeedingMerge = numLeaves; numAabbsNeedingMerge >= 2;
      ...
      b3BufferInfoCL bufferInfo[] = {
          b3BufferInfoCL(
              m_mergedAabb.getBufferCL())  // Resulting AABB is stored in
                                           // m_mergedAabb[0]
      };
  ...
  // contiguous subset of leaf node indices. The root node contains leaf node
  // indices in the range [0, numLeafNodes - 1]. The child nodes of each node
  // split their parent's index range into 2 contiguous halves.
  //
  // For example, if the root has indices [0, 31], its children might partition
  // that range into [0, 11] and [12, 31]. The next level in the tree could then
  // split those ranges into [0, 2], [3, 11], [12, 22], and [23, 31].
  //
...
void b3GpuParallelLinearBvh::constructBinaryRadixTree() {
  ...
  // nodes to its left and right to find its potential parent nodes and assign
  // child node indices
  {
  ...
  // move up the tree towards the root to set the AABBs of each internal node;
  // each internal node checks its children and merges their AABBs
  {
    ...
    for (int distanceFromRoot = maxDistanceFromRoot; distanceFromRoot >= 0;
      ...
      // actually faster than determining the exact nodes that are ready to merge
      // their child AABBs.
      launcher.launch1D(numInternalNodes);
    }
