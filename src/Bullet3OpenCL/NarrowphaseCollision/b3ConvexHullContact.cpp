...
void computeContactCompoundCompound(
  ...
  int childColIndexA = gpuChildShapes[childShapeIndexA].m_shapeIndex;
                          ...
                          b3Quat ornA = rigidBodies[bodyIndexA].m_quat;
                          float4 childPosA =
  gpuChildShapes[childShapeIndexA].m_childPosition; b3Quat childOrnA =
  gpuChildShapes[childShapeIndexA].m_childOrientation; float4 newPosA =
  b3QuatRotate(ornA,childPosA)+posA; b3Quat newOrnA = b3QuatMul(ornA,childOrnA);
  ...
  */
  ...
  for (int c=0;c<numChildrenB;c++)
  {
          ...
          b3Vector3 childPosB =
  cpuChildShapes[childShapeIndexB].m_childPosition; b3Quaternion childOrnB =
  cpuChildShapes[childShapeIndexB].m_childOrientation; float4  posB =
  b3QuatRotate(rootOrnB,childPosB)+rootPosB; b3Quaternion ornB =
...
void GpuSatCollision::computeConvexConvexContactsGPUSAT(
...

  {
    ...
    if (findSeparatingAxisOnGpu) {
      ...
      executed b3AlignedObjectArray<int> checkHasSepAxis;
      ...
      for (int i=0;i<nPairs;i++)
      {
          ...
          {
              printf("at frameCount %d hostHasSepAxis[%d] = %d but
      checkHasSepAxis[i] = %d\n",
...

  {
    ...
    if (treeNodesGPU->size() && treeNodesGPU->size()) {
      ...
      if (numConcavePairs) {
        ...
        //cpuConcavePairs; 				triangleConvexPairsOut.copyToHost(cpuConcavePairs);
      }
    }
  }
  ...
  if (contactClippingOnGpu) {
    ...
    if (numConcavePairs) {
      //			printf("numConcavePairs = %d\n",
      //numConcavePairs); 		nContacts = m_totalContactsOut.at(0);
      ...
      if (breakupConcaveConvexKernel) {
        ...
        // contactReduction
        {
          ...
          if (reduceConcaveContactsOnGPU) {
            ...
            m_totalContactsOut.copyFromHostPointer(&nContacts, 1, 0, true);
            //						nContacts =
            //m_totalContactsOut.at(0); contactOut->resize(nContacts);
            hostContacts.resize(nContacts);
            // printf("contactOut4 (after newContactReductionKernel) =
            // %d\n",nContacts);
            contactOut->copyFromHost(hostContacts);
