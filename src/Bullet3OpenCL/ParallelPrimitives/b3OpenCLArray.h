...
class b3OpenCLArray {
 ...
 public:
  b3OpenCLArray(cl_context ctx, cl_command_queue queue,
                size_t initialCapacity = 0, bool allowGrowingCapacity = true)
      : m_size(0),
        m_capacity(0),
        m_clBuffer(0),
        m_clContext(ctx),
  ...
  // we could enable this assignment, but need to make sure to avoid accidental
  // deep copies
  ...
  cl_mem getBufferCL() const { return m_clBuffer; }
  ...
  B3_FORCE_INLINE size_t size() const { return m_size; }

  B3_FORCE_INLINE size_t capacity() const { return m_capacity; }
  ...
  B3_FORCE_INLINE bool reserve(size_t _Count, bool copyOldContents = true) {
    ...
    if (capacity() < _Count) {  // not enough room, reallocate
      ...
      if (m_allowGrowingCapacity) {
        ...
        cl_mem buf = clCreateBuffer(m_clContext, CL_MEM_READ_WRITE,
                                    memSizeInBytes, NULL, &ciErrNum);
...
//#define B3_ALWAYS_INITIALIZE_OPENCL_BUFFERS
        ...
        for (size_t i = 0; i < memSizeInBytes; i++) src[i] = 0xbb;
        ciErrNum = clEnqueueWriteBuffer(m_commandQueue, buf, CL_TRUE, 0,
                                        memSizeInBytes, src, 0, 0, 0);
        ...
        if (result) {
          if (copyOldContents) copyToCL(buf, size());
        }
  ...
  void copyToCL(cl_mem destination, size_t numElements, size_t firstElem = 0,
                size_t dstOffsetInElems = 0) const {
    ...
    status = clEnqueueCopyBuffer(m_commandQueue, m_clBuffer, destination,
                                 srcOffsetBytes, dstOffsetInBytes,
                                 sizeof(T) * numElements, 0, 0, 0);
  ...
  void copyFromHostPointer(const T* src, size_t numElems,
                           size_t destFirstElem = 0,
                           bool waitForCompletion = true) {
    ...
    if (numElems + destFirstElem) {
      ...
      status = clEnqueueWriteBuffer(m_commandQueue, m_clBuffer, 0,
                                    sizeof(T) * destFirstElem, sizeInBytes, src,
                                    0, 0, 0);
      b3Assert(status == CL_SUCCESS);
      if (waitForCompletion) clFinish(m_commandQueue);
    } else {
      b3Error("copyFromHostPointer invalid range\n");
    }
  }

  void copyToHost(b3AlignedObjectArray<T>& destArray,
                  bool waitForCompletion = true) const {
    destArray.resize(this->size());
    if (size()) copyToHostPointer(&destArray[0], size(), 0, waitForCompletion);
  }

  void copyToHostPointer(T* destPtr, size_t numElem, size_t srcFirstElem = 0,
                         bool waitForCompletion = true) const {
    b3Assert(numElem + srcFirstElem <= capacity());

    if (numElem + srcFirstElem <= capacity()) {
      cl_int status = 0;
      status = clEnqueueReadBuffer(m_commandQueue, m_clBuffer, 0,
                                   sizeof(T) * srcFirstElem,
                                   sizeof(T) * numElem, destPtr, 0, 0, 0);
      b3Assert(status == CL_SUCCESS);
