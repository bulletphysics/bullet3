...
static const char* prefixScanKernelsFloat4CL =
    ...
    "} ConstBuffer;\n"
    "*/\n"
    "float4 ScanExclusiveFloat4(__local float4* data, u32 n, int lIdx, int "
    "lSize)\n"
    "{\n"
    "	float4 blocksum;\n"
    "    int offset = 1;\n"
    ...
    "__kernel\n"
    "void LocalScanKernel(__global float4* dst, __global float4* src, __global "
    "float4* sumBuffer,	uint4 cb)\n"
    "{\n"
    "	__local float4 ldsData[WG_SIZE*2];\n"
    "	int gIdx = GET_GLOBAL_IDX;\n"
    "	int lIdx = GET_LOCAL_IDX;\n"
    "	ldsData[2*lIdx]     = ( 2*gIdx < cb.m_numElems )? src[2*gIdx]: 0;\n"
    "	ldsData[2*lIdx + 1] = ( 2*gIdx+1 < cb.m_numElems )? src[2*gIdx + 1]: "
    "0;\n"
    "	float4 sum = ScanExclusiveFloat4(ldsData, WG_SIZE*2, GET_LOCAL_IDX, "
    "GET_GROUP_SIZE);\n"
    "	if( lIdx == 0 ) \n"
    ...
    "__attribute__((reqd_work_group_size(WG_SIZE,1,1)))\n"
    "__kernel\n"
    "void AddOffsetKernel(__global float4* dst, __global float4* blockSum, "
    "uint4 cb)\n"
    "{\n"
    ...
    "	}\n"
    "	GROUP_LDS_BARRIER;\n"
    "	float4 sum = ScanExclusiveFloat4(ldsData, cb.m_numScanBlocks, "
    "GET_LOCAL_IDX, GET_GROUP_SIZE);\n"
