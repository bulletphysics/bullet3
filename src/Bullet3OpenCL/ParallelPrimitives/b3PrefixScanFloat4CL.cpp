...
#include "b3FillCL.h"
#define B3_PREFIXSCAN_FLOAT4_PROG_PATH \
...
b3PrefixScanFloat4CL::b3PrefixScanFloat4CL(cl_context ctx, cl_device_id device,
  ...
  cl_program scanProg = b3OpenCLUtils::compileCLProgramFromString(
      ctx, device, scanKernelSource, &pErrNum, additionalMacros,
      B3_PREFIXSCAN_FLOAT4_PROG_PATH);
  b3Assert(scanProg);

  m_localScanKernel = b3OpenCLUtils::compileCLKernelFromString(
      ctx, device, scanKernelSource, "LocalScanKernel", &pErrNum, scanProg,
      additionalMacros);
  b3Assert(m_localScanKernel);
  m_blockSumKernel = b3OpenCLUtils::compileCLKernelFromString(
      ctx, device, scanKernelSource, "TopLevelScanKernel", &pErrNum, scanProg,
      additionalMacros);
  b3Assert(m_blockSumKernel);
  m_propagationKernel = b3OpenCLUtils::compileCLKernelFromString(
      ctx, device, scanKernelSource, "AddOffsetKernel", &pErrNum, scanProg,
      additionalMacros);
...
T b3NextPowerOf2(T n) {
  n -= 1;
  for (int i = 0; i < sizeof(T) * 8; i++) n = n | (n >> i);
  return n + 1;
}

void b3PrefixScanFloat4CL::execute(b3OpenCLArray<b3Vector3>& src,
                                   b3OpenCLArray<b3Vector3>& dst, int n,
                                   b3Vector3* sum) {
  //	b3Assert( data->m_option == EXCLUSIVE );
  const unsigned int numBlocks =
      (const unsigned int)((n + BLOCK_SIZE * 2 - 1) / (BLOCK_SIZE * 2));
  ...
  {
    b3BufferInfoCL bInfo[] = {b3BufferInfoCL(dstNative->getBufferCL()),
                              b3BufferInfoCL(srcNative->getBufferCL()),
                              b3BufferInfoCL(m_workBuffer->getBufferCL())};
...
void b3PrefixScanFloat4CL::executeHost(b3AlignedObjectArray<b3Vector3>& src,
                                       b3AlignedObjectArray<b3Vector3>& dst,
                                       int n, b3Vector3* sum) {
