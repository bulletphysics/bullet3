...
b3LauncherCL::b3LauncherCL(cl_command_queue queue, cl_kernel kernel,
                           const char* name)
...
void b3LauncherCL::setBuffer(cl_mem clBuffer) {
  if (m_enableSerialization) {
    ...
    cl_int err;
    err = clGetMemObjectInfo(kernelArg.m_clBuffer, param_name, sizeInBytes,
                             &param_value, &actualSizeInBytes);
...
void b3LauncherCL::setBuffers(b3BufferInfoCL* buffInfo, int n) {
  for (int i = 0; i < n; i++) {
    ...
    cl_int status = clSetKernelArg(m_kernel, m_idx++, sizeof(cl_mem),
                                   &buffInfo[i].m_clBuffer);
    b3Assert(status == CL_SUCCESS);
  }
}
...
int b3LauncherCL::deserializeArgs(unsigned char* buf, int bufSize,
                                  cl_context ctx) {
  int index = 0;
  ...
  for (int i = 0; i < numArguments; i++) {
    ...
    if (arg->m_isBuffer) {
      b3OpenCLArray<unsigned char>* clData = new b3OpenCLArray<unsigned char>(
          ctx, m_commandQueue, arg->m_argSizeInBytes);
      ...
      cl_int status =
          clSetKernelArg(m_kernel, m_idx++, sizeof(cl_mem), &arg->m_clBuffer);
      b3Assert(status == CL_SUCCESS);
      index += arg->m_argSizeInBytes;
    } else {
      cl_int status = clSetKernelArg(m_kernel, m_idx++, arg->m_argSizeInBytes,
                                     &arg->m_argData);
      b3Assert(status == CL_SUCCESS);
    }
    b3KernelArgData b;
...
int b3LauncherCL::validateResults(unsigned char* goldBuffer,
                                  int goldBufferCapacity, cl_context ctx) {
  int index = 0;
  ...
  for (int ii = 0; ii < numArguments; ii++) {
    ...
    if (m_kernelArguments[ii].m_argSizeInBytes != argGold->m_argSizeInBytes) {
      printf(
          ...
          ii, argGold->m_argSizeInBytes,
          m_kernelArguments[ii].m_argSizeInBytes);
      return -2;
    }

    {
      ...
      if (expected != found) {
        printf(
            "failed validation: argument %d isBuffer expected: %d, found %d\n",
            ii, expected, found);
        return -3;
      }
    }
    ...
    if (argGold->m_isBuffer) {
      unsigned char* memBuf =
          (unsigned char*)malloc(m_kernelArguments[ii].m_argSizeInBytes);
      ...
      status = clEnqueueReadBuffer(
          m_commandQueue, m_kernelArguments[ii].m_clBuffer, CL_TRUE, 0,
          m_kernelArguments[ii].m_argSizeInBytes, memBuf, 0, 0, 0);
      b3Assert(status == CL_SUCCESS);
      ...
      for (int b = 0; b < m_kernelArguments[ii].m_argSizeInBytes; b++) {
        ...
        if (expected != found) {
          printf(
              "failed validation: argument %d OpenCL data at byte position %d "
              "expected: %d, found %d\n",
              ii, b, expected, found);
          return -4;
        }
      ...
      for (int b = 0; b < m_kernelArguments[ii].m_argSizeInBytes; b++) {
        ...
        if (expected != found) {
          printf(
              "failed validation: argument %d const data at byte position %d "
              "expected: %d, found %d\n",
              ii, b, expected, found);
          return -5;
        }
...
int b3LauncherCL::serializeArguments(unsigned char* destBuffer,
                                     int destBufferCapacity) {
  // initialize to known values
  for (int i = 0; i < destBufferCapacity; i++) destBuffer[i] = 0xec;
  ...
  for (int i = 0; i < this->m_kernelArguments.size(); i++) {
    ...
    *arg = m_kernelArguments[i];
    ...
    if (arg->m_isBuffer == 1) {
      ...
      cl_int status = 0;
      status = clEnqueueReadBuffer(m_commandQueue, arg->m_clBuffer, 0, 0,
                                   arg->m_argSizeInBytes,
