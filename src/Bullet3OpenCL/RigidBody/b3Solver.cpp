    ...
    /*		for (int bb=0;bb<m_maxNumBatches;bb++)
                    {
                            ...
                            for(int ic=0; ic<m_nConstraints; ic++)
                            {
                                    ...
                                    float frictionCoeff =
       m_constraints[i].getFrictionCoeff(); int aIdx =
       (int)m_constraints[i].m_bodyA; int bIdx = (int)m_constraints[i].m_bodyB;
                                    int localBatch =
       m_constraints[i].m_batchIdx; b3RigidBodyData& bodyA = m_bodies[aIdx];
                                    b3RigidBodyData& bodyB = m_bodies[bIdx];

                                    if( !m_solveFriction )
                                    {
                                            float maxRambdaDt[4] =
       {FLT_MAX,FLT_MAX,FLT_MAX,FLT_MAX}; float minRambdaDt[4] =
       {0.f,0.f,0.f,0.f};

                                            solveContact<false>(
       m_constraints[i], (b3Vector3&)bodyA.m_pos, (b3Vector3&)bodyA.m_linVel,
       (b3Vector3&)bodyA.m_angVel, bodyA.m_invMass, (const b3Matrix3x3
       &)m_shapes[aIdx].m_invInertiaWorld, (b3Vector3&)bodyB.m_pos,
       ...
       minRambdaDt );
                                    ...
                                    else
                                    {
                                            float maxRambdaDt[4] =
       {FLT_MAX,FLT_MAX,FLT_MAX,FLT_MAX}; float minRambdaDt[4] =
       {0.f,0.f,0.f,0.f}; float sum = 0; for(int j=0; j<4; j++)
                                            {
                                                    sum
       +=m_constraints[i].m_appliedRambdaDt[j];
                                            }
                                            frictionCoeff = 0.7f;
                                            for(int j=0; j<4; j++)
                                            {
                                                    maxRambdaDt[j] =
       frictionCoeff*sum; minRambdaDt[j] = -maxRambdaDt[j];
                                            }
    ...
    for (int iter = 0; iter < m_nIterations; iter++) {
      for (int cellBatch = 0; cellBatch < B3_SOLVER_N_BATCHES; cellBatch++) {
        ...
        for (int wgIdx = 0; wgIdx < numWorkgroups; wgIdx++) {
          ...
          // %d\n",wgIdx,xIdx,yIdx,zIdx,cellIdx,cellBatch); printf("cell %d has
...
additionalData, int nContacts, const b3Solver::ConstraintCfg& cfg )
{
...
m_offsets;//BufferUtils::map<TYPE_CL, false>( data->m_device, &offsetsHost );
                ...
                {	//	2. set cell idx
                        ...
                        cdata.m_scale
= 1.f/(N_OBJ_PER_SPLIT*cfg.m_averageExtent); cdata.m_nSplit = B3_SOLVER_N_SPLIT;


                        b3BufferInfoCL bInfo[] = { b3BufferInfoCL(
contactsIn->getBufferCL() ), b3BufferInfoCL( bodyBuf->getBufferCL() ),
b3BufferInfoCL( m_sortDataBuffer->getBufferCL() ) }; b3LauncherCL launcher(
m_queue, m_setSortDataKernel ); launcher.setBuffers( bInfo,
sizeof(bInfo)/sizeof(b3BufferInfoCL) ); launcher.setConst(  cdata );
...
BufferUtils::map<TYPE_CL, false>( data->m_device, contactsIn );	// copying
contacts to this buffer
...
b3BufferInfoCL( m_sortDataBuffer->getBufferCL() ) }; b3LauncherCL launcher(
m_queue, m_reorderContactKernel ); launcher.setBuffers( bInfo,
sizeof(bInfo)/sizeof(b3BufferInfoCL) ); launcher.setConst(  cdata );
                                launcher.launch1D( nContacts, 64 );
                        }
