...
#define SOLVER_UTILS_KERNEL_PATH \
...
b3GpuJacobiContactSolver::b3GpuJacobiContactSolver(cl_context ctx,
                                                   cl_device_id device,
  ...
  m_data->m_contactConstraintOffsets =
      new b3OpenCLArray<b3Int2>(m_context, m_queue);
  m_data->m_offsetSplitBodies =
      new b3OpenCLArray<unsigned int>(m_context, m_queue);
  m_data->m_contactConstraints =
      new b3OpenCLArray<b3GpuConstraint4>(m_context, m_queue);
  m_data->m_deltaLinearVelocities =
      new b3OpenCLArray<b3Vector3>(m_context, m_queue);
  ...
  const char* solverUtilsSource = solverUtilsCL;
  {
    cl_program solverUtilsProg = b3OpenCLUtils::compileCLProgramFromString(
        ctx, device, solverUtilsSource, &pErrNum, additionalMacros,
        SOLVER_UTILS_KERNEL_PATH);
    b3Assert(solverUtilsProg);
    m_data->m_countBodiesKernel = b3OpenCLUtils::compileCLKernelFromString(
        ctx, device, solverUtilsSource, "CountBodiesKernel", &pErrNum,
        solverUtilsProg, additionalMacros);
    b3Assert(m_data->m_countBodiesKernel);

    m_data->m_contactToConstraintSplitKernel =
        b3OpenCLUtils::compileCLKernelFromString(
            ctx, device, solverUtilsSource, "ContactToConstraintSplitKernel",
            &pErrNum, solverUtilsProg, additionalMacros);
    b3Assert(m_data->m_contactToConstraintSplitKernel);
    m_data->m_clearVelocitiesKernel = b3OpenCLUtils::compileCLKernelFromString(
        ctx, device, solverUtilsSource, "ClearVelocitiesKernel", &pErrNum,
        solverUtilsProg, additionalMacros);
    b3Assert(m_data->m_clearVelocitiesKernel);

    m_data->m_averageVelocitiesKernel =
        b3OpenCLUtils::compileCLKernelFromString(
            ctx, device, solverUtilsSource, "AverageVelocitiesKernel", &pErrNum,
            solverUtilsProg, additionalMacros);
    b3Assert(m_data->m_averageVelocitiesKernel);

    m_data->m_updateBodyVelocitiesKernel =
        b3OpenCLUtils::compileCLKernelFromString(
            ctx, device, solverUtilsSource, "UpdateBodyVelocitiesKernel",
            &pErrNum, solverUtilsProg, additionalMacros);
    b3Assert(m_data->m_updateBodyVelocitiesKernel);

    m_data->m_solveContactKernel = b3OpenCLUtils::compileCLKernelFromString(
        ctx, device, solverUtilsSource, "SolveContactJacobiKernel", &pErrNum,
        solverUtilsProg, additionalMacros);
    b3Assert(m_data->m_solveContactKernel);

    m_data->m_solveFrictionKernel = b3OpenCLUtils::compileCLKernelFromString(
        ctx, device, solverUtilsSource, "SolveFrictionJacobiKernel", &pErrNum,
        solverUtilsProg, additionalMacros);
...
b3Vector3 make_float4(float v) { return b3MakeVector3(v, v, v); }

b3Vector4 make_float4(float x, float y, float z, float w) {
  return b3MakeVector4(x, y, z, w);
}

static inline float calcRelVel(const b3Vector3& l0, const b3Vector3& l1,
...
static inline void setLinearAndAngular(const b3Vector3& n, const b3Vector3& r0,
...
static __inline void solveContact(
    b3GpuConstraint4& cs, const b3Vector3& posA, const b3Vector3& linVelARO,
    const b3Vector3& angVelARO, float invMassA, const b3Matrix3x3& invInertiaA,
    const b3Vector3& posB, const b3Vector3& linVelBRO,
    const b3Vector3& angVelBRO, float invMassB, const b3Matrix3x3& invInertiaB,
    float maxRambdaDt[4], float minRambdaDt[4], b3Vector3& dLinVelA,
    b3Vector3& dAngVelA, b3Vector3& dLinVelB, b3Vector3& dAngVelB) {
...

    {
      ...
      float rambdaDt =
          calcRelVel((const b3Vector3&)cs.m_linear,
                     ...
                     linVelARO + dLinVelA, angVelARO + dAngVelA,
                     linVelBRO + dLinVelB, angVelBRO + dAngVelB) +
          cs.m_b[ic];
...
void solveContact3(b3GpuConstraint4* cs, b3Vector3* posAPtr, b3Vector3* linVelA,
                   b3Vector3* angVelA, float invMassA,
                   const b3Matrix3x3& invInertiaA, b3Vector3* posBPtr,
                   b3Vector3* linVelB, b3Vector3* angVelB, float invMassB,
                   const b3Matrix3x3& invInertiaB, b3Vector3* dLinVelA,
                   b3Vector3* dAngVelA, b3Vector3* dLinVelB,
                   b3Vector3* dAngVelB) {
  float minRambdaDt = 0;
  ...
  for (int ic = 0; ic < 4; ic++) {
    ...
    float rambdaDt = calcRelVel(cs->m_linear, -cs->m_linear, angular0, angular1,
                                ...
                                *linVelB + *dLinVelB, *angVelB + *dAngVelB) +
                     cs->m_b[ic];
    rambdaDt *= cs->m_jacCoeffInv[ic];
...
static inline void solveFriction(
    b3GpuConstraint4& cs, const b3Vector3& posA, const b3Vector3& linVelARO,
    const b3Vector3& angVelARO, float invMassA, const b3Matrix3x3& invInertiaA,
    const b3Vector3& posB, const b3Vector3& linVelBRO,
    const b3Vector3& angVelBRO, float invMassB, const b3Matrix3x3& invInertiaB,
  ...
  {  //	angular damping for point constraint
    ...
    if (b3Dot(ab, ac) > 0.95f || (invMassA == 0.f || invMassB == 0.f)) {
      ...
      if (invMassA) dAngVelA -= (angNA * 0.1f) * n;
      if (invMassB) dAngVelB -= (angNB * 0.1f) * n;
    }
  }
}

float calcJacCoeff(const b3Vector3& linear0, const b3Vector3& linear1,
                   const b3Vector3& angular0, const b3Vector3& angular1,
                   float invMass0, const b3Matrix3x3* invInertia0,
                   float invMass1, const b3Matrix3x3* invInertia1, float countA,
                   float countB) {
...
void setConstraint4(const b3Vector3& posA, const b3Vector3& linVelA,
                    const b3Vector3& angVelA, float invMassA,
                    const b3Matrix3x3& invInertiaA, const b3Vector3& posB,
                    const b3Vector3& linVelB, const b3Vector3& angVelB,
                    float invMassB, const b3Matrix3x3& invInertiaB,
                    b3Contact4* src, float dt, float positionDrift,
  ...
  for (int ic = 0; ic < 4; ic++) {
    ...
    float relVelN;
    {
      ...
      setLinearAndAngular(src->m_worldNormalOnB, r0, r1, linear, angular0,
                          angular1);

      dstC->m_jacCoeffInv[ic] =
          calcJacCoeff(linear, -linear, angular0, angular1, invMassA,
                       &invInertiaA, invMassB, &invInertiaB, countA, countB);

      relVelN = calcRelVel(linear, -linear, angular0, angular1, linVelA,
                           angVelA, linVelB, angVelB);
      ...
      dstC->m_b[ic] += (src->m_worldPosB[ic][3] + positionDrift) *
                       positionConstraintCoeff * dtInv;
      dstC->m_appliedRambdaDt[ic] = 0.f;
    }
  }
...
void ContactToConstraintKernel(b3Contact4* gContact, b3RigidBodyData* gBodies,
                               b3InertiaData* gShapes,
                               b3GpuConstraint4* gConstraintOut, int nContacts,
                               float dt, float positionDrift,
  ...
  if (gIdx < nContacts) {
    ...
    setConstraint4(posA, linVelA, angVelA, invMassA, invInertiaA, posB, linVelB,
                   angVelB, invMassB, invInertiaB, &gContact[gIdx], dt,
                   positionDrift, positionConstraintCoeff, countA, countB, &cs);
...
void b3GpuJacobiContactSolver::solveGroupHost(
  ...
  m_data->m_scan->executeHost(bodyCount, offsetSplitBodies, numBodies,
                              &totalNumSplitBodies);
  ...
  for (int i = 0; i < numManifolds; i++) {
    ContactToConstraintKernel(
        &manifoldPtr[0], bodies, inertias, &contactConstraints[0], numManifolds,
        solverInfo.m_deltaTime, solverInfo.m_positionDrift,
        solverInfo.m_positionConstraintCoeff, i, bodyCount);
  }
  ...
  for (int iter = 0; iter < maxIter; iter++) {
    ...
    for (i = 0; i < numManifolds; i++) {
...

      {
        ...
        solveContact(contactConstraints[i], (b3Vector3&)bodyA.m_pos,
                     (b3Vector3&)bodyA.m_linVel, (b3Vector3&)bodyA.m_angVel,
                     bodyA.m_invMass, inertias[aIdx].m_invInertiaWorld,
                     (b3Vector3&)bodyB.m_pos, (b3Vector3&)bodyB.m_linVel,
                     (b3Vector3&)bodyB.m_angVel, bodyB.m_invMass,
                     inertias[bIdx].m_invInertiaWorld, maxRambdaDt, minRambdaDt,
  ...
  for (int iter = 0; iter < maxIter; iter++) {
    ...
    for (int i = 0; i < numManifolds; i++) {
      ...
      solveFriction(contactConstraints[i], (b3Vector3&)bodyA.m_pos,
                    (b3Vector3&)bodyA.m_linVel, (b3Vector3&)bodyA.m_angVel,
                    bodyA.m_invMass, inertias[aIdx].m_invInertiaWorld,
                    (b3Vector3&)bodyB.m_pos, (b3Vector3&)bodyB.m_linVel,
                    (b3Vector3&)bodyB.m_angVel, bodyB.m_invMass,
...
void b3GpuJacobiContactSolver::solveContacts(int numBodies, cl_mem bodyBuf,
...
//
//
// void  b3GpuJacobiContactSolver::solveGroup(b3OpenCLArray<b3RigidBodyData>*
// bodies,b3OpenCLArray<b3InertiaData>* inertias,b3OpenCLArray<b3Contact4>*
// manifoldPtr,const btJacobiSolverInfo& solverInfo)
{
...

  {
    ...
    m_data->m_filler->execute(*m_data->m_contactConstraintOffsets, val2,
                              numManifolds);
  }
  ...
  unsigned int totalNumSplitBodies = 0;
  {
    ...
    m_data->m_scan->execute(*m_data->m_bodyCount, *m_data->m_offsetSplitBodies,
                            numBodies, &totalNumSplitBodies);
...

  {
    ...
    b3LauncherCL launcher(m_queue, m_data->m_contactToConstraintSplitKernel,
                          "m_contactToConstraintSplitKernel");
