...
b3FixedConstraint::b3FixedConstraint(int rbA, int rbB,
...
void b3FixedConstraint::getInfo1(b3ConstraintInfo1* info,
                                 const b3RigidBodyData* bodies) {
  info->m_numConstraintRows = 6;
  info->nub = 6;
}

void b3FixedConstraint::getInfo2(b3ConstraintInfo2* info,
                                 const b3RigidBodyData* bodies) {
  ...
  b3TransformUtil::calculateDiffAxisAngleQuaternion(m_relTargetAB, qrelCur,
                                                    diff, angle);
  diff *= -angle;
  for (j = 0; j < 3; j++) {
