...
void b3PgsJacobiSolver::solveContacts(int numBodies, b3RigidBodyData* bodies,
  ...
  // B3_SOLVER_INTERLEAVE_CONTACT_AND_FRICTION_CONSTRAINTS) if
  // ((infoGlobal.m_solverMode & B3_SOLVER_USE_2_FRICTION_DIRECTIONS) &&
...

{
  ...
  {
    b3Vector3 iMJaA = body0 ? getInvInertiaTensorWorld(
                                  &inertias[solverBodyA.m_originalBodyIndex]) *
                                  solverConstraint.m_relpos1CrossNormal
                            : b3MakeVector3(0, 0, 0);
    b3Vector3 iMJaB = body1 ? getInvInertiaTensorWorld(
                                  &inertias[solverBodyB.m_originalBodyIndex]) *
...
void b3PgsJacobiSolver::setupContactConstraint(
  ...
  //colObj0->getWorldTransform().getOrigin(); 			b3Vector3 rel_pos2 = pos2 -
  ...
  solverConstraint.m_angularComponentA =
      rb0 ? getInvInertiaTensorWorld(&inertias[bodyA->m_originalBodyIndex]) *
                torqueAxis0
          : b3MakeVector3(0, 0, 0);
  b3Vector3 torqueAxis1 = rel_pos2.cross(cp.m_normalWorldOnB);
  solverConstraint.m_angularComponentB =
      rb1 ? getInvInertiaTensorWorld(&inertias[bodyB->m_originalBodyIndex]) *
                -torqueAxis1
          : b3MakeVector3(0, 0, 0);
...
void b3PgsJacobiSolver::convertContact(b3RigidBodyData* bodies,
  ...
  for (int j = 0; j < numContacts; j++) {
    ...
    if (cp.getDistance() <= getContactProcessingThreshold(manifold)) {
      ...
      //cp.getPositionWorldOnA(); 			const b3Vector3& pos2 =
      ...
      /// B3_SOLVER_USE_2_FRICTION_DIRECTIONS. In that case, the second friction
        ...
        if (info1.m_numConstraintRows) {
          ...
          for (j = 0; j < info1.m_numConstraintRows; j++) {
...

            {
              ...
              // -solverConstraint.m_contactNormal because it gets multiplied
              // iMJlB
              b3Vector3 iMJlA =
