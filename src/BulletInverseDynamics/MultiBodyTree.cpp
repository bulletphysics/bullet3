...
int MultiBodyTree::calculateMassMatrix(const vecx &q,
  ...
  if (-1 == m_impl->calculateMassMatrix(q, update_kinematics, initialize_matrix,
...
int MultiBodyTree::calculateKinematics(const vecx &q, const vecx &u,
  ...
  if (-1 == m_impl->calculateKinematics(
                q, u, dot_u,
...
int MultiBodyTree::calculateJacobians(const vecx &q, const vecx &u) {
  ...
  if (-1 == m_impl->calculateJacobians(
...
int MultiBodyTree::calculateJacobians(const vecx &q) {
  ...
  if (-1 == m_impl->calculateJacobians(
                q, q, MultiBodyTree::MultiBodyImpl::POSITION_ONLY)) {
    error_message("error in jacobian calculation\n");
    return -1;
  }
  return 0;
