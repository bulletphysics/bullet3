...
/// to avoid collisions against internal edges it can be generated using
...
struct btTriangleInfoMap : public btInternalTriangleInfoMap {
  ...
  btScalar
      m_edgeDistanceThreshold;  /// used to determine edge contacts: if the
                                /// closest distance between a contact point and
                                /// an edge is smaller than this distance
                                /// threshold it is considered to "hit the edge"
  btScalar
      m_maxEdgeAngleThreshold;  // ignore edges that connect triangles at an
                                // angle larger than this m_maxEdgeAngleThreshold
...
/// those fields have to be float and not btScalar for the serialization to work
/// properly
...
SIMD_FORCE_INLINE const char* btTriangleInfoMap::serialize(
  ...
  tmapData->m_valueArrayPtr =
      tmapData->m_numValues ? (btTriangleInfoData*)serializer->getUniquePointer(
                                  (void*)&m_valueArray[0])
                            : 0;
