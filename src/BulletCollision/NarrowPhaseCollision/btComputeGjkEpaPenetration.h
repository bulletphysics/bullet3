...
int btComputeGjkEpaPenetration(const btConvexTemplate& a,
  ...
  int m_curIter = 0;
  int gGjkMaxIter =
      colDesc.m_maxGjkIterations;  // this is to catch invalid input, perhaps
                                   // check for #NaN?
...

  {
    ...
    if (checkPenetration && (!isValid || catchDegeneratePenetrationCase)) {
...

      {
        ...
        /// reports a degenerate case EPA reports no penetration, and the second
        /// GJK (using the supporting vector without margin) reports a valid
        /// positive distance. Use the results of the second GJK instead of
        /// failing. thanks to Jacob.Langford for the reproduction case
