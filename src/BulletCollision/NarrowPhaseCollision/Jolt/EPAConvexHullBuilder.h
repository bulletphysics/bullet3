...
#ifndef __JOLT_EPA_CONVEX_HULL_BUILDER_H
#define __JOLT_EPA_CONVEX_HULL_BUILDER_H
...
private:
	...
	static constexpr btScalar cDrawScale = 10.0f;
#endif
...
public:
	...
	static constexpr btScalar cMinTriangleArea = 1.0e-10f;		///< Minimum area of a triangle before, if smaller than this it will not be added to the priority queue
	static constexpr btScalar cBarycentricEpsilon = 1.0e-3f;	///< Epsilon value used to determine if a point is in the interior of a triangle 
	...
	public:
		...
		btScalar			mClosestLenSq = SIMD_INFINITY;			///< Closest distance^2 from origin to triangle
		btScalar			mLambda[2];							///< Barycentric coordinates of closest point to origin on triangle
	...
	private:
		...
		{
			jUint8		mTriangle[sizeof(Triangle)];
			Block *		mNextFree;
		};
	...
	Triangle *			FindFacingTriangle(Vec3Arg inPosition, btScalar &outBestDistSq)
	{
		Triangle *best = nullptr;
		btScalar best_dist_sq = 0.0f;

		for (Triangle *t : mTriangleQueue)
			if (!t->mRemoved)
			{
				btScalar dot = t->mNormal.dot(inPosition - t->mCentroid);
				if (dot > 0.0f)
				{
	...
	bool				AddPoint(Triangle *inFacingTriangle, int inIdx, btScalar inClosestDistSq, NewTriangles &outTriangles)
	{
		// Get position
	...
	void				FreeTriangle(Triangle *inT)
	{
		...
		for (const Edge &e : inT->mEdge)
			assert(e.mNeighbourTriangle == nullptr);
#endif
...
private:
	...
	{
		assert(inEdge1 >= 0 && inEdge1 < 3);
		assert(inEdge2 >= 0 && inEdge2 < 3);
		Edge &e1 = inT1->mEdge[inEdge1];
		Edge &e2 = inT2->mEdge[inEdge2];

		// Check not connected yet
		assert(e1.mNeighbourTriangle == nullptr);
		assert(e2.mNeighbourTriangle == nullptr);

		// Check vertices match
		assert(e1.mStartIdx == inT2->GetNextEdge(inEdge2).mStartIdx);
		assert(e2.mStartIdx == inT1->GetNextEdge(inEdge1).mStartIdx);
	...
	void				UnlinkTriangle(Triangle *inT)
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			if (edge.mNeighbourTriangle != nullptr)
			{
				...
				assert(neighbour_edge.mNeighbourTriangle == inT);
				assert(neighbour_edge.mNeighbourEdge == i);
	...
	bool				FindEdge(Triangle *inFacingTriangle, Vec3Arg inVertex, Edges &outEdges)
	{
		...
		assert(inFacingTriangle->IsFacing(inVertex));
			...
			else
			{
				...
				if (n != nullptr && !n->mRemoved)
				{
					...
					if (n->IsFacing(inVertex))
					{
						...
						cur_stack_pos++;
						assert(cur_stack_pos < cMaxEdgeLength);
		...
		assert(outEdges.empty() || outEdges[0].mStartIdx == next_expected_start_idx);
	...
	void				ValidateTriangle(const Triangle *inT) const
	{
		if (inT->mRemoved)
		{
			...
			for (const Edge &my_edge : inT->mEdge)
				assert(my_edge.mNeighbourTriangle == nullptr);
		}
		else
		{
			for (int i = 0; i < 3; ++i)
			{
				...
				if (nb != nullptr)
				{
					...
					assert(nb_edge.mNeighbourTriangle == inT);
					assert(nb_edge.mNeighbourEdge == i);
...
public:
	...
	void				DrawMarker(Vec3Arg inPosition, ColorArg inColor, btScalar inSize)
	{
		DebugRenderer::sInstance->DrawMarker(cDrawScale * (mOffset + inPosition), inColor, inSize);
	}

	/// Draw an arrow for debugging purposes
	void				DrawArrow(Vec3Arg inFrom, Vec3Arg inTo, ColorArg inColor, btScalar inArrowSize)
	{
...
{
	// Fill in indexes
	assert(inIdx0 != inIdx1 && inIdx0 != inIdx2 && inIdx1 != inIdx2);
	...
	btScalar y20_dot_y20 = y20.dot(y20);
	btScalar y21_dot_y21 = y21.dot(y21);
	if (y20_dot_y20 < y21_dot_y21)
	{
		...
		btScalar normal_len_sq = mNormal.length2();
		if (normal_len_sq > cMinTriangleArea)
		{
			...
			btScalar c_dot_n = mCentroid.dot(mNormal);
			...
			btScalar y10_dot_y10 = y10.length2();
			btScalar y10_dot_y20 = y10.dot(y20);
			btScalar determinant = y10_dot_y10 * y20_dot_y20 - y10_dot_y20 * y10_dot_y20;
			...
			{
				btScalar y0_dot_y10 = y0.dot(y10);
				btScalar y0_dot_y20 = y0.dot(y20);
				btScalar l0 = (y10_dot_y20 * y0_dot_y20 - y20_dot_y20 * y0_dot_y10) / determinant;
				btScalar l1 = (y10_dot_y20 * y0_dot_y10 - y10_dot_y10 * y0_dot_y20) / determinant;
				mLambda[0] = l0;
				mLambda[1] = l1;
	...
	else
	{
		...
		if (normal_len_sq > cMinTriangleArea)
		{
			...
			// Cramers rule to invert matrix:
			btScalar y10_dot_y10 = y10.length2();
			btScalar y10_dot_y21 = y10.dot(y21);
			btScalar determinant = y10_dot_y10 * y21_dot_y21 - y10_dot_y21 * y10_dot_y21;
			if (determinant > 0.0f)
			{
				btScalar y1_dot_y10 = y1.dot(y10);
				btScalar y1_dot_y21 = y1.dot(y21);
				btScalar l0 = (y21_dot_y21 * y1_dot_y10 - y10_dot_y21 * y1_dot_y21) / determinant;
				btScalar l1 = (y10_dot_y21 * y1_dot_y10 - y10_dot_y10 * y1_dot_y21) / determinant;
				mLambda[0] = l0;
				mLambda[1] = l1;
...
#endif //__JOLT_EPA_CONVEX_HULL_BUILDER_H
