...
/// However, when using large size ratios GJK can be imprecise so the distance is
/// not conservative. In that case, enabling this USE_SEPDISTANCE_UTIL2 would
/// result in failing/missing collisions. Either improve GJK for large size
/// ratios (testing a 100 units versus a 0.1 unit object) or only enable the util
...
/// objects. Multiple contact points are calculated by perturbing the orientation
/// of the smallest object orthogonal to the separating normal. This idea was
/// described by Gino van den Bergen in this forum topic
