...
// 'stepSimulation'  or 'updateAabbs' before using a rayTest #define
//RECALCULATE_AABB_RAYCAST 1
    ...
    if (convexCaster.calcTimeOfImpact(rayFromTrans, rayToTrans,
      ...
      if (castResult.m_normal.length2() > btScalar(0.0001)) {
        if (castResult.m_fraction < resultCallback.m_closestHitFraction) {
          // todo: figure out what this is about. When is rayFromTest.getBasis()
          // not identity?
...
struct btSingleRayCallback : public btBroadphaseRayCallback {
  ...
  virtual bool process(const btBroadphaseProxy* proxy) {
    ...
    if (m_resultCallback.needsCollision(
      ...
      // ctrl->GetRigidcollisionObject(); btVector3
...
void btCollisionWorld::rayTest(const btVector3& rayFromWorld,
  ...
  /// aabb and for each object with ray-aabb overlap, perform an exact ray test
...
/// btCollisionWorld, and calls the resultCallback. it reports one or more
/// contact points for every overlapping object (including the one with deepest
/// penetration)
...
/// and calls the resultCallback if overlap if detected. it reports one or more
/// contact points (including the one with deepest penetration)
  ...
  if (shape->getShapeType() == COMPOUND_SHAPE_PROXYTYPE) {
    ...
    switch (shape->getShapeType()) {
      ...
      case SPHERE_SHAPE_PROXYTYPE: {
        ...
        btScalar radius =
            sphereShape->getMargin();  // radius doesn't include the margin, so
                                       // draw with margin
