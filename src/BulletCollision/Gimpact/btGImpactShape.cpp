...
btGImpactMeshShapePart::btGImpactMeshShapePart(
  ...
  // is best to avoid using it in h files)
  ...
  //  constructed and unlock once in the destructor.
  // The other way of locking and unlocking for each collision check in the
  // narrowphase is not threadsafe.  Note these are not thread-locks, they are
  // calls to the meshInterface's getLockedReadOnlyVertexIndexBase virtual
  // function, which by default just returns a couple of pointers.  In theory a
  // client could override the lock function to do all sorts of things like
  // reading data from GPU memory, or decompressing data on the fly, but such
  // things do not seem all that likely or useful, given the performance cost.
