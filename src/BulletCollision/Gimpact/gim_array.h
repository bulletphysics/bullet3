 ...
 public:
  ...
  inline bool growingCheck() {
    if (m_allocated_size <= m_size) {
      ...
      if (resizeData((requestsize + GIM_ARRAY_GROW_INCREMENT) *
                     GIM_ARRAY_GROW_FACTOR) == false)
        return false;
    }
    return true;
  }
  ...
  inline GUINT size() const { return m_size; }

  inline GUINT max_size() const { return m_allocated_size; }
  ...
  inline T* get_pointer_at(GUINT i) { return m_data + i; }

  inline const T* get_pointer_at(GUINT i) const { return m_data + i; }

  inline T& at(GUINT i) { return m_data[i]; }

  inline const T& at(GUINT i) const { return m_data[i]; }

  inline T& front() { return *m_data; }

  inline const T& front() const { return *m_data; }

  inline T& back() { return m_data[m_size - 1]; }

  inline const T& back() const { return m_data[m_size - 1]; }

  inline void swap(GUINT i, GUINT j) { gim_swap_elements(m_data, i, j); }
  ...
  inline void pop_back_mem() { m_size--; }
  ...
  inline void refit() { resizeData(m_size); }
};
