...
const b3AlignedObjectArray<b3Contact4Data>& b3CpuNarrowPhase::getContacts()
    const {
...
const b3Collidable& b3CpuNarrowPhase::getCollidableCpu(
...
b3CpuNarrowPhase::~b3CpuNarrowPhase() { delete m_data; }

void b3CpuNarrowPhase::computeContacts(
    b3AlignedObjectArray<b3Int4>& pairs,
    b3AlignedObjectArray<b3Aabb>& aabbsWorldSpace,
    b3AlignedObjectArray<b3RigidBodyData>& bodies) {
  ...
  for (int i = 0; i < nPairs; i++) {
    ...
    if (m_data->m_collidablesCPU[collidableIndexA].m_shapeType ==
            SHAPE_SPHERE &&
        m_data->m_collidablesCPU[collidableIndexB].m_shapeType ==
            SHAPE_CONVEX_HULL) {
    ...
    if (m_data->m_collidablesCPU[collidableIndexA].m_shapeType ==
      ...
      //			&m_data->m_collidablesCPU[0],&hostConvexData[0],&cpuChildShapes[0],
      //hostAabbsWorldSpace,hostAabbsLocalSpace,hostVertices,hostUniqueEdges,hostIndices,hostFaces,&hostContacts[0],
    ...
    if (m_data->m_collidablesCPU[collidableIndexA].m_shapeType ==
      ...
      //			&m_data->m_collidablesCPU[0],&hostConvexData[0],&cpuChildShapes[0],
      //&hostVertices[0],&hostIndices[0],&hostFaces[0],&hostContacts[0],nContacts,maxContactCapacity);
      //			printf("convex-plane\n");
    }
    ...
    if (m_data->m_collidablesCPU[collidableIndexA].m_shapeType ==
      ...
      // printf("pairs[i].z=%d\n",pairs[i].z);
      // int contactIndex =
      // computeContactConvexConvex2(i,bodyIndexA,bodyIndexB,collidableIndexA,collidableIndexB,bodies,
      ...
      int contactIndex = b3ContactConvexConvexSAT(
          i, bodyIndexA, bodyIndexB, collidableIndexA, collidableIndexB, bodies,
          m_data->m_collidablesCPU, m_data->m_convexPolyhedra,
          m_data->m_convexVertices, m_data->m_uniqueEdges,
          m_data->m_convexIndices, m_data->m_convexFaces, m_data->m_contacts,
          numContacts, maxContactCapacity);
...
int b3CpuNarrowPhase::registerConvexHullShape(const float* vertices,
                                              int strideInBytes,
                                              int numVertices,
                                              const float* scaling) {
...
int b3CpuNarrowPhase::registerConvexHullShapeInternal(
