...
inline void b3ProjectAxis(const b3ConvexPolyhedronData& hull,
  ...
  for (int i = 0; i < numVerts; i++) {
    ...
    b3Scalar dp =
        b3Dot3F4((b3Float4&)vertices[hull.m_vertexOffset + i], localDir);
    // b3Assert(dp==dpL);
    if (dp < min) min = dp;
    if (dp > max) max = dp;
  }
...
inline bool b3FindSeparatingAxis(
    const b3ConvexPolyhedronData& hullA, const b3ConvexPolyhedronData& hullB,
    const b3Float4& posA1, const b3Quaternion& ornA, const b3Float4& posB1,
  ...
  for (int i = 0; i < numFacesA; i++) {
    ...
    if (b3Dot3F4(deltaC2, faceANormalWS) < 0) faceANormalWS *= -1.f;
  ...
  for (int i = 0; i < numFacesB; i++) {
    ...
    b3Scalar d;
    if (!b3TestSepAxis(hullA, hullB, posA, ornA, posB, ornB, WorldNormal,
                       verticesA, verticesB, d))
      return false;
  ...
  for (int e0 = 0; e0 < hullA.m_numUniqueEdges; e0++) {
    const b3Float4& edge0 =
        (b3Float4&)uniqueEdgesA[hullA.m_uniqueEdgesOffset + e0];
    ...
    for (int e1 = 0; e1 < hullB.m_numUniqueEdges; e1++) {
      ...
      if (!b3IsAlmostZero((b3Vector3&)crossje)) {
        ...
        if (b3Dot3F4(deltaC2, crossje) < 0) crossje *= -1.f;
        ...
        b3Scalar dist;
        if (!b3TestSepAxis(hullA, hullB, posA, ornA, posB, ornB, crossje,
  ...
  if ((b3Dot3F4(-deltaC2, (b3Float4&)sep)) > 0.0f) sep = -sep;
