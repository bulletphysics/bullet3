...
/// on axis aligned bounding boxes (aabb tree). This b3DynamicBvh is used for
/// soft body collision detection and for the b3DynamicBvhBroadphase. It has a
/// fast insert, remove and update of nodes. Unlike the b3QuantizedBvh, nodes can
...
struct b3DynamicBvh {
  ...
  /// the b3AlignedAlloc is thread-safe (uses locking etc) rayTest is slower than
  /// rayTestInternal, because it builds a local stack, using memory allocations,
  /// and it recomputes signs/rayDirectionInverses each time
  B3_DBVT_PREFIX
  ...
  /// signs/rayInverseDirections rayTestInternal is used by
  /// b3DynamicBvhBroadphase to accelerate world ray casts
  B3_DBVT_PREFIX
