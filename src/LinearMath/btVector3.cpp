...
long _maxdot_large(const float *vv, const float *vec, unsigned long count,
  ...
  if (0 == segment ||
      0xf !=
          _mm_movemask_ps((float4)_mm_cmpeq_ps(
              max, dotMax))) {  // we found a new max. Search for it
                                // find max across the max vector, place in all
                                // elements of max -- big latency hit here
    max = _mm_max_ps(max, (float4)_mm_shuffle_ps(max, max, 0x4e));
    max = _mm_max_ps(max, (float4)_mm_shuffle_ps(max, max, 0xb1));

    // It is slightly faster to do this part in scalar code when count < 8.
    // However, the common case for this where it actually makes a difference is
    // handled in the early out at the top of the function, so it is less than a
    // 1% difference here. I opted for improved code size, fewer branches and
    // reduced complexity, and removed it.
      ...
      0xf !=
          _mm_movemask_ps((float4)_mm_cmpeq_ps(
              min, dotmin))) {  // we found a new min. Search for it
                                // find min across the min vector, place in all
                                // elements of min -- big latency hit here
    min = _mm_min_ps(min, (float4)_mm_shuffle_ps(min, min, 0x4e));
    min = _mm_min_ps(min, (float4)_mm_shuffle_ps(min, min, 0xb1));
