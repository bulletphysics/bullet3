...
// noticeably slower presumably because when it fails to lock at first it would
// sleep the thread and trigger costly context switching.
//
...
// of a task scheduler to assign nice sequential thread-indexes. We also do not
// get notified if a worker thread is destroyed, so we can't tell when a
// thread-index is no longer being used. We allocate thread-indexes as needed
// with a sequential global thread counter.
//
// Our simple thread-counting scheme falls apart if the task scheduler destroys
// some threads but continues to re-use other threads and the application
// repeatedly resizes the thread pool of the task scheduler. In order to prevent
// the thread-counter from exceeding the global max (BT_MAX_THREAD_COUNT), we
// wrap the thread counter back to 1. This should only happen if the worker
// threads have all been destroyed and re-created.
//
  ...
  if (gBtTaskScheduler && sThreadIndex > 0) {
    ...
    if (gDebugThreadIds[sThreadIndex] == kInvalidThreadId) {
      ...
      if (gDebugThreadIds[sThreadIndex] != tid) {
        ...
        // about how threads are managed when threadpool is resized
...
void btITaskScheduler::activate() {
  ...
  // task scheduler. The main thread is always thread-index 0, and worker
  // threads are numbered from 1 to 63 (BT_MAX_THREAD_COUNT-1) The
  // thread-indexes need to be unique amongst the threads that can be running
  // simultaneously. Since only one task scheduler can be used at a time, it is
  // OK for a pair of threads that belong to different task schedulers to share
  // the same thread index because they can't be running at the same time. So
  // each task scheduler needs to keep its own thread counter value
  if (!m_isActive) {
...
void btSetTaskScheduler(btITaskScheduler* ts) {
  int threadId =
      btGetCurrentThreadIndex();  // make sure we call this on main thread at
                                  // least once before any workers run
  if (threadId != 0) {
...
class btTaskSchedulerOpenMP : public btITaskScheduler {
 ...
 public:
  ...
  virtual void setNumThreads(int numThreads) BT_OVERRIDE {
    ...
    // can't know for sure if every implementation has the same behavior of
    // destroying all previous threads when resizing the threadpool
    m_numThreads =
 ...
 public:
  ...
  virtual void setNumThreads(int numThreads) BT_OVERRIDE {
    ...
    SchedulerPolicy policy;
    {
      ...
      // reusing old threads force it to destroy old threads
