...
// enable it manually by commenting out the following lines #if
//(defined(__GNUC__) && defined(__x86_64__) && !defined(__ICL))  // ||
//(defined(__ICL) && defined(_M_X64))   bug in Intel compiler, disable inline
//assembly 	#define USE_X86_64_ASM #endif
  ...
  class Int128 {
   public:
    ...
    btScalar toScalar() const {
      return ((int64_t)high >= 0) ? btScalar(high) * (btScalar(0x100000000LL) *
                                                      btScalar(0x100000000LL)) +
                                        btScalar(low)
                                  : -(-*this).toScalar();
    }
...
int btConvexHullInternal::Rational64::compare(const Rational64& b) const {
  ...
  return result ? result ^
                      sign  // if sign is +1, only bit 0 of result is inverted,
                            // which does not change the sign of result (and
                            // cannot result in zero) if sign is -1, all bits of
                            // result are inverted, which changes the sign of
                            // result (and again cannot result in zero)
                : 0;

#else

  return sign * Int128::mul(m_numerator, b.m_denominator)
