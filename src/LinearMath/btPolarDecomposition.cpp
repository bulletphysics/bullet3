...
btPolarDecomposition::btPolarDecomposition(btScalar tolerance,
...
unsigned int btPolarDecomposition::decompose(const btMatrix3x3& a,
  ...
  for (unsigned int i = 0; i < m_maxIterations; ++i) {
    ...
    if (btFuzzyZero(h_norm) || btFuzzyZero(u_norm)) break;

    const btScalar gamma = btPow(h_norm / u_norm, 0.25f);
    const btScalar inv_gamma = btScalar(1.0) / gamma;

    // Determine the delta to 'u'
    const btMatrix3x3 delta =
        (u * (gamma - btScalar(2.0)) + h.transpose() * inv_gamma) *
        btScalar(0.5);
