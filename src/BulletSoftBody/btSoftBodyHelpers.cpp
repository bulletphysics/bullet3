...
// graph algorithm [another 930 links]. The way the links are stored by default,
// we have a number of cases where adjacent links share a node in common
...
// link. So, we now have the possibility of a data dependency between iteration X
...
// beyond a certain point because it has to respect a possible dependency
...
// L+1 share a common node, we create a temporal gap between when the node
// position is written, and when it is subsequently read. This in turn allows the
// CPU to continue execution without risking a dependency violation. Such a
// reordering would result in significant speedups on modern CPUs with lots of
// execution resources. In our testing, we see it have a tremendous impact not
// only on the A7, but also on all x86 cores that ship with modern Macs. The
// attached source file includes a single function (ReoptimizeLinkOrder) which
// can be called on a btSoftBody object in the solveConstraints() function before
// the actual solver is invoked, or right after generateBendingConstraints() once
// we have all 1410 links.
...
btSoftBody* btSoftBodyHelpers::CreatePatch(
    ...
    const btVector3& corner11, int resx, int resy, int fixeds, bool gendiags) {
