...
bool btSoftBody::checkContact(const btCollisionObjectWrapper* colObjWrap,
  ...
  //btRigidBody::upcast(colObjWrap->getCollisionObject()); const btTransform
  // &wtr = tmpRigid ? tmpRigid->getWorldTransform() :
...
void btSoftBody::updateBounds() {
  ...
  if (m_ndbvt.m_root) {
    ...
    const btVector3 mrg = btVector3(csm, csm,
                                    csm) *
                          1;  // ??? to investigate...
    m_bounds[0] = mins - mrg;
    m_bounds[1] = maxs + mrg;
...
void btSoftBody::applyForces() {
  ...
  // && 												(m_cfg.aeromodel < btSoftBody::eAeroModel::F_TwoSided); const bool
  // as_faero =		as_aero	&& 												(m_cfg.aeromodel >=
...
const char* btSoftBody::serialize(void* dataBuffer,
  ...
  if (sbd->m_tetrahedra) {
    ...
    for (int i = 0; i < numElem; i++, memPtr++) {
      for (int j = 0; j < 4; j++) {
        ...
        memPtr->m_nodeIndices[j] =
            m_tetras[j].m_n[j] ? m_tetras[j].m_n[j] - &m_nodes[0] : -1;
      }
  ...
  if (sbd->m_anchors) {
    ...
    for (int i = 0; i < numElem; i++, memPtr++) {
      ...
      memPtr->m_rigidBody =
          m_anchors[i].m_body ? (btRigidBodyData*)serializer->getUniquePointer(
