...
bool btKinematicCharacterController::recoverFromPenetration(
  ...
  // itself or the previous recovery iteration might have used setWorldTransform
  // and pushed us into an object that is not in the previous cache contents
  // from the last timestep, as will happen if we are pushed into a new AABB
  // overlap. Unhandled this means the next convex sweep gets stuck.
  //
  // Do this by calling the broadphase's setAabb with the moved AABB, this will
  // update the broadphase paircache and the ghostobject's internal paircache at
  // the same time.    /BW
...
void btKinematicCharacterController::stepDown(btCollisionWorld* collisionWorld,
  ...
  additionalDownStep); btScalar downVelocity = (additionalDownStep == 0.0 &&
  m_verticalVelocity<0.0?-m_verticalVelocity:0.0) * dt; btVector3 gravity_drop =
  m_up * downVelocity; m_targetPosition -= (step_drop + gravity_drop);*/
  ...
  while (1) {
    ...
    if (downVelocity2 > 0.0 && downVelocity2 < stepHeight && has_hit == true &&
      ...
      // stairs motion for larger falls, use the smoother/slower interpolated
      // movement by not touching the target position
