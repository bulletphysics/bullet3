...
void btConeTwistConstraint::calcAngleInfo2(
...

  {
    ...
    if (m_swingSpan1 >= m_fixThresh && m_swingSpan2 >= m_fixThresh) {
      ...
      // so, do you really want a conetwist constraint?) anyway, we have either
      // hinge or fixed joint
...
// removed) this method computes its corresponding swing angle and axis. more
// interestingly, it computes the cone/swing limit (angle) for this cone "pose".
                                                 ...
                                                 btScalar& swingLimit)   // out
{
  ...
  if (swingAngle > SIMD_EPSILON) {
    ...
    // cone ellipse. (Since we're dealing with angles, this ellipse is embedded
    // on the surface of a sphere.)

    // For starters, compute the direction from center to surface of ellipse.
    // This is just the perpendicular (ie. rotate 2D vector by PI/2) of the
    // swing axis. (vSwingAxis is the cone rotation (in z,y); change vars and
    // rotate to (x,y) coords.)
    btScalar xEllipse = vSwingAxis.y();
    btScalar yEllipse = -vSwingAxis.z();

    // Now, we use the slope of the vector (using x/yEllipse) and find the
    // length of the line that intersects the ellipse:
    //  x^2   y^2
    //  --- + --- = 1, where a and b are semi-major axes 2 and 1 respectively
    //  (ie. the limits) a^2   b^2
...
// removed) this method computes its corresponding angle and axis.
...
void btConeTwistConstraint::adjustSwingAxisToUseEllipseNormal(
  ...
  // swingspan2). so, if we're outside the limits, the closest way back inside
  // the cone isn't along the vector back to the center. better (and more
  // stable) to use the ellipse normal.
...
void btConeTwistConstraint::setMotorTarget(const btQuaternion& q) {
  ...
  //* m_rbAFrame); btQuaternion qConstraintCur = trConstraintCur.getRotation();
...
/// optionally provide the axis (0..5). If no axis is provided, it uses the
/// default axis for this constraint.
