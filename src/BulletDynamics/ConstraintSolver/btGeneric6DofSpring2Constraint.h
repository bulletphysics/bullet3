...
btGeneric6DofSpring2Constraint : public btTypedConstraint {
 ...
 public:
  ...
  void setServo(int index,
                bool onOff);  // set the type of the motor (servo or not) (the
                              // motor has to be turned on for servo also)
  ...
  void setStiffness(
      ...
      bool limitIfNeeded =
          true);  // if limitIfNeeded is true the system will automatically
                  // limit the stiffness in necessary situations where otherwise
                  // the spring would move unrealistically too widely
  void setDamping(
      int index, btScalar damping,
      bool limitIfNeeded =
          true);  // if limitIfNeeded is true the system will automatically
                  // limit the damping in necessary situations where otherwise
                  // the spring would blow up
  void
  setEquilibriumPoint();  // set the current constraint position/orientation as
                          // an equilibrium point for all DOF
  void setEquilibriumPoint(
      int index);  // set the current constraint position/orientation as an
