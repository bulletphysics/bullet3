...
(2.82.2712) btGeneric6DofConstraint by Gabor Puhr and Tamas Umenhoffer Pros:
...
void btGeneric6DofSpring2Constraint::calculateAngleInfo() {
  ...
  switch (m_rotateOrder) {
    case RO_XYZ: {
      ...
      // (http://en.wikipedia.org/wiki/Euler_angles) The two planes are
      // non-homologous, so this is a Tait–Bryan angle formalism and not a proper
      // Euler Extrinsic rotations are equal to the reversed order intrinsic
      // rotations so the above xyz extrinsic rotations (axes are fixed) are the
      // same as the zy'x" intrinsic rotations (axes are refreshed after each
      // rotation) that is why xy and YZ planes are chosen (this will describe a
      // zy'x" intrinsic rotation) (see the figure on the left at
...
int btGeneric6DofSpring2Constraint::get_limit_motor_info2(
  ...
  if (limot->m_enableSpring) {
    ...
    // limot->m_springDamping); if(cfm > 0.99999) 	cfm = 0.99999; btScalar erp =
    // (1.0/info->fps)*limot->m_springStiffness /
    // ((1.0/info->fps)*limot->m_springStiffness + limot->m_springDamping);
    // info->m_constraintError[srow] = info->fps * erp * error * (rotational ?
    // -1.0 : 1.0); info->m_lowerLimit[srow] = -SIMD_INFINITY;
    // info->m_upperLimit[srow] = SIMD_INFINITY;
