...
void btSliderConstraint::getInfo2NonVirtual(
  ...
  if (m_useOffsetForConstraintFrame) {
    ...
    tmpB = relB.cross(q);
    if (hasStaticBody &&
        getSolveAngLimit()) {  // to make constraint between static and dynamic
                               // objects more rigid
  ...
  if (limit || powered) {
    ...
    // to the two bodies do not create a torque couple. in other words, the
    // points that the constraint force is applied at must lie along the same
    // ax1 axis. a torque couple will result in limited slider-jointed free
