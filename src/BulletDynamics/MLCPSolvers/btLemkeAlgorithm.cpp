...
// original author Math library was replaced from fmatvec to a the file
// src/LinearMath/btMatrixX.h STL/std::vector replaced by btAlignedObjectArray
...
btVectorXu btLemkeAlgorithm::solve(unsigned int maxloops /* = 0*/) {
  ...
  if (!greaterZero) {
    ...
    for (steps = 0; steps < maxloops; steps++) {
      ...
      if (z0Row ==
          pivotRowIndex) {  // if z0 leaves the basis the solution is found -->
                            // one last elimination step is necessary
        GaussJordanEliminationStep(A, pivotRowIndex, pivotColIndex, basis);
