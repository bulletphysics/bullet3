...
/// Rigid Body Contact Simulation (John E. Lloyd) " It is a slower but more
/// accurate solver. Increase the m_maxLoops for better convergence, at the cost
/// of more CPU time. The original implementation of the btLemkeAlgorithm was
/// done by Kilian Grundl from the MBSim team
  ...
  virtual bool solveMLCP(const btMatrixXu& A, const btVectorXu& b,
    ...
    if (m_useLoHighBounds) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        // the Lemke solver we need to figure out why it happens, and fix it, or
        // detect it properly)
        if (x[i] > m_maxValue) {
