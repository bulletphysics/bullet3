...
btScalar btMultiBodyConstraintSolver::resolveSingleConstraintRowGeneric(
  ...
  if (c.m_multiBodyA) {
    ...
    // iterations it would make the multibody solver more like the regular one
    // with m_deltaVelocities being equivalent to
...
void btMultiBodyConstraintSolver::setupMultiBodyContactConstraint(
  ...
  if (multiBodyA) {
    ...
    solverConstraint.m_angularComponentA =
        rb0 ? rb0->getInvInertiaTensorWorld() * torqueAxis0 *
  ...
  if (multiBodyB) {
    ...
    solverConstraint.m_angularComponentB =
        rb1 ? rb1->getInvInertiaTensorWorld() * -torqueAxis1 *
  ...
  // disable warmstarting for btMultiBody, it has issues gaining energy
  // (==explosion)
  if (0)  // infoGlobal.m_solverMode & SOLVER_USE_WARMSTARTING)
  {
    solverConstraint.m_appliedImpulse =
        isFriction ? 0 : cp.m_appliedImpulse * infoGlobal.m_warmstartingFactor;
...

  {
    ...
    btScalar velocityError =
        restitution -
        rel_vel;  // * damping;	//note for friction restitution is always set to
                  // 0 (check above) so it is acutally velocityError = -rel_vel
                  // for friction
    if (isFriction) {
    ...
    if (!isFriction) {
      ...
      /*else
              {
                      //split position and velocity into rhs and
         m_rhsPenetration solverConstraint.m_rhs = velocityImpulse;
...
void btMultiBodyConstraintSolver::setupMultiBodyTorsionalFrictionConstraint(
...

  {
    ...
    if (multiBodyA) {
      ...
      if (rb0) {
        btVector3 iMJaA = rb0 ? rb0->getInvInertiaTensorWorld() *
    ...
    if (multiBodyB) {
      ...
      if (rb1) {
        btVector3 iMJaB = rb1 ? rb1->getInvInertiaTensorWorld() *
      ...
      if (rb0) {
        ...
        rel_vel +=
            solverConstraint.m_contactNormal1.dot(
                rb0 ? solverBodyA->m_linearVelocity +
      ...
      if (rb1) {
        ...
        rel_vel +=
            solverConstraint.m_contactNormal2.dot(
                rb1 ? solverBodyB->m_linearVelocity +
...
btMultiBodyConstraintSolver::addMultiBodyFrictionConstraint(
    ...
    int frictionIndex, btManifoldPoint& cp, btCollisionObject* colObj0,
  ...
  setupMultiBodyContactConstraint(solverConstraint, normalAxis, cp, infoGlobal,
                                  relaxation, isFriction, desiredVelocity,
                                  cfmSlip);
...
void btMultiBodyConstraintSolver::convertMultiBodyContact(
  ...
  //&m_tmpSolverBodyPool[solverBodyIdA]; 	btSolverBody* solverBodyB = mbB ? 0 :
  //&m_tmpSolverBodyPool[solverBodyIdB];

  /// avoid collision response between two static objects
  //	if (!solverBodyA || (solverBodyA->m_invMass.isZero() && (!solverBodyB ||
  //solverBodyB->m_invMass.isZero()))) 	return;
  ...
  for (int j = 0; j < manifold->getNumContacts(); j++) {
    ...
    if (cp.getDistance() <= manifold->getContactProcessingThreshold()) {
      ...
      setupMultiBodyContactConstraint(solverConstraint, cp.m_normalWorldOnB, cp,
                                      infoGlobal, relaxation, isFriction);

      //			const btVector3& pos1 =
      //cp.getPositionWorldOnA(); 			const btVector3& pos2 =
...
#define ENABLE_FRICTION
      ...
      /// recomputed automatically every frame based on the relative linear
      /// velocity. If the relative velocity is zero, it will automatically
      ...
      if (!(infoGlobal.m_solverMode &
                                                                    ...
                                                                    */
        {
          ...
          applyAnisotropicFriction(colObj1, cp.m_lateralFrictionDir1,
                                   btCollisionObject::CF_ANISOTROPIC_FRICTION);
          addMultiBodyFrictionConstraint(cp.m_lateralFrictionDir1, manifold,
                                         frictionIndex, cp, colObj0, colObj1,
                                         relaxation, infoGlobal);
          ...
          if ((infoGlobal.m_solverMode & SOLVER_USE_2_FRICTION_DIRECTIONS)) {
            ...
            applyAnisotropicFriction(
                colObj1, cp.m_lateralFrictionDir2,
                btCollisionObject::CF_ANISOTROPIC_FRICTION);
            addMultiBodyFrictionConstraint(cp.m_lateralFrictionDir2, manifold,
                                           frictionIndex, cp, colObj0, colObj1,
                                           relaxation, infoGlobal);
          }
        ...
        // setMultiBodyFrictionConstraintImpulse( solverConstraint,
        // solverBodyIdA, solverBodyIdB, cp, infoGlobal); todo:
        solverConstraint.m_appliedImpulse = 0.f;
        solverConstraint.m_appliedPushImpulse = 0.f;
      }
...
btScalar btMultiBodyConstraintSolver::solveGroupCacheFriendlyFinish(
  ...
  // (direct velocity change) or as applied force, so we can measure the joint
  // reaction forces easier
