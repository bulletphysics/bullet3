...
void btSimulationIslandManagerMt::buildIslands(
  ...
  // size afterwards, we clean unionfind, to make sure no-one uses it anymore
...
void btSimulationIslandManagerMt::serialIslandDispatch(
    btAlignedObjectArray<Island*>* islandsPtr, IslandCallback* callback) {
  ...
  for (int i = 0; i < islands.size(); ++i) {
    ...
    callback->processIsland(&island->bodyArray[0], island->bodyArray.size(),
                            manifolds, island->manifoldArray.size(),
                            constraintsPtr, island->constraintArray.size(),
                            island->id);
  }
}
...
void btSimulationIslandManagerMt::buildAndProcessIslands(
  ...
  if (!getSplitIslands()) {
    ...
    callback->processIsland(&collisionObjects[0], collisionObjects.size(),
                            manifolds, maxNumManifolds, constraintsPtr,
                            constraints.size(), -1);
  } else {
    initIslandPools();
    ...
    // each Island should have all the necessary bodies, manifolds and
    // constraints.
